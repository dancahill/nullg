...
int config_read(char *section, void *callback)
{
	...
	if (fp==NULL) {
		snprintf(configfile, sizeof(configfile)-1, "%s.conf", SERVER_BASENAME);
		fp=fopen(configfile, "r");
	}
	if (fp==NULL) {
		snprintf(configfile, sizeof(configfile)-1, "../etc/%s.conf", SERVER_BASENAME);
		fp=fopen(configfile, "r");
	}
#ifndef WIN32
	if (fp==NULL) {
		snprintf(configfile, sizeof(configfile)-1, "%s/%s.conf", ETCDIR, SERVER_BASENAME);
		fp=fopen(configfile, "r");
	}
#endif
	if (fp==NULL) {
