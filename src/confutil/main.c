...
void dumpvars(nsp_state *N, obj_t *tobj, int depth)
{
	obj_t *cobj = tobj;
	char indent[20];
	int i;
...
int savefile(nsp_state *N)
{
	...
	if (nsp_isstr(cobj)) {
		if ((fp = fopen(nsp_tostr(N, cobj), "w")) == NULL) {
...
int main(int argc, char *argv[])
{
	...
	printf("===================================\r\n");
	snprintf(sql_type, sizeof(sql_type) - 1, "%s", nsp_getstr(N, tobj, "sql_server_type"));
	...
	*/
	...
	nsp_setstr(N, tobj, "sql_server_type", sql_type, strlen(sql_type));
	if (strcasecmp(sql_type, "MYSQL") == 0) {
		snprintf(sql_host, sizeof(sql_host) - 1, "%s", nsp_getstr(N, tobj, "sql_host_name"));
	...
	if ((strcasecmp(sql_type, "MYSQL") == 0) || (strcasecmp(sql_type, "PGSQL") == 0)) {
		...
		nsp_setstr(N, tobj, "sql_host_name", sql_host, strlen(sql_host));
		...
		nsp_setnum(N, tobj, "sql_port", atoi(line));
		...
		nsp_setstr(N, tobj, "sql_database_name", sql_db, strlen(sql_db));
		...
		nsp_setstr(N, tobj, "sql_user_name", sql_user, strlen(sql_user));
		...
		nsp_setstr(N, tobj, "sql_password", sql_pass, strlen(sql_pass));
		printf("\r\n");
