...
static int sqlite3Callback(void *vptr, int argc, char **argv, char **azColName)
{
	...
	numfields=argc;
	numtuples=nes_getnum(NULL, qobj, "_tuples");
	tobj=nes_getobject(NULL, qobj, "_rows");
	if (tobj->type!=NIL_TTABLE) return -1;
	memset(name, 0, sizeof(name));
	sprintf(name, "%d", numtuples);
	/* get pointer to this record table */
	tobj=nes_settable(NULL, &tobj->d.table, name);
	if (numtuples==0) nes_setnumber(NULL, qobj, "_fields", numfields);
	for (field=0;field<numfields;field++) {
		if (argv==NULL) continue;
		nes_setstring(NULL, &tobj->d.table, azColName[field], (argv[field]!=NULL)?argv[field]:"NULL");
	}
	if (argv!=NULL) nes_setnumber(NULL, qobj, "_tuples", numtuples+1);
	return 0;
}
...
static int sqlite3Query(obj_t **qobj, char *sqlquery)
{
	...
	nes_setstring(NULL, qobj, "_query", sqlquery);
	tobj=nes_settable(NULL, qobj, "_rows");
	if (tobj->type!=NIL_TTABLE) return -1;
	nes_setnumber(NULL, qobj, "_fields", 0);
	nes_setnumber(NULL, qobj, "_tuples", 0);
retry:
...
void _sql_disconnect(nes_state *N)
{
	...
	tobj=nes_getobject(N, &N->globals, "CONFIG");
	if (tobj->type!=NIL_TTABLE) return;
	strncpy(sql_type, nes_tostring(N, nes_getobject(N, &tobj->d.table, "sql_type")), sizeof(sql_type)-1);
...
int _sql_update(nes_state *N, char *sqlquery)
{
	...
	tobj=nes_getobject(N, &N->globals, "CONFIG");
	if (tobj->type!=NIL_TTABLE) return -1;
	strncpy(cnf.sql_type,     nes_getstring(N, &tobj->d.table, "sql_server_type"), sizeof(cnf.sql_type)-1);
	if (!sql_is_connected) {
		strncpy(cnf.lib_path,     nes_getstring(N, &tobj->d.table, "lib_path"    ), sizeof(cnf.lib_path)-1);
//		strncpy(cnf.var_db_path,  nes_getstring(N, &tobj->d.table, "var_db_path" ), sizeof(cnf.var_db_path)-1);
		strncpy(cnf.var_path,     nes_getstring(N, &tobj->d.table, "var_path"    ), sizeof(cnf.var_path)-1);
		strncpy(cnf.sql_odbc_dsn, nes_getstring(N, &tobj->d.table, "sql_odbc_dsn"), sizeof(cnf.sql_odbc_dsn)-1);
		strncpy(cnf.sql_hostname, nes_getstring(N, &tobj->d.table, "sql_hostname"), sizeof(cnf.sql_hostname)-1);
		strncpy(cnf.sql_port,     nes_getstring(N, &tobj->d.table, "sql_port"    ), sizeof(cnf.sql_port)-1);
		strncpy(cnf.sql_dbname,   nes_getstring(N, &tobj->d.table, "sql_dbname"  ), sizeof(cnf.sql_dbname)-1);
		strncpy(cnf.sql_username, nes_getstring(N, &tobj->d.table, "sql_username"), sizeof(cnf.sql_username)-1);
		strncpy(cnf.sql_password, nes_getstring(N, &tobj->d.table, "sql_password"), sizeof(cnf.sql_password)-1);
	}
...
char *sql_getname(nes_state *N, obj_t **qobj, int fieldnumber)
{
	...
	cobj=nes_getobject(N, qobj, "_rows");
...
char *sql_getvalue(nes_state *N, obj_t **qobj, int tuple, int field)
{
	...
	tobj=nes_getobject(N, qobj, "_rows");
	if (tobj->type!=NIL_TTABLE) return "";
...
char *sql_getvaluebyname(nes_state *N, obj_t **qobj, int tuple, char *fieldname)
{
	...
	for (cobj=tobj->d.table; cobj; cobj=cobj->next) {
		...
		if (cobj->type!=NIL_TTABLE) continue;
		if (i!=tuple) { i++; continue; }
		return nes_getstring(N, &cobj->d.table, fieldname);
	}
	return NULL;
}
...
{
	return nes_getnum(N, qobj, "_fields");
}

int sql_numtuples(nes_state *N, obj_t **qobj)
{
	return nes_getnum(N, qobj, "_tuples");
}
/*
