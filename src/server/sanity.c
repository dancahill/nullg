...
*/
#include "main.h"
#include "nullgw/common/records.h"
...
//	if (strcasecmp(proc.config.sql_type, "SQLITE")!=0) {
	...
	if (sanity_dbcheck_table("gw_bookmarks_folders",	"folderid",		BOOKMARKFOLDERFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_calls",			"callid",		CALLFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_calls_actions",		"actionid",		CALLACTIONFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_contacts",			"contactid",		CONTACTFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_contacts_folders",		"folderid",		CONTACTFOLDERFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_contacts_sessions",	"sessionid",		CONTACTSESSIONFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_domains",			"domainid",		DOMAINFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_domains_aliases",		"domainaliasid",	DOMAINALIASFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_email_accounts",		"mailaccountid",	MAILACCTFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_email_filters",		"mailfilterid",		MAILFILTERFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_email_folders",		"mailfolderid",		MAILFOLDERFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_email_headers",		"mailheaderid",		MAILHEADFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_events",			"eventid",		EVENTFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_events_closings",		"eventclosingid",	EVENTCLOSINGFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_events_types",		"eventtypeid",		EVENTTYPEFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_files",			"fileid",		FILEFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_finance_inventory",	"inventoryid",		INVENTORYFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_finance_invoices",		"invoiceid",		INVOICEFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_finance_invoices_items",	"invoiceitemid",	INVOICEITEMFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_forums",			"forumid",		FORUMFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_forums_groups",		"forumgroupid",		FORUMGROUPFIELDS)==-1) checkerror++;
	if (sanity_dbcheck_table("gw_forums_posts",		"messageid",		FORUMPOSTFIELDS)==-1) checkerror++;
	...
	if (sanity_dbcheck_table("gw_users_sessions",		"sessionid",		USERSESSIONFIELDS)==-1) checkerror++;
...
int sanity_checkdirs()
{
	...
	if (sql_numtuples(&sqr)<1) {
		...
		sql_update("INSERT INTO gw_domains (domainid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, domainname) VALUES ('1', '2008-01-01 00:00:00', '2008-01-01 00:00:00', '0', '0', '0', '0', '0', 'default');");
	}
