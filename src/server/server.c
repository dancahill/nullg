...
#endif
{
	...
	case 11:
#ifdef SIGCONTEXT
//		log_error("core", __FILE__, __LINE__, 0, "SIGSEGV EIP=0x%08X, PID=%d", context.eip, getpid());
		log_error("core", __FILE__, __LINE__, 0, "SIGSEGV [%d] Segmentation Violation\n"
...
#if defined(linux)
		{
			...
			log_error("core", __FILE__, __LINE__, 0, "SIGSEGV backtrace() returned %d addresses\n", nptrs);
			strings = backtrace_symbols(buffer, nptrs);
			if (strings!=NULL) {
				for (j = 0; j < nptrs; j++) {
					log_error("core", __FILE__, __LINE__, 0, "SIGSEGV [%s]", strings[j]);
				}
				free(strings);
			}
		}
...
int addlistener(char *modname, TCP_SOCKET *sock, void *fn_getconn, void *fn_doreq, int use_ssl)
{
	...
	for (i=0;i<MAXLISTENERS;i++) {
		...
		if (ioctlsocket(proc.srvlistener[i].socket->socket, FIONBIO, &opt) == SOCKET_ERROR) {
			log_error("core", __FILE__, __LINE__, 0, "failed to ioctl new listener");
		}
#else
		int fd=proc.srvlistener[i].socket->socket;
...
#endif
{
	...
	log_error("core", __FILE__, __LINE__, 2, "Starting accept_loop() thread");
#ifndef WIN32
	pthread_detach(pthread_self());
#endif
	if (pthread_attr_init(&thr_attr)) {
		log_error("core", __FILE__, __LINE__, 0, "pthread_attr_init()");
	...
	for (;;) {
...
#ifdef WIN32
		if (rc==SOCKET_ERROR) {
			log_error("core", __FILE__, __LINE__, 0, "select: %.100s", WSAGetLastError());
			exit(-1);
		}
#else
		if ((rc<0)&&(errno!=EINTR)) {
			log_error("core", __FILE__, __LINE__, 0, "select: %.100s", strerror(errno));
			exit(-1);
		}
#endif
		if (rc<1) {
		...
		for (i=0;i<numfd;i++) {
			...
			if ((getconn=proc.srvlistener[i].fn_getconn)==NULL) {
				log_error("core", __FILE__, __LINE__, 0, "fn_getconn missing");
				/* no function to handle the conn */
				continue;
			}
			if ((conn=getconn())==NULL) {
				log_error("core", __FILE__, __LINE__, 0, "getconn failed");
...
//			fromlen=sizeof(ClientAddr);
			...
			if (rc<0) {
				...
				if (rc==INVALID_SOCKET) {
					log_error("core", __FILE__, __LINE__, 0, "accept_loop() shutting down...");
					return 0;
				}
#else
				if ((errno!=EINTR)&&(errno!=EWOULDBLOCK)) {
					log_error("core", __FILE__, __LINE__, 0, "accept: %.100s", strerror(errno));
					return 0;
				}
#endif
				log_error("core", __FILE__, __LINE__, 0, "accept: %.100s", strerror(errno));
			...
			log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_accept: new %s connection", conn->socket.RemoteAddr, conn->socket.RemotePort, proc.srvlistener[i].modname);
			log_error("core", __FILE__, __LINE__, 4, "%s connection from %s:%d", proc.srvlistener[i].modname, conn->socket.RemoteAddr, conn->socket.RemotePort);
			if (proc.srvlistener[i].use_ssl) ssl_accept(&conn->socket);
			if (pthread_create(&conn->handle, &thr_attr, proc.srvlistener[i].fn_doreq, (void *)conn)==-1) {
				log_error("core", __FILE__, __LINE__, 0, "htloop() failed to start...");
				exit(-1);
			}
		}
	}
...
int startlisteners()
{
	...
	if (pthread_create(&ListenThread, &thr_attr, accept_loop, NULL)==-1) {
		log_error("core", __FILE__, __LINE__, 0, "accept_loop() failed to start...");
		exit(-1);
	}
	return 0;
}
