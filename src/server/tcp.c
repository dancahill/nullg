...
int tcp_accept(int listensock, TCP_SOCKET *sock)
{
	...
	if (clientsock<0) {
		log_error("tcp", __FILE__, __LINE__, 5, "failed tcp_accept");
		return -1;
	}
...
//	log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_accept: new connection", sock->RemoteAddr, sock->RemotePort);
//	timeout.tv_sec=1;
//	timeout.tv_usec=0;
...
//	log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_accept: new connection", sock->RemoteAddr, sock->RemotePort);
	...
	//	log_error("core", __FILE__, __LINE__, 0, "failed to ioctl new accept socket");
	//}
#endif
...
retry:
	...
	if (socket->want_close) {
		log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] socket->want_close=1", socket->RemoteAddr, socket->RemotePort);
		tcp_close(socket, 1);
		return -1;
	}
...
//		if (rc==0) rc=-1;
	...
	rc=recv(socket->socket, buffer, len, flags);
#endif
	//log_error("tcp", __FILE__, __LINE__, 5, "retry %d %d %d %d", socket->want_close, socket->socket, socket->RemotePort, rc);
	if (rc<0) {
#ifdef WIN32
		return -1;
#else
		...
		default:
			log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_recv: %.100s", socket->RemoteAddr, socket->RemotePort, strerror(errno));
			errno=0;
		}
		return -1;
#endif
	...
	log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_recv: %d bytes of binary data", socket->RemoteAddr, socket->RemotePort, rc);
	return rc;
}
...
int tcp_send_nolog(TCP_SOCKET *socket, const char *buffer, int len, int flags)
{
	...
	if (rc<0) {
		...
		if (errno==EWOULDBLOCK) {
			...
			log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_send: %d%.100s", socket->RemoteAddr, socket->RemotePort, errno, strerror(errno));
			errno=0;
		}
#endif
	} else if (rc==0) {
...
int tcp_send(TCP_SOCKET *socket, const char *buffer, int len, int flags)
{
	...
	log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_send: %d bytes of binary data", socket->RemoteAddr, socket->RemotePort, rc);
	return rc;
}
...
int tcp_fprintf(TCP_SOCKET *socket, const char *format, ...)
{
	...
	if ((buffer=calloc(2048, sizeof(char)))==NULL) {
		log_error("core", __FILE__, __LINE__, 0, "OUT OF MEMORY");
		return -1;
	}
	va_start(ap, format);
	vsnprintf(buffer, 2047, format, ap);
	va_end(ap);
	log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_fprintf: %s", socket->RemoteAddr, socket->RemotePort, buffer);
...
retry:
	if (!socket->recvbufsize) {
		...
		/* log_error("tcp", __FILE__, __LINE__, 4, "[%d][%d][%d][%d]", x, sizeof(socket->recvbuf), socket->recvbufoffset, socket->recvbufsize); */
	...
	*pbuffer='\0';
	if (n>max-1) {
		log_error("tcp", __FILE__, __LINE__, 5, "[%s:%d] tcp_fgets: %s", socket->RemoteAddr, socket->RemotePort, buffer);
		return n;
	}
	if (!lf) {
