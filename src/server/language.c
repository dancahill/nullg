...
{
	LANG_CALLBACK language_callback=callback;
	FILE *fp=NULL;
	char file[256];
	char line[512];
	char *pVar;
	...
	snprintf(file, sizeof(file)-1, "%s/share/locale/%s/strings", proc.config.dir_var, langcode);
	if ((fp=fopen(file, "r"))==NULL) {
		log_error("core", __FILE__, __LINE__, 0, "ERROR: Language file '%s' does not exist.", file);
		return -1;
	}
	match=0;
