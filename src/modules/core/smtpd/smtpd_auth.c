...
int auth_login(CONN *sid, char *username, char *password, int mbox)
{
	...
	if (sql_queryf(&sqr, "SELECT * FROM gw_users WHERE username = '%s'", username)<0) {
		return -1;
	}
	...
	if (sql_queryf(&sqr, "SELECT mailaccountid, accountname FROM gw_email_accounts WHERE mailaccountid = %d AND obj_uid = %d order by mailaccountid ASC", mbox, sid->dat->user_uid)<0) {
		return -1;
	}
	i=sql_numtuples(&sqr);
	sql_freeresult(&sqr);
