...
int filter_scan(CONN *sid, char *msgfilename)
{
	...
	if ((err==1)||(err==2)||(err==3)) {
		if (err==1) {
			log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "[%s][SPAM]", sid->dat->user_RemoteAddr);
		} else if (err==2) {
			log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "[%s][VIRUS]", sid->dat->user_RemoteAddr);
		} else if (err==3) {
			log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "[%s][VIRUS+SPAM]", sid->dat->user_RemoteAddr);
		}
		return err;
	}
	return 0;
