...
static int auth_renewcookie(CONN *sid, int settoken)
{
	...
	if (sql_queryf(&sqr, "SELECT userid, domainid FROM gw_users_sessions WHERE remoteip = '%s' AND token = '%s'", sid->dat->in_RemoteAddr, sid->dat->user_token)<0) {
		DEBUG_OUT(sid, "auth_renewcookie()");
		return -1;
	}
	if (sql_numtuples(&sqr)!=1) {
	...
	for (i=0,j=0;i<sql_numfields(&sqr);i++) {
		if (strncasecmp(sql_getname(&sqr, i), "auth", 4)!=0) continue;
		strncpy(sid->dat->auth[j].name, sql_getname(&sqr, i)+4, sizeof(sid->dat->auth[j].name)-1);
		char *ptemp=sid->dat->auth[j].name;
...
int auth_setcookie(CONN *sid)
{
	...
	if ((getxmlparam(sid, 1, "string")!=NULL)&&(getxmlparam(sid, 2, "string")!=NULL)&&(getxmlparam(sid, 3, "string")!=NULL)) {
		...
		if (result==0) {
			md5_init(&c);
			md5_update(&c, (unsigned char *)sid->dat->user_username, strlen(sid->dat->user_username));
	...
	if ((getpostenv(sid, "USERNAME")!=NULL)&&(getpostenv(sid, "PASSWORD")!=NULL)&&(getpostenv(sid, "DOMAIN")!=NULL)) {
		...
		if (result==0) {
			...
			memset(sid->dat->user_token, 0, sizeof(sid->dat->user_token));
			for (i=0;i<MD5_SIZE;i++) strncatf(sid->dat->user_token, sizeof(sid->dat->user_token)-strlen(sid->dat->user_token)-1, "%02x", md[i]);
			if (sql_queryf(&sqr, "SELECT sessionid FROM gw_users_sessions WHERE userid = %d AND domainid = %d ORDER BY obj_mtime DESC", sid->dat->user_uid, sid->dat->user_did)<0) return -1;
			for (i=http_proc.config.http_sesslimit-1;i<sql_numtuples(&sqr);i++) {
				if (i<0) continue;
				sql_updatef("DELETE FROM gw_users_sessions WHERE sessionid = %d AND userid = %d AND domainid = %d", atoi(sql_getvalue(&sqr, i, 0)), sid->dat->user_uid, sid->dat->user_did);
			}
			sql_freeresult(&sqr);
			sql_updatef("INSERT INTO gw_users_sessions (obj_ctime, obj_mtime, obj_uid, obj_did, remoteip, token, userid, domainid) VALUES ('%s', '%s', %d, %d, '%s', '%s', %d, %d)", timebuffer, timebuffer, sid->dat->user_uid, sid->dat->user_did, sid->dat->in_RemoteAddr, sid->dat->user_token, sid->dat->user_uid, sid->dat->user_did);
			memset(password, 0, sizeof(password));
			DEBUG_OUT(sid, "auth_setcookie()");
			return auth_renewcookie(sid, 1);
		}
...
void auth_logout(CONN *sid)
{
	...
	sql_updatef("DELETE FROM gw_users_sessions WHERE token = '%s' AND userid = %d AND domainid = %d", sid->dat->user_token, sid->dat->user_uid, sid->dat->user_did);
	t=time(NULL)+604800;
	strftime(timebuffer, sizeof(timebuffer), "%a, %d %b %Y %H:%M:%S GMT", gmtime(&t));
...
int auth_priv(CONN *sid, char *service)
{
	...
	//return A_READ+A_MODIFY+A_INSERT+A_DELETE+A_ADMIN;
	return authlevel;
}
