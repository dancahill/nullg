...
void ReadPOSTData(CONN *sid)
{
	...
	if (bytesleft<0) {
		log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "ERROR: negative Content-Length of %d provided by client.", sid->dat->in_ContentLength);
		sid->dat->in_ContentLength=0;
		closeconnect(sid, 1);
		return;
	} else if (bytesleft>http_proc.config.http_maxpostsize) {
		log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "ERROR: Content-Length of %d is too large.", sid->dat->in_ContentLength);
		sid->dat->in_ContentLength=0;
		closeconnect(sid, 1);
		return;
	}
	...
	if (sid->PostData==NULL) {
		log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "Memory allocation error while reading POST data.");
		closeconnect(sid, 1);
	}
	...
	if (sid->socket.recvbufsize>bytesleft) {
		log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "ERROR: Content-Length is %d bytes larger than advertised (%d).", sid->socket.recvbufsize>bytesleft, sid->dat->in_ContentLength);
		sid->dat->in_ContentLength=0;
		closeconnect(sid, 1);
		return;
	}
	memcpy(pPostData, sid->socket.recvbuf+sid->socket.recvbufoffset, sid->socket.recvbufsize);
	memset(sid->socket.recvbuf, 0, sizeof(sid->socket.recvbuf));
	pPostData+=sid->socket.recvbufsize;
	bytesleft-=sid->socket.recvbufsize;
	sid->socket.recvbufoffset=0;
	sid->socket.recvbufsize=0;
	while (bytesleft>0) {
		rc=tcp_recv(&sid->socket, pPostData, bytesleft, 0);
		if (rc<0) {
			closeconnect(sid, 1);
			return;
		} else if (rc>0) {
	...
	/* log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "[%s]", sid->PostData); */
	return;
}
...
char *getmimeenv(CONN *sid, char *query, unsigned int *buffersize)
{
	...
	*buffersize=0;
	...
	while ((strncmp(pPostData, boundary2, sizeof(boundary2))!=0)&&(index<sid->dat->in_ContentLength)) {
		...
		if (strncasecmp(pPostData, "Content-Disposition: ", 21)!=0) {
			pPostData++; index++;
			continue;
		}
		pPostData+=21;
		if (strncasecmp(pPostData, "form-data; name=", 16)!=0) {
			pPostData++; index++;
			continue;
		}
		pPostData+=16;
		/*
		 * we just found a new var
		 */
		if (*pPostData=='\"') pPostData++;
		memset(queryt, 0, sizeof(queryt));
		i=0;
		while ((*pPostData)&&(*pPostData!='\"')&&(*pPostData!=';')&&(*pPostData!='\r')&&(*pPostData!='\n')) {
		...
		if (*pPostData=='\n') { pPostData++; index++; }
		/*
		 * i think we're at the var body now.  if the label matches, process the body
		 */
		if (strcmp(queryl, queryt)!=0) {
			while (index<sid->dat->in_ContentLength) {
...
char *getpostenv(CONN *sid, char *query)
{
	...
	pToken=sid->PostData;
	while (*pToken) {
		if ((strncasecmp(pToken, query, strlen(query))!=0)||(pToken[strlen(query)]!='=')) {
...
int read_header(CONN *sid)
{
	...
	x=time(NULL);
	log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "[%s:%d] sid->socket.socket=%d", sid->socket.RemoteAddr, sid->socket.RemotePort, sid->socket.socket);
	do {
		memset(line, 0, sizeof(line));
		if (sid->socket.want_close==1) {
			log_error(MODSHORTNAME, __FILE__, __LINE__, 2, "[%s:%d] - sid->socket.want_close==1 marked for closing", sid->socket.RemoteAddr, sid->socket.RemotePort);
	...
	log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "sid->socket.socket=%d", sid->socket.socket);
	...
	if (*ptemp2) *ptemp2='\0';
/*
	log_error(MODSHORTNAME, __FILE__, __LINE__, 2, "[%s][%s][%s][%s]", line, sid->dat->in_RequestMethod, sid->dat->in_RequestURI, sid->dat->in_Protocol);
*/
	...
	if (strcmp(sid->dat->in_RequestMethod, "POST")==0) {
		if (sid->dat->in_ContentLength<http_proc.config.http_maxpostsize) {
			...
			log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "%s - Large POST (>%d bytes) disallowed", sid->dat->in_RemoteAddr, http_proc.config.http_maxpostsize);
			DEBUG_OUT(sid, "read_header()");
			closeconnect(sid, 1);
			return -1;
		}
...
void http_dorequest(CONN *sid)
{
	...
	log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "sid->socket.socket=%d", sid->socket.socket);
	gettimeofday(&sid->dat->runtime, &tzone);
	proc->stats.http_pages++;
	log_error(MODSHORTNAME, __FILE__, __LINE__, 3, "%s - HTTP Request: %s %s", sid->dat->in_RemoteAddr, sid->dat->in_RequestMethod, sid->dat->in_RequestURI);
	...
	if (strncmp(sid->dat->in_RequestURI, htdir, strlen(htdir))==0) {
		if (filesend(sid, file)==0) goto wrap;
	}
	...
	if (relocate) {
		 ...
		 *
		 * but...   we _are_ the web server..... fixme?
		 */
/*
...
*/
		...
		prints(sid, "<NOSCRIPT><META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL='%s'\"></NOSCRIPT>\r\n", pageuri);
	...
	if (strncmp(sid->dat->in_RequestURI, "/logout", 7)==0) {
		...
		log_error(MODSHORTNAME, __FILE__, __LINE__, 2, "%s - Bad request '%s' by %s", sid->dat->in_RemoteAddr, sid->dat->in_RequestURI, sid->dat->user_username);
	}
		log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "sid->socket.socket=%d", sid->socket.socket);
wrap:
		log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "sid->socket.socket=%d", sid->socket.socket);
	gettimeofday(&ttime, &tzone);
	totaltime=((ttime.tv_sec-sid->dat->runtime.tv_sec)*1000000)+(ttime.tv_usec-sid->dat->runtime.tv_usec);
	log_error(MODSHORTNAME "-runtime", __FILE__, __LINE__, 2, "%1.3f seconds %s", (float)totaltime/1000000, sid->dat->in_RequestURI);
	log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "sid->socket.socket=%d", sid->socket.socket);
	DEBUG_OUT(sid, "dorequest()");
	return;
}
