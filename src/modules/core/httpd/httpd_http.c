...
void read_cgienv(CONN *sid)
{
	...
	if ((ptemp=getenv("CONTENT_LENGTH"))!=NULL) {
		...
		if (sid->dat->in_ContentLength<0) {
			log_error(MODSHORTNAME, __FILE__, __LINE__, 2, "ERROR: negative Content-Length of %d provided by client.", sid->dat->in_ContentLength);
			sid->dat->in_ContentLength=0;
		}
	}
...
int read_header(CONN *sid)
{
	...
	if (*ptemp2) *ptemp2='\0';
/*
	log_error(MODSHORTNAME, __FILE__, __LINE__, 2, "[%s]", line);
	log_error(MODSHORTNAME, __FILE__, __LINE__, 2, "[%s][%s][%s]", sid->dat->in_RequestMethod, sid->dat->in_RequestURI, sid->dat->in_Protocol);
*/
