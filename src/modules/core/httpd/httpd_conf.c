...
static void conf_callback(char *var, char *val)
{
	if (strcmp(var, "interface")==0) {
		...
		log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "unknown configuration directive '%s'", var);
	}
	return;
}
...
int conf_read()
{
	...
	config_read(MODSHORTNAME, conf_callback);
	return 0;
}

int conf_read_modules()
{
	config_read(MODSHORTNAME, conf_callback_modules);
	return 0;
}
