...
DllExport int mod_cron()
{
	...
	for (i=0;i<http_proc.config.http_maxconn;i++) {
		...
		log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "Reaping idle socket [%s:%d] (idle %d seconds)", http_proc.conn[i].socket.RemoteAddr, http_proc.conn[i].socket.RemotePort, ctime-http_proc.conn[i].socket.atime);
