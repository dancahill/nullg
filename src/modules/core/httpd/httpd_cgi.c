...
static void cgi_makeargs(CONN *conn, char *args[])
{
	...
	//		*ptemp='\0';
	//	}
	snprintf(args[0], 254, "%s/cgi-bin/%s", nsp_getstr(proc->N, nsp_settable(proc->N, confobj, "paths"), "var"), progname);
	fixslashes(args[0]);
...
static void cgi_makeenv(CONN *conn, char *env[], char *args[])
{
	obj_t *confobj = nsp_settable(proc->N, &proc->N->g, "CONFIG");
	obj_t *hrobj = nsp_settable(conn->N, &conn->N->g, "_HEADER");
	obj_t *htobj = nsp_settable(conn->N, &conn->N->g, "_SERVER");
	...
	env[n] = calloc(1024, sizeof(char));
	snprintf(env[n++], 1023, "DOCUMENT_ROOT=%s/htdocs", nsp_getstr(proc->N, nsp_settable(proc->N, confobj, "paths"), "var"));
	env[n] = calloc(1024, sizeof(char));
