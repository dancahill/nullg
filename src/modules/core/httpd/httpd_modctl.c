...
*/
	...
	snprintf(libname, sizeof(libname)-1, "%s/" MODSHORTNAME "/%s.%s", config->dir_lib, modname, ext);
	fixslashes(libname);
	...
	return 0;
fail:
	log_error(MODSHORTNAME, __FILE__, __LINE__, 0, "ERROR: Failed to load %s.%s", modname, ext);
