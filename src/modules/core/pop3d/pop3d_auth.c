...
int auth_login(CONN *sid, char *username, char *domain, char *password, int mbox)
{
	...
	if (sql_queryf(proc->N, &qptr, "SELECT mailaccountid, accountname FROM gw_email_accounts WHERE mailaccountid = %d AND obj_uid = %d AND obj_did = %d ORDER BY mailaccountid ASC", mbox, sid->dat->uid, sid->dat->did)<0) {
		return -1;
	}
