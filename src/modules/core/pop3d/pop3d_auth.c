...
int auth_login(CONN *sid, char *username, char *domain, char *password, int mbox)
{
	...
	if (sql_queryf(&sqr, "SELECT mailaccountid, accountname FROM gw_email_accounts WHERE mailaccountid = %d AND obj_uid = %d AND obj_did = %d ORDER BY mailaccountid ASC", mbox, sid->dat->user_uid, sid->dat->user_did)<0) {
		return -1;
	}
	i=sql_numtuples(&sqr);
	sql_freeresult(&sqr);
