...
void pop3_dorequest(CONN *conn)
{
	...
	sql_updatef(proc->N, "INSERT INTO gw_smtp_relayrules (obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, persistence, ipaddress) values ('%s', '%s', %d, 0, %d, 'temp', '%s')", curdate, curdate, conn->dat->did, conn->dat->did, conn->dat->RemoteAddr);
	if (mbox>0) {
		pop3_remote(conn);
	} else {
		pop3_local(conn);
