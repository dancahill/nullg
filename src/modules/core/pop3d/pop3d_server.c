...
#endif
			log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "Error creating directory '%s'", dirname);
			return;
		}
	}
...
static void pop3_remote(CONN *sid)
{
	...
	if (sql_queryf(&sqr, "SELECT mailheaderid, size, uidl FROM gw_email_headers WHERE accountid = %d AND obj_uid = %d AND folder = 1 ORDER BY hdr_date ASC", sid->dat->user_mailcurrent, sid->dat->user_uid)<0) return;
	...
	for (i=0;i<sql_numtuples(&sqr);i++) {
		...
		snprintf(mdir.msg[i]->filename, sizeof(mdir.msg[i]->filename)-1, "%s/%04d/mail/%04d/%04d/%06d.msg", config->dir_var_domains, sid->dat->user_did, sid->dat->user_mailcurrent, 1, mdir.msg[i]->localid);
	...
	for (i=0;i<mdir.mboxtotal;i++) {
		if (mdir.msg[i]->deleted==1) {
			sql_updatef("UPDATE gw_email_headers SET folder = 4 WHERE accountid = %d AND obj_uid = %d AND mailheaderid = %d", sid->dat->user_mailcurrent, sid->dat->user_uid, mdir.msg[i]->localid);
		}
	}
cleanup:
	for (i=0;i<mdir.mboxtotal;i++) {
	...
	do {
		...
		if (strcasecmp(line, "quit")==0) {
			...
			if (auth_login(sid, username, domain, password, mbox)==0) {
				...
				memset(password, 0, sizeof(password));
				mbox=0;
				log_error(MODSHORTNAME, __FILE__, __LINE__, 1, "%s - FAILED AUTH: %s @ %s", sid->dat->user_RemoteAddr, username, domain);
				sleep(2);
