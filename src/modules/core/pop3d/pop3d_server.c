...
static void pop3_local(CONN *sid)
{
	...
	snprintf(dirname, sizeof(dirname)-1, "%s/domains/%04d/mailspool/%s", nes_getstr(proc->N, confobj, "var_path"), sid->dat->did, sid->dat->username);
...
static void pop3_remote(CONN *sid)
{
	...
	if (sql_queryf(proc->N, &qptr, "SELECT mailheaderid, size, uidl FROM gw_email_headers WHERE accountid = %d AND obj_uid = %d AND folder = 1 ORDER BY hdr_date ASC", sid->dat->mailcurrent, sid->dat->uid)<0) return;
	...
	for (i=0;i<sql_numtuples(proc->N, &qptr);i++) {
		...
		snprintf(mdir.msg[i]->filename, sizeof(mdir.msg[i]->filename)-1, "%s/domains/%04d/%04d/%04d/%06d.msg", nes_getstr(proc->N, confobj, "var_path"), sid->dat->did, sid->dat->mailcurrent, 1, mdir.msg[i]->localid);
	...
	for (i=0;i<mdir.mboxtotal;i++) {
		if (mdir.msg[i]->deleted==1) {
			sql_updatef(proc->N, "UPDATE gw_email_headers SET folder = 4 WHERE accountid = %d AND obj_uid = %d AND mailheaderid = %d", sid->dat->mailcurrent, sid->dat->uid, mdir.msg[i]->localid);
		}
	}
cleanup:
	for (i=0;i<mdir.mboxtotal;i++) {
...
void pop3_dorequest(CONN *sid)
{
	...
	do {
		...
		if (strcasecmp(line, "quit")==0) {
			...
			if (auth_login(sid, username, domain, password, mbox)==0) {
				...
				memset(password, 0, sizeof(password));
				mbox=0;
				log_error(proc->N, MODSHORTNAME, __FILE__, __LINE__, 1, "%s - FAILED AUTH: %s @ %s", sid->dat->RemoteAddr, username, domain);
				sleep(2);
	...
	sid->state=1;
	log_access(proc->N, "pop3d", "%s - SUCCESSFUL AUTH: %s @ %s", sid->dat->RemoteAddr, username, domain);
	memset(curdate, 0, sizeof(curdate));
	time_unix2sql(curdate, sizeof(curdate)-1, time(NULL));
	sql_updatef(proc->N, "INSERT INTO gw_smtp_relayrules (obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, persistence, ipaddress) values ('%s', '%s', %d, 0, %d, 'temp', '%s')", curdate, curdate, sid->dat->did, sid->dat->did, sid->dat->RemoteAddr);
	if (mbox>0) {
		pop3_remote(sid);
	} else {
		pop3_local(sid);
