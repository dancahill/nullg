...
#endif
{
	...
	strncpy(sid->dat->user_RemoteAddr, sid->socket.RemoteAddr, sizeof(sid->dat->user_RemoteAddr)-1);
	sid->dat->user_RemotePort=sid->socket.RemotePort;
	pop3_dorequest(sid);
	sid->state=0;
	log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "Closing connection [%s:%d]", sid->socket.RemoteAddr, sid->socket.RemotePort);
...
DllExport int mod_cron()
{
	...
	for (i=0;i<mod_config.pop3_maxconn;i++) {
		...
		log_error(MODSHORTNAME, __FILE__, __LINE__, 4, "Reaping idle socket [%s:%d] (idle %d seconds)", conn[i].socket.RemoteAddr, conn[i].socket.RemotePort, ctime-conn[i].socket.atime);
