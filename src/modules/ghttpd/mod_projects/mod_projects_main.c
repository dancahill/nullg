...
//	project.projectfinish+=time_tzoffset(sid, project.projectfinish);
	...
	htselect_user(sid, project.projectadmin, sid->dat->user_did);
	...
	htselect_user(sid, project.obj_uid, sid->dat->user_did);
	prints(sid, "</SELECT></TD></TR>\r\n");
...
void projectsave(CONN *sid)
{
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		prints(sid, "<SCRIPT LANGUAGE=JavaScript>\r\n<!--\r\nlocation.replace(\"%s/projects/view?projectid=%d\");\r\n// -->\r\n</SCRIPT>\r\n", sid->dat->in_ScriptName, project.projectid);
		prints(sid, "<NOSCRIPT>\r\n<META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL=%s/projects/view?projectid=%d\">\r\n</NOSCRIPT>\r\n", sid->dat->in_ScriptName, project.projectid);
	}
	return;
}
