...
void htselect_forumgroup(CONN *sid, int selected)
{
	...
	if (sql_queryf(&sqr, "SELECT forumgroupid, title FROM gw_forums_groups WHERE obj_did = %d ORDER BY title ASC", sid->dat->user_did)<0) return;
...
char *htview_forumgroup(CONN *sid, int selected)
{
	...
	if (sql_queryf(&sqr, "SELECT title FROM gw_forums_groups WHERE forumgroupid = %d AND obj_did = %d", selected, sid->dat->user_did)<0) return buffer;
...
void fmessageview(CONN *sid, int forumid, int messageid)
{
	...
	if (sql_queryf(&sqr, "SELECT messageid, referenceid FROM gw_forums_posts WHERE forumid = %d AND obj_did = %d ORDER BY messageid ASC", forumid, sid->dat->user_did)<0) return;
	...
	free(btree);
	sql_freeresult(&sqr);
	if (sql_queryf(&sqr, "SELECT messageid, postername, posttime, subject, message FROM gw_forums_posts WHERE messageid = %d and forumid = %d AND obj_did = %d", messageid, forumid, sid->dat->user_did)<0) return;
	prints(sid, "<CENTER>\r\n");
	prints(sid, "<TABLE BORDER=1 CELLPADDING=2 CELLSPACING=0 WIDTH=90%% STYLE='border-style:solid'>\r\n");
...
void fmessagepost(CONN *sid)
{
	...
	if (sql_queryf(&sqr, "SELECT subject FROM gw_forums_posts WHERE messageid = %d and forumid = %d AND obj_did = %d", referenceid, forumid, sid->dat->user_did)<0) return;
	if (sql_numtuples(&sqr)>0) {
	...
	if (longlist==0) {
		...
		if (sql_numtuples(&sqr1)>0) {
			...
			for (i=0;i<sql_numtuples(&sqr1);i++) {
				...
				if (sql_queryf(&sqr2, "SELECT count(messageid) FROM gw_forums_posts where forumid = %d AND obj_did = %d", atoi(sql_getvalue(&sqr1, i, 0)), sid->dat->user_did)<0) return;
...
void fmessagelist(CONN *sid)
{
	...
	forumid=atoi(ptemp);
	forumview(sid, forumid);
	if (sql_queryf(&sqr, "SELECT messageid, referenceid, postername, posttime, subject FROM gw_forums_posts WHERE forumid = %d AND obj_did = %d ORDER BY messageid ASC", forumid, sid->dat->user_did)<0) return;
