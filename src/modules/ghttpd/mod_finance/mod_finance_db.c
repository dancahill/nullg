...
int db_read(CONN *sid, short int perm, short int table, int index, void *record)
{
	...
	if (index==0) {
		...
		case DB_INVOICE:
			...
			rec.invoice->userid=sid->dat->user_uid;
	...
	switch (table) {
	case DB_INVENTORY:
		if (sql_queryf(&sqr, "SELECT * FROM gw_finance_inventory where inventoryid = %d AND obj_did = %d", index, sid->dat->user_did)<0) return -1;
		break;
	case DB_INVOICE:
		if (sql_queryf(&sqr, "SELECT * FROM gw_finance_invoices where invoiceid = %d AND obj_did = %d", index, sid->dat->user_did)<0) return -1;
		break;
	case DB_INVOICEITEMS:
		if (sql_queryf(&sqr, "SELECT * FROM gw_finance_invoices_items where invoiceitemid = %d AND obj_did = %d", index, sid->dat->user_did)<0) return -1;
		break;
	default:
		return -1;
		break;
...
int dbread_account(CONN *sid, short int perm, int index, REC_FIN_ACCOUNT *account)
{
	int authlevel;
	SQLRES sqr;
	...
	if (!(authlevel&A_INSERT)&&(index==0)) return -1;
	if (index==0) {
		account->obj_uid=sid->dat->user_uid;
		account->obj_gid=sid->dat->user_gid;
		account->obj_did=sid->dat->user_did;
		account->obj_gperm=1;
		account->obj_operm=1;
		return 0;
	}
//	if (authlevel&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_finance_accounts where accountid = %d AND obj_did = %d", index, sid->dat->user_did)<0) return -1;
//	} else {
//		if (sql_queryf(&sqr, "SELECT * FROM gw_finance_accounts where accountid = %d and (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d) AND obj_did = %d", index, sid->dat->user_uid, sid->dat->user_gid, perm, perm, sid->dat->user_did)<0) return -1;
//	}
	if (sql_numtuples(&sqr)!=1) {
		sql_freeresult(&sqr);
		return -2;
...
int dbwrite_account(CONN *sid, int index, REC_FIN_ACCOUNT *account)
{
	char curdate[32];
	char query[12288];
	int authlevel;
