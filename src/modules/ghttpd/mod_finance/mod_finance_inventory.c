...
void inventorylist(CONN *sid)
{
	...
	if (sql_queryf(&sqr, "SELECT inventoryid, itemname, category, unitprice, internalcost FROM gw_finance_inventory WHERE obj_did = %d ORDER BY itemname ASC", sid->dat->user_did)<0) return;
	prints(sid, "<CENTER>\r\n");
...
void inventorysave(CONN *sid)
{
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		if (sql_updatef("DELETE FROM gw_finance_inventory WHERE inventoryid = %d AND obj_did = %d", inventory.inventoryid, sid->dat->user_did)<0) return;
		prints(sid, "<CENTER>Inventory Item %d deleted successfully</CENTER><BR>\r\n", inventory.inventoryid);
		db_log_activity(sid, "inventory", inventory.inventoryid, "delete", "%s - %s deleted inventory item %d", sid->dat->in_RemoteAddr, sid->dat->user_username, inventory.inventoryid);
		...
		strncatf(query, sizeof(query)-strlen(query)-1, "'%d', '%s', '%s', '%d', '0', '%d', '0', '0', ", inventory.inventoryid, curdate, curdate, sid->dat->user_uid, sid->dat->user_did);
		...
		db_log_activity(sid, "inventory", inventory.inventoryid, "insert", "%s - %s added inventory item %d", sid->dat->in_RemoteAddr, sid->dat->user_username, inventory.inventoryid);
	} else {
		if (!(auth_priv(sid, "finance")&A_ADMIN)) {
			prints(sid, "<BR><CENTER>%s</CENTER><BR>\r\n", lang.err_noaccess);
			return;
		...
		strncatf(query, sizeof(query)-strlen(query)-1, " WHERE inventoryid = %d AND obj_did = %d", inventory.inventoryid, sid->dat->user_did);
		if (sql_update(query)<0) return;
		prints(sid, "<CENTER>Inventory Item %d modified successfully</CENTER><BR>\r\n", inventory.inventoryid);
		db_log_activity(sid, "inventory", inventory.inventoryid, "modify", "%s - %s modified inventory item %d", sid->dat->in_RemoteAddr, sid->dat->user_username, inventory.inventoryid);
	}
