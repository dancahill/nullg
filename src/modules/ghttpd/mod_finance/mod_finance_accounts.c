...
void account_edit(CONN *sid)
{
	...
	prints(sid, "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=100%%>\r\n");
	if ((account.obj_uid==sid->dat->user_uid)||(auth_priv(sid, "finance")&A_ADMIN)) {
		...
		htselect_user(sid, account.obj_uid, sid->dat->user_did);
		...
		htselect_group(sid, auth_priv(sid, "finance"), account.obj_gid, sid->dat->user_did);
		prints(sid, "</SELECT></TD></TR>\r\n");
...
void account_view(CONN *sid)
{
	...
	prints(sid, "</TABLE>\r\n");
	if (sql_queryf(&sqr, "SELECT entryid, entrydate, accountid, contactid, debit, credit, details FROM gw_finance_journal WHERE accountid = %d AND obj_did = %d ORDER BY entrydate ASC, entryid ASC", account.accountid, sid->dat->user_did)<0) return;
...
void account_list(CONN *sid)
{
	...
	prints(sid, "<CENTER>\r\n");
	if (sql_queryf(&sqr, "SELECT accountid, accountname FROM gw_finance_accounts WHERE obj_did = %d ORDER BY accountid ASC", sid->dat->user_did)<0) return;
