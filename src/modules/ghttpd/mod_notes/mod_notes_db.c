...
int dbread_note(CONN *sid, short int perm, int index, REC_NOTE *note)
{
	...
	if (index==0) {
		note->obj_uid=sid->dat->user_uid;
		note->obj_gid=sid->dat->user_gid;
		note->obj_did=sid->dat->user_did;
		note->obj_gperm=1;
		note->obj_operm=1;
...
int dbwrite_note(CONN *sid, int index, REC_NOTE *note)
{
	...
	if (index==0) {
		...
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "'%d', '%s', '%s', '%d', '%d', '%d', '%d', '%d', ", note->noteid, curdate, curdate, note->obj_uid, note->obj_gid, note->obj_did, note->obj_gperm, note->obj_operm);
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "'%s', ", str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, note->tablename));
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "'%d', ", note->tableindex);
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "'%s', ", str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, note->notetitle));
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "'");
		strncat(query, note->notetext, QUERYBUFSIZE-strlen(query)-3);
		strncat(query, "')", QUERYBUFSIZE-strlen(query)-1);
		rc=sql_update(query);
		...
		query=calloc(QUERYBUFSIZE, sizeof(char));
		snprintf(query, QUERYBUFSIZE-1, "UPDATE gw_notes SET obj_mtime = '%s', obj_uid = '%d', obj_gid = '%d', obj_gperm = '%d', obj_operm = '%d', ", curdate, note->obj_uid, note->obj_gid, note->obj_gperm, note->obj_operm);
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "tablename = '%s', ", str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, note->tablename));
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "tableindex = '%d', ", note->tableindex);
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "notetitle = '%s', ", str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, note->notetitle));
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, "notetext = '");
		strncat(query, note->notetext, QUERYBUFSIZE-strlen(query)-3);
		strncat(query, "'", QUERYBUFSIZE-strlen(query)-27);
		strncatf(query, QUERYBUFSIZE-strlen(query)-1, " WHERE noteid = %d", note->noteid);
		rc=sql_update(query);
		if (query!=NULL) { free(query); query=NULL; }
		if (rc<0) return -1;
		return note->noteid;
