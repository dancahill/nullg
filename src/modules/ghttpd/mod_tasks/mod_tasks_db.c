...
int dbread_task(CONN *sid, short int perm, int index, REC_TASK *task)
{
	...
	task->contactid  =atoi(sql_getvaluebyname(&sqr, 0, "contactid"));
	task->projectid  =atoi(sql_getvaluebyname(&sqr, 0, "projectid"));
	task->duedate    =time_sql2unix(sql_getvaluebyname(&sqr, 0, "duedate"));
	task->priority   =atoi(sql_getvaluebyname(&sqr, 0, "priority"));
	task->reminder   =atoi(sql_getvaluebyname(&sqr, 0, "reminder"));
	task->status     =atoi(sql_getvaluebyname(&sqr, 0, "status"));
	strncpy(task->details, sql_getvaluebyname(&sqr, 0, "details"), sizeof(task->details)-1);
	sql_freeresult(&sqr);
	return 0;
}
