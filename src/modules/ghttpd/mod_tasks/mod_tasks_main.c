...
void taskedit(CONN *sid)
{
	...
	prints(sid, "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=500>\r\n");
	...
	htselect_user(sid, task.assignedto, sid->dat->user_did);
	...
	htselect_user(sid, task.obj_uid, sid->dat->user_did);
	prints(sid, "</SELECT></TD></TR>\r\n");
	...
	htselect_group(sid, auth_priv(sid, "calendar"), task.obj_gid, sid->dat->user_did);
	prints(sid, "</SELECT></TD></TR>\r\n");
