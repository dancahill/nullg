...
void calendarreminders(CONN *sid)
{
	...
	for (i=0;i<sql_numtuples(&sqr);i++) {
		a = (int)(time_sql2unix(sql_getvalue(&sqr, i, 2)) - time(NULL));
...
void calendaredit(CONN *sid, REC_EVENT *event)
{
	...
	prints(sid, "// -->\r\n</SCRIPT>\r\n");
	prints(sid, "<script language=JavaScript src=\"%s/groupware/js/calpopup.js\"></script>\r\n", sid->dat->in_ScriptName);
	prints(sid, "<CENTER>\r\n");
	if (event->eventid>0) {
	...
	htselect_user(sid, event->assignedto, sid->dat->user_did);
	...
	prints(sid, "</SELECT></TD></TR>\r\n");
	duration = (int)(event->eventfinish - event->eventstart);
	...
	prints(sid, "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=100%%>\r\n");
	prints(sid, "<TR CLASS=\"EDITFORM\"><TD ALIGN=CENTER COLSPAN=2><TEXTAREA WRAP=PHYSICAL NAME=details ROWS=6 COLS=60 style='width:100%%'>%s</TEXTAREA></TD></TR>\r\n", str2html(sid, event->details));
	prints(sid, "</TABLE>\r\n");
	prints(sid, "</DIV>\r\n");
	...
	if ((event->obj_uid==sid->dat->user_uid)||(auth_priv(sid, "calendar")&A_ADMIN)) {
		...
		htselect_user(sid, event->obj_uid, sid->dat->user_did);
		prints(sid, "</SELECT></TD></TR>\r\n");
