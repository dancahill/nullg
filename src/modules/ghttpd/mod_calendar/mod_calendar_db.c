...
int dblist_events(CONN *sid, SQLRES *sqr, char *startdate, char *enddate)
{
	if (auth_priv(sid, "calendar")&A_ADMIN) {
		return sql_queryf(sqr, "SELECT eventid, eventstart, eventfinish, status, assignedto, eventname FROM gw_events where eventstart >= '%s' and eventstart < '%s' and obj_did = %d ORDER BY eventstart ASC", startdate, enddate, sid->dat->user_did);
	} else {
...
int dbread_event(CONN *sid, short int perm, int index, REC_EVENT *event)
{
	...
	if (authlevel&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_events where eventid = %d and obj_did = %d", index, sid->dat->user_did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_events where eventid = %d and (obj_uid = %d or assignedby = %d or assignedto = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d) and obj_did = %d", index, sid->dat->user_uid, sid->dat->user_uid, sid->dat->user_uid, sid->dat->user_gid, perm, perm, sid->dat->user_did)<0) return -1;
	}
	...
	event->status=atoi(sql_getvaluebyname(&sqr, 0, "status"));
	event->closingstatus=atoi(sql_getvaluebyname(&sqr, 0, "closingstatus"));
	strncpy(event->details, sql_getvaluebyname(&sqr, 0, "details"), sizeof(event->details)-1);
