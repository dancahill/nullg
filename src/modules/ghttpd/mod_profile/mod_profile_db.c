...
int dbread_profile(CONN *sid, short int perm, int index, REC_USER *profile)
{
	...
	if (authlevel&A_ADMIN) {
		...
		if (sql_queryf(&sqr, "SELECT * FROM gw_users where userid = %d and (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d)", index, sid->dat->user_uid, sid->dat->user_gid, perm, perm)<0) return -1;
	}
	...
	profile->authemail		= atoi(sql_getvalue(&sqr, 0, 19));
	profile->authfiles		= atoi(sql_getvalue(&sqr, 0, 20));
	profile->authfinance		= atoi(sql_getvalue(&sqr, 0, 21));
	profile->authforums		= atoi(sql_getvalue(&sqr, 0, 22));
	profile->authmessages		= atoi(sql_getvalue(&sqr, 0, 23));
	profile->authprofile		= atoi(sql_getvalue(&sqr, 0, 24));
	profile->authprojects		= atoi(sql_getvalue(&sqr, 0, 25));
	profile->authquery		= atoi(sql_getvalue(&sqr, 0, 26));
	...
	if (profile->authemail&A_ADMIN) profile->authemail=A_READ+A_MODIFY+A_INSERT+A_DELETE+A_ADMIN;
	if (profile->authfiles&A_ADMIN) profile->authfiles=A_READ+A_MODIFY+A_INSERT+A_DELETE+A_ADMIN;
	if (profile->authfinance&A_ADMIN) profile->authfinance=A_READ+A_MODIFY+A_INSERT+A_DELETE+A_ADMIN;
