...
int dirlist(CONN *sid)
{
	...
	if (strncmp(sid->dat->in_RequestURI, "/files/", 7)!=0) return -1;
	memset(uri, 0, sizeof(uri));
	snprintf(uri, sizeof(uri)-1, "%s", sid->dat->in_RequestURI);
	decodeurl(uri);
	if (strcmp("/files/~/", uri)==0) {
		snprintf(uri, sizeof(uri)-1, "/files/users/%s/", sid->dat->user_username);
		snprintf(sid->dat->in_RequestURI, sizeof(sid->dat->in_RequestURI)-1, "/files/users/%s/", sid->dat->user_username);
		snprintf(filename, sizeof(filename)-1, "%s/%04d%s", config->dir_var_domains, sid->dat->user_did, uri);
		while (filename[strlen(filename)-1]=='/') filename[strlen(filename)-1]='\0';
		if (stat(filename, &sb)!=0) {
			snprintf(uri, sizeof(uri)-1, "/files/users/");
			snprintf(sid->dat->in_RequestURI, sizeof(sid->dat->in_RequestURI)-1, "/files/users/");
			snprintf(filename, sizeof(filename)-1, "%s/%04d%s", config->dir_var_domains, sid->dat->user_did, uri);
			while (filename[strlen(filename)-1]=='/') filename[strlen(filename)-1]='\0';
		}
		if (stat(filename, &sb)!=0) {
			snprintf(uri, sizeof(uri)-1, "/files/");
			snprintf(sid->dat->in_RequestURI, sizeof(sid->dat->in_RequestURI)-1, "/files/");
		}
//	} else {
//		snprintf(uri, sizeof(uri)-1, "%s", sid->dat->in_RequestURI);
	}
...
//	pthread_mutex_lock(&Lock.FileList);
...
//				db_log_activity(sid, "files", fileid, "insert", "%s - system uploaded file %d %s", sid->dat->in_RemoteAddr, fileid, dentry->d_name);
	...
	closedir(handle);
//	pthread_mutex_unlock(&Lock.FileList);
	if (!isvalid) {
		sql_freeresult(&sqr1);
		if (sql_queryf(&sqr1, "SELECT fileid, filename, filetype, lastdldate, numdownloads, description FROM gw_files WHERE obj_did = %d AND filepath = '%s' ORDER BY filetype, filename ASC", sid->dat->user_did, uri)<0) return -1;
	}
	...
	for (i=0;i<sql_numtuples(&sqr1);i++) {
		...
		printhex(sid, "%s%s", sid->dat->in_RequestURI, showfile);
