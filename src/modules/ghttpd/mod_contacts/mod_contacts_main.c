...
//		http://maps.google.com/maps?q=1000+rue+de+la+Gauchetiere+Ouest,+Montreal,+Quebec+H3B+4W5&iwloc=A&hl=en
//		snprintf(maddr, sizeof(maddr)-1, "http://www.mapquest.com/maps/map.adp?country=%s&address=%s&city=%s&state=%s", contact->homecountry, contact->homeaddress, contact->homelocality, contact->homeregion);
		snprintf(maddr, sizeof(maddr)-1, "http://maps.google.com/maps?iwloc=A&hl=en&q=%s, %s, %s, %s",
			contact->homeaddress, contact->homelocality, contact->homeregion, contact->homecountry);
	}
	ptemp=maddr;
	while (*ptemp!=0) {
		if (*ptemp==' ') *ptemp='+';
...
//		snprintf(baddr, sizeof(baddr)-1, "http://www.mapquest.com/maps/map.adp?country=%s&address=%s&city=%s&state=%s", contact->workcountry, contact->workaddress, contact->worklocality, contact->workregion);
		snprintf(baddr, sizeof(baddr)-1, "http://maps.google.com/maps?iwloc=A&hl=en&q=%s, %s, %s, %s",
			contact->workaddress, contact->worklocality, contact->workregion, contact->workcountry);
	}
	ptemp=baddr;
	while (*ptemp!=0) {
		if (*ptemp==' ') *ptemp='+';
...
//	} else if ((auth_priv(sid, "contacts")&A_MODIFY)&&(perm>=2)) {
	} else if (auth_priv(sid, "contacts")&A_MODIFY) {
		greytoggle="";
		is_editable=1;
	}
	...
	htselect_user(sid, contact->obj_uid, sid->dat->user_did);
	prints(sid, "</SELECT></TD></TR>\r\n");
	...
	if (contact->contactid>0) {
		...
		if (module_exists("mod_finance")&&(auth_priv(sid, "finance")&A_READ)) {
			if (sql_queryf(&sqr, "SELECT count(invoiceid) FROM gw_finance_invoices WHERE contactid = %d AND obj_did = %d", contact->contactid, sid->dat->user_did)<0) return;
			prints(sid, "<TR CLASS=\"EDITFORM\">");
	...
	if (module_exists("mod_email")&&(auth_priv(sid, "email")&A_READ)&&(strchr(contact->email, '@')!=NULL)) {
		if (sql_queryf(&sqr, "SELECT count(mailheaderid) FROM gw_email_headers WHERE obj_uid = %d AND hdr_from LIKE '%%%s%%' OR hdr_to LIKE '%%%s%%' OR hdr_cc LIKE '%%%s%%' AND status != 'd'", sid->dat->user_uid, contact->email, contact->email, contact->email)<0) return;
		prints(sid, "<TR CLASS=\"EDITFORM\">");
...
void contactlist(CONN *sid)
{
	...
	if (sql_numtuples(&sqr1)>0) {
		...
		for (i=offset;(i<sql_numtuples(&sqr1))&&(i<offset+sid->dat->user_maxlist);i++) {
			prints(sid, "<TR CLASS=\"FIELDVAL\">");
			prints(sid, "<TD NOWRAP style='cursor:hand; border-style:solid' onClick=\"window.location.href='%s/contacts/view?contactid=%d'\">", sid->dat->in_ScriptName, atoi(sql_getvalue(&sqr1, i, 0)));
			prints(sid, "<A HREF=%s/contacts/view?contactid=%d>%s", sid->dat->in_ScriptName, atoi(sql_getvalue(&sqr1, i, 0)), str2html(sid, sql_getvalue(&sqr1, i, 14)));
			if (strlen(sql_getvalue(&sqr1, i, 14))&&strlen(sql_getvalue(&sqr1, i, 15))) prints(sid, ", ");
			if (strlen(sql_getvalue(&sqr1, i, 14))==0&&strlen(sql_getvalue(&sqr1, i, 15))==0) prints(sid, "&nbsp;");
			prints(sid, "%s</A>&nbsp;</TD>", str2html(sid, sql_getvalue(&sqr1, i, 15)));
			prints(sid, "<TD NOWRAP STYLE='border-style:solid'>%s&nbsp;</TD>", str2html(sid, sql_getvaluebyname(&sqr1, i, "organization")));
			prints(sid, "<TD NOWRAP STYLE='border-style:solid'>%s&nbsp;</TD>", str2html(sid, sql_getvaluebyname(&sqr1, i, "worknumber")));
			prints(sid, "<TD NOWRAP STYLE='border-style:solid;border-right-width:0px'>");
			if (strlen(sql_getvalue(&sqr1, i, 22))==0) {
				prints(sid, "&nbsp;</TD>");
			} else if (sid->dat->user_maildefault==0) {
				prints(sid, "<A HREF=\"mailto:%s\">%s</A>&nbsp;</TD>", sql_getvaluebyname(&sqr1, i, "email"), sql_getvaluebyname(&sqr1, i, "email"));
			} else {
				if (sid->dat->user_menustyle>0) {
					prints(sid, "<A HREF=\"javascript:MsgTo('&quot;");
					prints(sid, "%s%s%s", str2html(sid, sql_getvalue(&sqr1, i, 15)), strlen(sql_getvalue(&sqr1, i, 15))?" ":"", str2html(sid, sql_getvalue(&sqr1, i, 14)));
					prints(sid, "&quot; <%s>')\">", sql_getvaluebyname(&sqr1, i, "email"));
				} else {
					prints(sid, "<A HREF=\"%s/mail/write?to=%s\">", sid->dat->in_ScriptName, sql_getvaluebyname(&sqr1, i, "email"));
				}
				prints(sid, "%s</A>&nbsp;</TD>", str2html(sid, sql_getvaluebyname(&sqr1, i, "email")));
			}
			prints(sid, "<INPUT TYPE=hidden NAME=addr%d VALUE=\"%s\">", i-offset, sql_getvaluebyname(&sqr1, i, "email"));
			...
			if (strchr(sql_getvaluebyname(&sqr1, i, "email"), '@')) {
...
void contactcallslist(CONN *sid)
{
	...
	if (sql_numtuples(&sqr1)>0) {
		...
		for (i=0;i<sql_numtuples(&sqr1);i++) {
			...
			duration=(int)(time_sql2unix(sql_getvalue(&sqr1, i, 4))-time_sql2unix(sql_getvalue(&sqr1, i, 3)));
...
void contactsave(CONN *sid)
{
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		prints(sid, "<NOSCRIPT>\r\n<META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL=%s/contacts/list\">\r\n</NOSCRIPT>\r\n", sid->dat->in_ScriptName);
...
skipcheck1:
		...
		if ((ptemp=getpostenv(sid, "CALLSET"))!=NULL) {
			...
			prints(sid, "<NOSCRIPT>\r\n<META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL=%s/contacts/view?contactid=%d\">\r\n</NOSCRIPT>\r\n", sid->dat->in_ScriptName, contact.contactid);
		}
	} else {
		if (!(auth_priv(sid, "contacts")&A_MODIFY)) {
			prints(sid, "<BR><CENTER>%s</CENTER><BR>\r\n", lang.err_noaccess);
