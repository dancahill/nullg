...
void adminactivitylist(CONN *sid)
{
	...
	prints(sid, "<CENTER>\r\n");
	if (userid==0) {
		if (sql_queryf(&sqr, "SELECT activityid, obj_ctime, userid, clientip, category, indexid, action FROM gw_activity WHERE category like '%s' AND obj_did = %d ORDER BY activityid DESC", category, sid->dat->user_did)<0) return;
	} else {
		if (sql_queryf(&sqr, "SELECT activityid, obj_ctime, userid, clientip, category, indexid, action FROM gw_activity WHERE userid = %d AND category like '%s' AND obj_did = %d ORDER BY activityid DESC", userid, category, sid->dat->user_did)<0) return;
	}
...
void adminactivityview(CONN *sid)
{
	...
	logid=atoi(ptemp);
	if (sql_queryf(&sqr1, "SELECT activityid, obj_ctime, userid, clientip, category, indexid, action, details FROM gw_activity WHERE activityid = %d AND obj_did = %d", logid, sid->dat->user_did)<0) return;
