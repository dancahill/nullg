...
void adminuserlist(CONN *sid)
{
	...
	if (rc<0) return;
	if (auth_priv(sid, "domainadmin")&A_ADMIN) {
		rc=sql_queryf(&sqr2, "SELECT gw_groups_members.userid, gw_groups_members.groupid, gw_groups.groupname FROM gw_groups_members, gw_groups WHERE gw_groups_members.groupid = gw_groups.groupid ORDER BY gw_groups.groupname ASC");
	} else {
		rc=sql_queryf(&sqr2, "SELECT gw_groups_members.userid, gw_groups_members.groupid, gw_groups.groupname FROM gw_groups_members, gw_groups WHERE gw_groups_members.groupid = gw_groups.groupid AND gw_groups.obj_did = %d AND gw_groups_members.obj_did = %d ORDER BY gw_groups.groupname ASC", sid->dat->user_did, sid->dat->user_did);
//		rc=sql_queryf(&sqr2, "SELECT groupid, groupname FROM gw_groups WHERE obj_did = %d ORDER BY groupid ASC", sid->dat->user_did);
	...
	if (auth_priv(sid, "domainadmin")&A_ADMIN) {
		rc=sql_queryf(&sqr4, "SELECT domainid, domainname FROM gw_domains ORDER BY domainname ASC");
	} else {
	...
	for (i=0;i<sql_numtuples(&sqr1);i++) {
		...
		prints(sid, "<TR CLASS=\"FIELDVAL\"><TD NOWRAP VALIGN=top style='cursor:hand; border-style:solid' onClick=\"window.location.href='%s/admin/useredit?userid=%d'\">", sid->dat->in_ScriptName, atoi(sql_getvalue(&sqr1, i, 0)));
		...
		prints(sid, "<TD NOWRAP VALIGN=top STYLE='border-style:solid'>%s", str2html(sid, sql_getvalue(&sqr1, i, 4)));
		...
		for (j=0;j<sql_numtuples(&sqr3);j++) {
			if (atoi(sql_getvalue(&sqr3, j, 0))==atoi(sql_getvalue(&sqr1, i, 6))) {
				prints(sid, "<TD NOWRAP VALIGN=top STYLE='border-style:solid'>%s</TD>", str2html(sid, sql_getvalue(&sqr3, j, 1)));
				match++;
				break;
			}
		}
		...
		for (j=0;j<sql_numtuples(&sqr2);j++) {
			if (atoi(sql_getvalue(&sqr2, j, 0))==atoi(sql_getvalue(&sqr1, i, 0))) {
				if (match) prints(sid, ", ");
				prints(sid, "<A HREF=%s/admin/groupedit?groupid=%d>%s</A>", sid->dat->in_ScriptName, atoi(sql_getvalue(&sqr2, j, 1)), str2html(sid, sql_getvalue(&sqr2, j, 2)));
				match++;
			}
		}
		if (!match) prints(sid, "&nbsp;");
		prints(sid, "</TD>");
		if (auth_priv(sid, "domainadmin")&A_ADMIN) {
			prints(sid, "<TD STYLE='border-style:solid'><A HREF=%s/admin/su?uid=%d&did=%d>su</A></TD>", sid->dat->in_ScriptName, atoi(sql_getvalue(&sqr1, i, 0)), thisdomain);
		}
		prints(sid, "</TR>\r\n");
	}
	prints(sid, "</TABLE>\r\n");
