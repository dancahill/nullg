...
void admin_su(CONN *sid)
{
	...
	domainid=sid->dat->user_did;
	if ((ptemp=getgetenv(sid, "UID"))!=NULL) userid=atoi(ptemp);
	if ((ptemp=getgetenv(sid, "DID"))!=NULL) domainid=atoi(ptemp);
	sql_updatef("UPDATE gw_users_sessions SET userid = %d, domainid = %d WHERE token = '%s'", userid, domainid, sid->dat->user_token);
	prints(sid, "<SCRIPT LANGUAGE=JavaScript TYPE=\"text/javascript\">\r\n<!--\r\nif (self!=parent) open('%s/','_top');\r\n// -->\r\n</SCRIPT>\r\n", sid->dat->in_ScriptName);
	db_log_activity(sid, "users", sid->dat->user_uid, "su", "%s - %s became user %d:%d", sid->dat->in_RemoteAddr, sid->dat->user_username, userid, domainid);
	return;
}
...
DllExport int mod_main(CONN *sid)
{
	...
	if (strncmp(sid->dat->in_RequestURI, "/admin/su", 9)==0)
		admin_su(sid);
