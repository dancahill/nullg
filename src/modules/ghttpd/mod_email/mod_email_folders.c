...
void wmfolder_edit(CONN *sid)
{
	...
	if (strncmp(sid->dat->in_RequestURI, "/mail/folders/editnew", 21)==0) {
		...
		folderid=atoi(ptemp);
		if (sql_queryf(&sqr, "SELECT mailfolderid, accountid, parentfolderid, foldername FROM gw_email_folders WHERE obj_uid = %d and accountid = %d and mailfolderid = %d ORDER BY parentfolderid ASC, foldername ASC", sid->dat->user_uid, accountid, folderid)<0) return;
...
void wmfolder_list(CONN *sid, int accountid)
{
	...
	if (sql_queryf(&sqr1, "SELECT mailfolderid, parentfolderid, foldername FROM gw_email_folders WHERE obj_uid = %d and accountid = %d ORDER BY parentfolderid ASC, foldername ASC", sid->dat->user_uid, accountid)<0) return;
	if (sql_numtuples(&sqr1)<1) {
		sql_freeresult(&sqr1);
		return;
	}
	if (sql_queryf(&sqr2, "SELECT folder, COUNT(mailheaderid), SUM(size) FROM gw_email_headers WHERE obj_uid = %d AND accountid = %d AND status != 'd' GROUP BY folder", sid->dat->user_uid, accountid)<0) {
		sql_freeresult(&sqr1);
		return;
	}
...
void wmfolder_save(CONN *sid)
{
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		if (sql_queryf(&sqr, "SELECT mailheaderid FROM gw_email_headers WHERE obj_uid = %d AND accountid = %d AND folder = %d", sid->dat->user_uid, accountid, folderid)<0) return;
		...
		sql_freeresult(&sqr);
		if (sql_queryf(&sqr, "SELECT mailfolderid FROM gw_email_folders WHERE obj_uid = %d AND accountid = %d AND parentfolderid = %d", sid->dat->user_uid, accountid, folderid)<0) return;
		if (sql_numtuples(&sqr)>0) {
			prints(sid, "<CENTER><B>This folder cannot be deleted unless it is empty.</B></CENTER>\r\n");
			sql_freeresult(&sqr);
			return;
