...
int dbread_mailaccount(CONN *sid, short int perm, int index, REC_MAILACCT *mailacct)
{
	...
	if (authlevel&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_email_accounts where mailaccountid = %d", index)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_email_accounts where mailaccountid = %d and obj_uid = %d", index, sid->dat->user_uid)<0) return -1;
	}
	...
	mailacct->popssl=atoi(sql_getvalue(&sqr, 0, 16));
	strncpy(mailacct->smtphost,	sql_getvalue(&sqr, 0, 17), sizeof(mailacct->smtphost)-1);
	mailacct->smtpport=atoi(sql_getvalue(&sqr, 0, 18));
	mailacct->smtpssl=atoi(sql_getvalue(&sqr, 0, 19));
	strncpy(mailacct->popusername,	sql_getvalue(&sqr, 0, 20), sizeof(mailacct->popusername)-1);
//	strncpy(mailacct->poppassword,	decode_b64s(sid, sql_getvalue(&sqr, 0, 21)), sizeof(mailacct->poppassword)-1);
	strncpy(mailacct->poppassword,  DecodeBase64string(sid, sql_getvalue(&sqr, 0, 21)), sizeof(mailacct->poppassword)-1);
	strncpy(mailacct->smtpauth,	sql_getvalue(&sqr, 0, 22), sizeof(mailacct->smtpauth)-1);
	mailacct->lastcount=atoi(sql_getvalue(&sqr, 0, 23));
	mailacct->notify=atoi(sql_getvalue(&sqr, 0, 24));
	mailacct->remove=atoi(sql_getvalue(&sqr, 0, 25));
	mailacct->lastcheck=time_sql2unix(sql_getvalue(&sqr, 0, 26));
	ptemp=sql_getvalue(&sqr, 0, 27);
	if (tolower(ptemp[0])=='y') {
		mailacct->showdebug=1;
	}
	strncpy(mailacct->signature,	sql_getvalue(&sqr, 0, 28), sizeof(mailacct->signature)-1);
	sql_freeresult(&sqr);
	return 0;
}
...
int dbread_mailcurrent(CONN *sid, int mailcurrent)
{
	...
	if (sql_queryf(&sqr, "SELECT realname, organization, popusername, poppassword, smtpauth, hosttype, pophost, popport, popssl, smtphost, smtpport, smtpssl, address, replyto, remove, showdebug, signature FROM gw_email_accounts where mailaccountid = %d and obj_uid = %d", mailcurrent, sid->dat->user_uid)<0) return -1;
	if (sql_numtuples(&sqr)!=1) {
		sql_freeresult(&sqr);
		if (sql_queryf(&sqr, "SELECT mailaccountid FROM gw_email_accounts where obj_uid = %d", sid->dat->user_uid)<0) return -1;
		if (sql_numtuples(&sqr)<1) {
			sql_freeresult(&sqr);
			return -1;
		}
	...
	if (sql_numtuples(&sqr)==1) {
		...
		sid->dat->wm->popssl=atoi(sql_getvalue(&sqr, 0, 8));
		strncpy(sid->dat->wm->smtpserver,   sql_getvalue(&sqr, 0, 9), sizeof(sid->dat->wm->smtpserver)-1);
		sid->dat->wm->smtpport=atoi(sql_getvalue(&sqr, 0, 10));
		sid->dat->wm->smtpssl=atoi(sql_getvalue(&sqr, 0, 11));
		strncpy(sid->dat->wm->address,      sql_getvalue(&sqr, 0, 12), sizeof(sid->dat->wm->address)-1);
		strncpy(sid->dat->wm->replyto,      sql_getvalue(&sqr, 0, 13), sizeof(sid->dat->wm->replyto)-1);
		sid->dat->wm->remove=atoi(sql_getvalue(&sqr, 0, 14));
		ptemp=sql_getvalue(&sqr, 0, 15);
		if (tolower(ptemp[0])=='y') {
			sid->dat->wm->showdebug=1;
		}
		strncpy(sid->dat->wm->signature,    sql_getvalue(&sqr, 0, 16), sizeof(sid->dat->wm->signature)-1);
	}
	sql_freeresult(&sqr);
	return 0;
}
