...
//		sql_updatef("UPDATE gw_email_headers SET size = %d WHERE mailheaderid = %d AND accountid = %d AND obj_did = %d", (int)sb.st_size, messageid, accountid, sid->dat->user_did);
//	}
	if ((err==1)||(err==2)||(err==3)) {
		...
		if (err==1) {
			sql_updatef("UPDATE gw_email_headers SET folder = 7, status = 'o', hdr_subject = '%s', hdr_contenttype = '%s', hdr_boundary = '%s', hdr_scanresult = '[SPAM]' WHERE mailheaderid = %d AND accountid = %d AND obj_did = %d", header->Subject, header->contenttype, header->boundary, messageid, accountid, sid->dat->user_did);
			prints(sid, "[SPAM]");
		} else if (err==2) {
			sql_updatef("UPDATE gw_email_headers SET folder = 7, status = 'o', hdr_subject = '%s', hdr_contenttype = '%s', hdr_boundary = '%s', hdr_scanresult = '[VIRUS]' WHERE mailheaderid = %d AND accountid = %d AND obj_did = %d", header->Subject, header->contenttype, header->boundary, messageid, accountid, sid->dat->user_did);
			prints(sid, "[VIRUS]");
		} else if (err==3) {
			sql_updatef("UPDATE gw_email_headers SET folder = 7, status = 'o', hdr_subject = '%s', hdr_contenttype = '%s', hdr_boundary = '%s', hdr_scanresult = '[VIRUS+SPAM]' WHERE mailheaderid = %d AND accountid = %d AND obj_did = %d", header->Subject, header->contenttype, header->boundary, messageid, accountid, sid->dat->user_did);
...
//	snprintf(msgfilename, sizeof(msgfilename)-1, "%s/%04d/mail/%04d/%04d/%06d.tmp", config->dir_var_domains, sid->dat->user_did, sid->dat->user_mailcurrent, 0, messageid);
	snprintf(msgfilename, sizeof(msgfilename)-1, "%s/%04d/mail/%04d/%04d/%06d.msg", config->dir_var_domains, sid->dat->user_did, sid->dat->user_mailcurrent, 0, messageid);
	fixslashes(msgfilename);
	rc=wmfilter_scan(sid, header, msgfilename, accountid, messageid);
	if (rc>0) return rc;
	dstmbox=1;
	if (sql_queryf(&sqr1, "SELECT header, string, rule, action, dstfolderid FROM gw_email_filters WHERE obj_uid = %d AND accountid = %d AND obj_did = %d ORDER BY mailfilterid ASC", sid->dat->user_uid, accountid, sid->dat->user_did)<0) return -1;
	if (sql_numtuples(&sqr1)<1) {
		sql_freeresult(&sqr1);
		sql_updatef("UPDATE gw_email_headers SET folder = %d WHERE mailheaderid = %d AND accountid = %d AND obj_did = %d", dstmbox, messageid, accountid, sid->dat->user_did);
...
testrule:
	sql_freeresult(&sqr1);
	if (sql_queryf(&sqr1, "SELECT mailfolderid FROM gw_email_folders WHERE obj_uid = %d AND obj_did = %d AND accountid = %d AND mailfolderid = %d", sid->dat->user_uid, sid->dat->user_did, accountid, dstmbox)<0) return -1;
	if (sql_numtuples(&sqr1)<1) {
		dstmbox=-1;
	}
	sql_freeresult(&sqr1);
...
//		snprintf(string,     sizeof(filtername)-1, "");
		...
		filterid=atoi(ptemp);
		if (sql_queryf(&sqr, "SELECT mailfilterid, accountid, filtername, header, string, rule, action, dstfolderid FROM gw_email_filters WHERE obj_uid = %d AND obj_did = %d AND accountid = %d AND mailfilterid = %d ORDER BY mailfilterid ASC", sid->dat->user_uid, sid->dat->user_did, accountid, filterid)<0) return;
...
void wmfilter_list(CONN *sid, int accountid)
{
	...
	if (sql_queryf(&sqr1, "SELECT mailfilterid, filtername FROM gw_email_filters WHERE obj_uid = %d AND obj_did = %d AND accountid = %d ORDER BY mailfilterid ASC", sid->dat->user_uid, sid->dat->user_did, accountid)<0) return;
	prints(sid, "<CENTER>");
