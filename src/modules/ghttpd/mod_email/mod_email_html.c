...
void htselect_mailfolder(CONN *sid, int selected, short int allow_zero, short int show_root)
{
	...
	if (sql_queryf(&sqr, "SELECT mailfolderid, parentfolderid, foldername FROM gw_email_folders WHERE obj_uid = %d and accountid = %d ORDER BY parentfolderid ASC, foldername ASC", sid->dat->user_uid, sid->dat->user_mailcurrent)<0) return;
...
void htselect_mailaccount(CONN *sid, int selected)
{
	...
	if (sql_queryf(&sqr, "SELECT mailaccountid, accountname FROM gw_email_accounts WHERE obj_uid = %d AND obj_did = %d ORDER BY mailaccountid ASC", sid->dat->user_uid, sid->dat->user_did)<0) return;
	for (i=0;i<sql_numtuples(&sqr);i++) {
...
void htselect_mailjump(CONN *sid, int selected)
{
	...
	if (sql_queryf(&sqr, "SELECT mailaccountid, accountname FROM gw_email_accounts WHERE obj_uid = %d AND obj_did = %d ORDER BY accountname ASC", sid->dat->user_uid, sid->dat->user_did)<0) return;
	if ((sql_numtuples(&sqr)>0)&&(sid->dat->user_mailcurrent==0)) {
		sid->dat->user_mailcurrent=atoi(sql_getvalue(&sqr, 0, 0));
		sql_updatef("UPDATE gw_users SET prefmailcurrent = %d WHERE userid = %d AND domainid = %d", sid->dat->user_mailcurrent, sid->dat->user_uid, sid->dat->user_did);
	}
