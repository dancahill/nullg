...
void wmnotice(CONN *sid)
{
	...
	if (sql_queryf(&sqr1, "SELECT mailaccountid, accountname, poppassword FROM gw_email_accounts where obj_uid = %d ORDER BY accountname ASC", sid->dat->user_uid)<0) return;
	for (i=0;i<sql_numtuples(&sqr1);i++) {
		sid->dat->user_mailcurrent=atoi(sql_getvalue(&sqr1, i, 0));
		if (sql_queryf(&sqr2, "SELECT count(mailheaderid) FROM gw_email_headers WHERE obj_uid = %d AND accountid = %d AND folder = 1", sid->dat->user_uid, sid->dat->user_mailcurrent)<0) continue;
		nummessages=atoi(sql_getvalue(&sqr2, 0, 0));
		sql_freeresult(&sqr2);
		if (sql_queryf(&sqr2, "SELECT count(mailheaderid) FROM gw_email_headers WHERE obj_uid = %d AND accountid = %d AND folder = 1 AND status = 'n'", sid->dat->user_uid, sid->dat->user_mailcurrent)<0) continue;
...
void wmloginform(CONN *sid)
{
	...
	if (sql_queryf(&sqr, "SELECT popusername, poppassword FROM gw_email_accounts where mailaccountid = %d and obj_uid = %d", sid->dat->user_mailcurrent, sid->dat->user_uid)<0) return;
...
void wmlogout(CONN *sid)
{
	...
	sql_updatef("UPDATE gw_email_accounts SET obj_mtime = '%s',  poppassword = '' WHERE obj_uid = %d AND mailaccountid = %d", timebuffer, sid->dat->user_uid, sid->dat->user_mailcurrent);
	wmloginform(sid);
	return;
}
...
void webmailraw(CONN *sid)
{
	...
	if (sql_queryf(&sqr, "SELECT mailheaderid, folder, status FROM gw_email_headers WHERE obj_uid = %d and accountid = %d AND mailheaderid = %d and status != 'd'", sid->dat->user_uid, sid->dat->user_mailcurrent, msgnum)<0) return;
	if (sql_numtuples(&sqr)!=1) {
...
void webmaillist(CONN *sid)
{
	...
	if (order<0) order=0;
	if (order>7) order=7;
	if (sql_queryf(&sqr, "SELECT mailaccountid FROM gw_email_accounts WHERE obj_uid = %d", sid->dat->user_uid)<0) return;
	if (sql_numtuples(&sqr)<1) {
		prints(sid, "<SCRIPT LANGUAGE=JavaScript>\r\n<!--\r\n");
...
void webmailread(CONN *sid)
{
	...
	if (order<0) order=0;
	if (order>7) order=7;
	if (sql_queryf(&sqr, "SELECT mailheaderid, folder FROM gw_email_headers WHERE obj_uid = %d and accountid = %d and status != 'd' and mailheaderid = %d", sid->dat->user_uid, sid->dat->user_mailcurrent, localid)<0) return;
	if (sql_numtuples(&sqr)==1) {
		folderid=atoi(sql_getvalue(&sqr, 0, 1));
	}
	sql_freeresult(&sqr);
	...
	sql_updatef("UPDATE gw_email_headers SET status = 'r' WHERE uidl = '%s' AND obj_uid = %d AND accountid = %d", str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, header.uidl), sid->dat->user_uid, sid->dat->user_mailcurrent);
	memset(msgfilename, 0, sizeof(msgfilename));
...
void webmailwrite(CONN *sid)
{
	...
	if (msgnum) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_email_headers WHERE obj_uid = %d and accountid = %d and status != 'd' AND mailheaderid = %d", sid->dat->user_uid, accountid, msgnum)<0) return;
		if (sql_numtuples(&sqr)!=1) {
			sql_freeresult(&sqr);
			return;
		} else {
	...
	prints(sid, "	</select></TD><TD BGCOLOR=#000000 STYLE='padding:1px'></TD><TD STYLE='white-space:nowrap;padding:0px'><IMG\r\n");
	prints(sid, "	ALT='Bold'                   SRC='/groupware/images/wmedit/bold.png'      onClick=wmformat('Bold')                 BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Italic'                 SRC='/groupware/images/wmedit/italic.png'    onClick=wmformat('Italic')               BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Underline'              SRC='/groupware/images/wmedit/underline.png' onClick=wmformat('Underline')            BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Font Colour'            SRC='/groupware/images/wmedit/fontcol.png'   onClick=SetForeColour()                  BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)></TD><TD BGCOLOR=#000000 STYLE='padding:1px'></TD><TD STYLE='white-space:nowrap;padding:0px'><IMG\r\n");
	prints(sid, "	ALT='Formatting Numbers'     SRC='/groupware/images/wmedit/numlist.png'   onClick=wmformat('InsertOrderedList')    BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Formatting Bullets'     SRC='/groupware/images/wmedit/bullist.png'   onClick=wmformat('InsertUnorderedList')  BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Decrease Indentation'   SRC='/groupware/images/wmedit/outdent.png'   onClick=wmformat('Outdent')              BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Increase Indentation'   SRC='/groupware/images/wmedit/indent.png'    onClick=wmformat('Indent')               BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)></TD><TD BGCOLOR=#000000 STYLE='padding:1px'></TD><TD STYLE='white-space:nowrap;padding:0px'><IMG\r\n");
	prints(sid, "	ALT='Align Left'             SRC='/groupware/images/wmedit/left.png'      onClick=wmformat('JustifyLeft')          BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Center'                 SRC='/groupware/images/wmedit/center.png'    onClick=wmformat('JustifyCenter')        BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Align Right'            SRC='/groupware/images/wmedit/right.png'     onClick=wmformat('JustifyRight')         BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Justify'                SRC='/groupware/images/wmedit/justify.png'   onClick=wmformat('Justify')              BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)></TD><TD BGCOLOR=#000000 STYLE='padding:1px'></TD><TD STYLE='white-space:nowrap;padding:0px' width=100%><IMG\r\n");
	prints(sid, "	ALT='Insert Horizontal Line' SRC='/groupware/images/wmedit/rule.png'      onClick=wmformat('InsertHorizontalRule') BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Create a Hyperlink'     SRC='/groupware/images/wmedit/link.png'      onClick=doLink()                         BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Insert Picture'         SRC='/groupware/images/wmedit/image.png'     onClick=doImage()                        BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)></TD>\r\n");
	prints(sid, "</TR></TABLE>\r\n");
