...
#endif
			log_error("mod_email", __FILE__, __LINE__, 1, "ERROR: Maildir '%s' is not accessible!", dirname);
			return -1;
		}
	}
...
int wmfolder_msgmove(CONN *sid, int accountid, int messageid, int srcfolderid, int dstfolderid)
{
	...
	if (srcfolderid==0) {
		snprintf(srcfilename, sizeof(srcfilename)-1, "%s/%04d/mail/%04d/%04d/%06d.tmp", config->dir_var_domains, sid->dat->user_did, accountid, srcfolderid, messageid);
	} else {
...
int wmfolder_makedefaults(CONN *sid, int accountid)
{
	...
	sql_updatef("INSERT INTO gw_email_folders (mailfolderid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, accountid, parentfolderid, foldername) values ('1', '%s', '%s', '%d', '0', '%d', '0', '0', '%d', '0', '%s');", curdate, curdate, sid->dat->user_uid, sid->dat->user_did, accountid, lang.folder1_name);
	sql_updatef("INSERT INTO gw_email_folders (mailfolderid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, accountid, parentfolderid, foldername) values ('2', '%s', '%s', '%d', '0', '%d', '0', '0', '%d', '0', '%s');", curdate, curdate, sid->dat->user_uid, sid->dat->user_did, accountid, lang.folder2_name);
	sql_updatef("INSERT INTO gw_email_folders (mailfolderid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, accountid, parentfolderid, foldername) values ('3', '%s', '%s', '%d', '0', '%d', '0', '0', '%d', '0', '%s');", curdate, curdate, sid->dat->user_uid, sid->dat->user_did, accountid, lang.folder3_name);
	sql_updatef("INSERT INTO gw_email_folders (mailfolderid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, accountid, parentfolderid, foldername) values ('4', '%s', '%s', '%d', '0', '%d', '0', '0', '%d', '0', '%s');", curdate, curdate, sid->dat->user_uid, sid->dat->user_did, accountid, lang.folder4_name);
	sql_updatef("INSERT INTO gw_email_folders (mailfolderid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, accountid, parentfolderid, foldername) values ('5', '%s', '%s', '%d', '0', '%d', '0', '0', '%d', '0', '%s');", curdate, curdate, sid->dat->user_uid, sid->dat->user_did, accountid, lang.folder5_name);
	sql_updatef("INSERT INTO gw_email_folders (mailfolderid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, accountid, parentfolderid, foldername) values ('6', '%s', '%s', '%d', '0', '%d', '0', '0', '%d', '0', '%s');", curdate, curdate, sid->dat->user_uid, sid->dat->user_did, accountid, lang.folder6_name);
	sql_updatef("INSERT INTO gw_email_folders (mailfolderid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, accountid, parentfolderid, foldername) values ('7', '%s', '%s', '%d', '0', '%d', '0', '0', '%d', '0', '%s');", curdate, curdate, sid->dat->user_uid, sid->dat->user_did, accountid, lang.folder7_name);
	return 0;
}
...
void webmailsave(CONN *sid)
{
	...
	if (sql_queryf(&sqr, "SELECT max(mailheaderid) FROM gw_email_headers where accountid = %d", sid->dat->user_mailcurrent)<0) return;
	...
	if (fp==NULL) {
		log_error("mod_email", __FILE__, __LINE__, 1, "ERROR: Could not open message '%s'!", msgfilename);
		return;
	}
	...
	if (forward>0) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_email_headers WHERE obj_uid = %d and accountid = %d and status != 'd' AND mailheaderid = %d", sid->dat->user_uid, fwdacct, forward)<0) goto stuff;
...
stuff:
	...
	sql_updatef("UPDATE gw_email_headers SET size = '%d' WHERE mailheaderid = %d AND accountid = %d", (int)sb.st_size, headerid, sid->dat->user_mailcurrent);
	prints(sid, "OK<BR>\r\n");
...
//	int deleted=0;
	...
	if (sql_queryf(&sqr, "SELECT mailheaderid, uidl, status, folder FROM gw_email_headers WHERE obj_uid = %d and accountid = %d and status != 'd'", sid->dat->user_uid, sid->dat->user_mailcurrent)<0) return;
	...
	for (i=0;i<nummessages;i++) {
		...
		if (folderid>0) {
			...
			if ((oldfolderid==4)&&(folderid==4)) {
				sql_updatef("UPDATE gw_email_headers SET status = 'd' WHERE accountid = %d AND obj_uid = %d and mailheaderid = %d", sid->dat->user_mailcurrent, sid->dat->user_uid, atoi(sql_getvalue(&sqr, i, 0)));
				purge=1;
			} else {
				sql_updatef("UPDATE gw_email_headers SET folder = '%d' WHERE accountid = %d AND obj_uid = %d and mailheaderid = %d", folderid, sid->dat->user_mailcurrent, sid->dat->user_uid, atoi(sql_getvalue(&sqr, i, 0)));
			}
...
void webmailpurge(CONN *sid)
{
	...
	flushbuffer(sid);
	if (sql_queryf(&sqr, "SELECT mailheaderid, folder FROM gw_email_headers WHERE obj_uid = %d AND obj_did = %d AND accountid = %d and status = 'd' ORDER BY mailheaderid ASC", sid->dat->user_uid, sid->dat->user_did, accountid)<0) return;
	err=0;
	memset(msgfilename, 0, sizeof(msgfilename));
	...
	sql_freeresult(&sqr);
	if (err==0) {
		sql_updatef("DELETE FROM gw_email_headers WHERE obj_uid = %d AND obj_did = %d AND accountid = %d and status = 'd'", sid->dat->user_uid, sid->dat->user_did, accountid);
	}
	if (sql_queryf(&sqr, "SELECT mailheaderid, folder FROM gw_email_headers WHERE obj_uid = %d AND obj_did = %d AND accountid = %d ORDER BY mailheaderid ASC", sid->dat->user_uid, sid->dat->user_did, accountid)<0) return;
	for (i=0,j=1;i<sql_numtuples(&sqr);i++) {
		...
		if (rename(msgfilename, newfilename)==0) {
			if (sql_updatef("UPDATE gw_email_headers SET mailheaderid = '%d' WHERE obj_uid = %d AND obj_did = %d AND accountid = %d AND mailheaderid = %d", j, sid->dat->user_uid, sid->dat->user_did, accountid, atoi(sql_getvalue(&sqr, i, 0)))<0) {
