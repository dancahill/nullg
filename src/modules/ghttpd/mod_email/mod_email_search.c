...
int wmsearch_doquery(CONN *sid, SQLRES *sqr, const char *order_by, int folderid)
{
	...
	return sql_queryf(sqr, "SELECT * FROM gw_email_headers WHERE obj_uid = %d and accountid = %d AND folder = %d AND status != 'd' ORDER BY %s", sid->dat->user_uid, sid->dat->user_mailcurrent, folderid, order_by);
}
