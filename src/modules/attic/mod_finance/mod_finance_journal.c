...
/*
	...
	prints(sid, "<CENTER>\r\n");
	if (sql_queryf(&sqr1, "SELECT entryid, entrydate, accountid, debit, credit, details FROM gw_finance_journal WHERE obj_did = %d ORDER BY entrydate ASC, entryid ASC", sid->dat->did)<0) return;
	if (sql_queryf(&sqr2, "SELECT accountid, accountname FROM gw_finance_accounts WHERE obj_did = %d", sid->dat->did)<0) return;
...
/*
	...
	if (accountid>0) {
		if (sql_queryf(&sqr1, "SELECT entryid, entrydate, accountid, debit, credit, details FROM gw_finance_journal WHERE obj_did = %d AND accountid = %d AND entrydate >= '%s' AND entrydate <= '%s' ORDER BY entrydate ASC, entryid ASC", sid->dat->did, accountid, curdate1, curdate2)<0) return;
		if (sql_queryf(&sqr2, "SELECT SUM(debit), SUM(credit) FROM gw_finance_journal WHERE obj_did = %d AND accountid = %d AND entrydate < '%s' ORDER BY entrydate ASC", sid->dat->did, accountid, curdate1)<0) return;
	} else {
		if (sql_queryf(&sqr1, "SELECT entryid, entrydate, accountid, debit, credit, details FROM gw_finance_journal WHERE obj_did = %d AND entrydate >= '%s' AND entrydate <= '%s' ORDER BY entrydate ASC, entryid ASC", sid->dat->did, curdate1, curdate2)<0) return;
		if (sql_queryf(&sqr2, "SELECT SUM(debit), SUM(credit) FROM gw_finance_journal WHERE obj_did = %d AND entrydate < '%s' ORDER BY entrydate ASC", sid->dat->did, curdate1)<0) return;
	}
	if (sql_queryf(&sqr3, "SELECT accountid, accountname FROM gw_finance_accounts WHERE obj_did = %d", sid->dat->did)<0) return;
	if (sql_numtuples(&sqr1)>0) {
		prints(sid, "<TABLE BORDER=1 CELLPADDING=2 CELLSPACING=0 WIDTH=500 STYLE='border-style:solid'>\r\n");
		prints(sid, "<TR><TH STYLE='border-style:solid'>id</TH><TH STYLE='border-style:solid'>date</TH><TH STYLE='border-style:solid'>accountid</TH><TH STYLE='border-style:solid'>details</TH><TH STYLE='border-style:solid'>debit</TH><TH STYLE='border-style:solid'>credit</TH><TH STYLE='border-style:solid'>balance</TH></TR>\r\n");
		balance=0;
