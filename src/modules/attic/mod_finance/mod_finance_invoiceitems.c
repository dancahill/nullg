...
/*
	...
	if (sql_queryf(&sqr, "SELECT inventoryid, itemname FROM gw_finance_inventory WHERE obj_did = %d ORDER BY itemname ASC", sid->dat->did)<0) return;
...
/*
	...
	if (sql_queryf(&sqr, "SELECT inventoryid, itemname FROM gw_finance_inventory WHERE inventoryid = %d AND obj_did = %d", selected, sid->dat->did)<0) return buffer;
...
/*
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		if (sql_updatef("DELETE FROM gw_finance_invoices_items WHERE invoiceitemid = %d AND obj_did = %d", invoiceitem.invoiceitemid, sid->dat->did)<0) return;
		prints(sid, "<CENTER>Invoice item %d deleted successfully</CENTER><BR />\r\n", invoiceitem.invoiceitemid);
		db_log_activity(sid, "invoiceitems", invoiceitem.invoiceitemid, "delete", "%s - %s deleted invoice item %d", sid->dat->in_RemoteAddr, sid->dat->username, invoiceitem.invoiceitemid);
		...
		strncatf(query, sizeof(query)-strlen(query)-1, "'%d', '%s', '%s', '%d', '0', '%d', '0', '0', ", invoiceitem.invoiceitemid, curdate, curdate, sid->dat->uid, sid->dat->did);
		...
		db_log_activity(sid, "invoiceitems", invoiceitem.invoiceitemid, "insert", "%s - %s added invoice item %d", sid->dat->in_RemoteAddr, sid->dat->username, invoiceitem.invoiceitemid);
	} else {
		if (!(auth_priv(sid, "finance")&A_MODIFY)) {
			prints(sid, "<BR /><CENTER>%s</CENTER><BR />\r\n", lang.err_noaccess);
			return;
		...
		strncatf(query, sizeof(query)-strlen(query)-1, " WHERE invoiceitemid = %d AND obj_did = %d", invoiceitem.invoiceitemid, sid->dat->did);
		if (sql_update(query)<0) return;
		prints(sid, "<CENTER>Invoice item %d modified successfully</CENTER><BR />\r\n", invoiceitem.invoiceitemid);
		db_log_activity(sid, "invoiceitems", invoiceitem.invoiceitemid, "modify", "%s - %s modified invoice item %d", sid->dat->in_RemoteAddr, sid->dat->username, invoiceitem.invoiceitemid);
	}
	totaldue=0;
