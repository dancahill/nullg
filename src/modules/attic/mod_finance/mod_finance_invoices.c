...
/*
	...
	htselect_user(sid, invoice.userid, sid->dat->did);
	prints(sid, "</SELECT></TD></TR>\r\n");
...
//				strncatf(query, sizeof(query)-strlen(query)-1, "%s like '%s' or ", sql_getname(&sqr1, i), searchstring);
	...
	strncatf(query, sizeof(query)-strlen(query)-1, ") AND obj_did = %d ORDER BY invoicedate DESC", sid->dat->did);
	sql_freeresult(&sqr1);
	prints(sid, "<CENTER>\r\n");
	...
	if (offset+sid->dat->maxlist<sql_numtuples(&sqr1)) {
		prints(sid, "<FORM METHOD=POST ACTION=%s/finance/invoices/list?offset=%d><TD>\r\n", sid->dat->in_ScriptName, offset+sid->dat->maxlist);
		prints(sid, "<INPUT TYPE=HIDDEN NAME=column VALUE='%s'>\r\n", column);
		prints(sid, "<INPUT TYPE=HIDDEN NAME=searchstring VALUE='%s'>\r\n", searchstring);
...
/*
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		db_log_activity(sid, "invoices", invoice.invoiceid, "delete", "%s - %s deleted invoice %d", sid->dat->in_RemoteAddr, sid->dat->username, invoice.invoiceid);
		...
		strncatf(query, sizeof(query)-strlen(query)-1, "'%d', '%s', '%s', '%d', '0', '%d', '0', '0', ", invoice.invoiceid, curdate, curdate, sid->dat->uid, sid->dat->did);
		...
		db_log_activity(sid, "invoices", invoice.invoiceid, "insert", "%s - %s added invoice %d", sid->dat->in_RemoteAddr, sid->dat->username, invoice.invoiceid);
	} else {
		if (!(auth_priv(sid, "finance")&A_MODIFY)) {
			prints(sid, "<BR /><CENTER>%s</CENTER><BR />\r\n", lang.err_noaccess);
			return;
		...
		db_log_activity(sid, "invoices", invoice.invoiceid, "modify", "%s - %s modified invoice %d", sid->dat->in_RemoteAddr, sid->dat->username, invoice.invoiceid);
	}
	prints(sid, "<SCRIPT LANGUAGE=JavaScript>\r\n<!--\r\n");
