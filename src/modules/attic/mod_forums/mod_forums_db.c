...
int dbread_forumgroup(CONN *sid, short int perm, int index, obj_t **qobj)
{
/*	int authlevel;
	SQLRES sqr;
	...
	if (index==0) {
		forumgroup->obj_uid=sid->dat->uid;
		forumgroup->obj_gid=sid->dat->gid;
		forumgroup->obj_did=sid->dat->did;
	...
	if (authlevel&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_forums_groups where forumgroupid = %d AND obj_did = %d", index, sid->dat->did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_forums_groups where forumgroupid = %d and (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d) AND obj_did = %d", index, sid->dat->uid, sid->dat->gid, perm, perm, sid->dat->did)<0) return -1;
	}
...
int dbwrite_forumgroup(CONN *sid, int index, obj_t **qobj)
{
/*	char curdate[32];
	char query[12288];
	int authlevel;
