...
static int forum_treesort_r(CONN *sid, BTREE *btree, obj_t **qobj, int depth, int parentid, int foldercount)
{
/*	int fid, pid;
	int i;
...
/*	int i, j;
	...
	if (sql_queryf(&sqr, "SELECT forumgroupid, title FROM gw_forums_groups WHERE obj_did = %d ORDER BY title ASC", sid->dat->did)<0) return;
...
/*	char *buffer=getbuffer(sid);
	...
	if (sql_queryf(&sqr, "SELECT title FROM gw_forums_groups WHERE forumgroupid = %d AND obj_did = %d", selected, sid->dat->did)<0) return buffer;
...
/*	REC_FORUMGROUP forumgroup;
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		if (sql_updatef("DELETE FROM gw_forums_groups WHERE forumgroupid = %d", forumgroup.forumgroupid)<0) return;
		prints(sid, "<CENTER>Forum Group %d deleted successfully</CENTER><BR />\r\n", forumgroup.forumgroupid);
		db_log_activity(sid, "forumgroups", forumgroup.forumgroupid, "delete", "%s - %s deleted forum group %d", sid->dat->in_RemoteAddr, sid->dat->username, forumgroup.forumgroupid);
		...
		db_log_activity(sid, "forumgroups", forumgroup.forumgroupid, "insert", "%s - %s added forum group %d", sid->dat->in_RemoteAddr, sid->dat->username, forumgroup.forumgroupid);
	} else {
		...
		db_log_activity(sid, "forumgroups", forumgroup.forumgroupid, "modify", "%s - %s modified forum group %d", sid->dat->in_RemoteAddr, sid->dat->username, forumgroup.forumgroupid);
	}
...
/*	int forumgroupid;
	...
	if (sql_queryf(&sqr, "SELECT forumid, forumgroupid, postername, subject, message FROM gw_forums WHERE forumid = %d AND obj_did = %d", forumid, sid->dat->did)<0) return;
	prints(sid, "<CENTER>\r\n");
...
/*	_btree *btree;
	...
	if (sql_queryf(&sqr, "SELECT messageid, referenceid FROM gw_forums_posts WHERE forumid = %d AND obj_did = %d ORDER BY messageid ASC", forumid, sid->dat->did)<0) return;
	...
	free(btree);
	sql_freeresult(&sqr);
	if (sql_queryf(&sqr, "SELECT messageid, postername, posttime, subject, message FROM gw_forums_posts WHERE messageid = %d and forumid = %d AND obj_did = %d", messageid, forumid, sid->dat->did)<0) return;
	prints(sid, "<CENTER>\r\n");
	prints(sid, "<TABLE BORDER=1 CELLPADDING=2 CELLSPACING=0 WIDTH=90%% STYLE='border-style:solid'>\r\n");
...
/*	char *ptemp;
	...
	if (sql_queryf(&sqr, "SELECT subject FROM gw_forums_posts WHERE messageid = %d and forumid = %d AND obj_did = %d", referenceid, forumid, sid->dat->did)<0) return;
	if (sql_numtuples(&sqr)>0) {
...
/*	char body[512];
	...
	if (sql_queryf(&sqr1, "SELECT distinct forumgroupid FROM gw_forums WHERE obj_did = %d ORDER BY forumgroupid ASC", sid->dat->did)<0) return;
	if (sql_numtuples(&sqr1)>1) {
		...
		if (selected!=2) {
			...
			for (i=0;i<sql_numtuples(&sqr1);i++) {
				...
				if (sql_queryf(&sqr2, "SELECT count(forumid) FROM gw_forums WHERE forumgroupid=%d AND obj_did = %d", atoi(sql_getvalue(&sqr1, i, 0)), sid->dat->did)<0) {
					sql_freeresult(&sqr1);
					return;
				}
	...
	if (longlist==0) {
		if (sql_queryf(&sqr1, "SELECT forumid, postername, posttime, subject FROM gw_forums WHERE forumgroupid = %d AND obj_did = %d ORDER BY forumid ASC", forumgroupid, sid->dat->did)<0) return;
		if (sql_numtuples(&sqr1)>0) {
			...
			for (i=0;i<sql_numtuples(&sqr1);i++) {
				...
				if (sql_queryf(&sqr2, "SELECT count(messageid) FROM gw_forums_posts where forumid = %d AND obj_did = %d", atoi(sql_getvalue(&sqr1, i, 0)), sid->dat->did)<0) return;
		...
		if (sql_queryf(&sqr1, "SELECT forumid, postername, posttime, subject, message FROM gw_forums WHERE forumgroupid = %d AND obj_did = %d ORDER BY forumid DESC", forumgroupid, sid->dat->did)<0) return;
		if (sql_numtuples(&sqr1)>0) {
			for (i=0;i<sql_numtuples(&sqr1);i++) {
				prints(sid, "<TABLE BORDER=1 CELLPADDING=2 CELLSPACING=0 WIDTH=90%% STYLE='border-style:solid'>\r\n");
				prints(sid, "<TR><TH ALIGN=LEFT NOWRAP STYLE='border-style:solid'>");
...
/*	BTREE *xtree;
	...
	forumid=atoi(ptemp);
	forumview(sid, forumid);
	if (sql_queryf(&sqr, "SELECT messageid, referenceid, postername, posttime, subject FROM gw_forums_posts WHERE forumid = %d AND obj_did = %d ORDER BY messageid ASC", forumid, sid->dat->did)<0) return;
	...
	for (i=0;i<msgcount;i++) {
		...
		for (indent=0;indent<xtree[i].depth;indent++) {
			...
			for (j=i-1;j>-1;j--) {
				if (indent==xtree[i].depth-1) {
					if (xtree[j].numchildren>1) {
						prints(sid, "<IMG BORDER=0 SRC=/nullgs/images/tree-t.png HEIGHT=21 WIDTH=9>");
						xtree[j].numchildren--;
						x=1;
						break;
					} else if (xtree[j].numchildren==1) {
						prints(sid, "<IMG BORDER=0 SRC=/nullgs/images/tree-l.png HEIGHT=21 WIDTH=9>");
						xtree[j].numchildren--;
						x=1;
						break;
					}
					...
					if (indent==xtree[j].depth) {
						if (xtree[j].numchildren>0) {
							prints(sid, "<IMG BORDER=0 SRC=/nullgs/images/tree-i.png HEIGHT=21 WIDTH=9>");
							x=1;
							break;
						}
					}
			...
			if (!x) {
				prints(sid, "<IMG BORDER=0 SRC=/nullgs/images/blank.png HEIGHT=21 WIDTH=9>");
			}
		}
		if (xtree[i].numchildren>0) {
			prints(sid, "<IMG BORDER=0 SRC=/nullgs/images/tree-m.png HEIGHT=21 WIDTH=9>");
		} else if (xtree[i].depth<1) {
			prints(sid, "<IMG BORDER=0 SRC=/nullgs/images/tree-o.png HEIGHT=21 WIDTH=9>");
		} else {
			prints(sid, "<IMG BORDER=0 SRC=/nullgs/images/tree-c.png HEIGHT=21 WIDTH=9>");
		}
...
/*
	...
	strncatf(query, sizeof(query)-strlen(query)-1, "'%d','%s','%s',0,0,%d,0,0,'%d','%s','%s','%s','", forumid, posttime, posttime, sid->dat->did, forumgroupid, sid->dat->username, posttime, str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, subject));
	...
	forumview(sid, forumid);
	db_log_activity(sid, "forums", forumid, "insert", "%s - %s added forum %d", sid->dat->in_RemoteAddr, sid->dat->username, forumid);
*/
	return;
}
...
/*
	...
	memset(query, 0, sizeof(query));
	if (sql_queryf(&sqr, "SELECT max(messageid) FROM gw_forums_posts where forumid = %d", forumid)<0) return;
	messageid=atoi(sql_getvalue(&sqr, 0, 0))+1;
	if (messageid<1) messageid=1;
	sql_freeresult(&sqr);
	t=time(NULL);
	strftime(posttime, sizeof(posttime), "%Y-%m-%d %H:%M:%S", gmtime(&t));
	snprintf(query, sizeof(query)-1, "INSERT INTO gw_forums_posts (messageid,obj_ctime,obj_mtime,obj_uid,obj_gid,obj_did,obj_gperm,obj_operm,forumid,referenceid,postername,posttime,subject,message) values (%d, '%s', '%s', 0, 0, %d, 0, 0, %d, %d, '%s', '%s', '%s', '", messageid, posttime, posttime, sid->dat->did, forumid, referenceid, sid->dat->username, posttime, str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, subject));
	strncat(query, message, sizeof(query)-strlen(query)-3);
	strncat(query, "')", sizeof(query)-strlen(query)-1);
	if (sql_update(query)<0) return;
	prints(sid, "<CENTER>Message %d added successfully</CENTER>\r\n", messageid);
	fmessageview(sid, forumid, messageid);
	db_log_activity(sid, "forumposts", forumid, "insert", "%s - %s added message %d on forum %d", sid->dat->in_RemoteAddr, sid->dat->username, messageid, forumid);
*/
	return;
}
...
DllExport int mod_main(CONN *sid)
{
	...
	prints(sid, "<BR />\r\n");
	if (strncmp(RequestURI, "/forums/msgread",      15)==0) {
		fmessagepost(sid);
	} else if (strncmp(RequestURI, "/forums/post",    12)==0) {
		forumpost(sid);
	} else if (strncmp(RequestURI, "/forums/msgpost", 15)==0) {
		fmessagepost(sid);
	} else if (strncmp(RequestURI, "/forums/list",    12)==0) {
		forumlist(sid, 1);
	} else if (strncmp(RequestURI, "/forums/msglist", 15)==0) {
		fmessagelist(sid);
	} else if (strncmp(RequestURI, "/forums/save",    12)==0) {
		forumsave(sid);
	} else if (strncmp(RequestURI, "/forums/msgsave", 15)==0) {
		fmessagesave(sid);
	} else if (strncmp(RequestURI, "/forums/groupedit", 17)==0) {
		forumgroupedit(sid);
	} else if (strncmp(RequestURI, "/forums/grouplist", 17)==0) {
		forumgrouplist(sid);
	} else if (strncmp(RequestURI, "/forums/groupsave", 17)==0) {
		forumgroupsave(sid);
	}
	htpage_footer(sid);
	return 0;
...
DllExport int mod_init(_PROC *_proc)
{
	...
	tobj=nes_settable(proc->N, nes_settable(proc->N, &proc->N->g, "GWMODULES"), "forums");
	nes_exec(proc->N, "GWMODULES.forums={submenu=1,name='FORUMS',pic='default.png',uri='/forums/list',perm='forums',fn_name='mod_main',fn_uri='/forums/'};");
