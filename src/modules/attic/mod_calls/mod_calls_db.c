...
int dbread_call(CONN *sid, short int perm, int index, obj_t **qobj)
{
	short int approved;
	int auth=auth_priv(sid, "calls");
	obj_t *cobj, *tobj;
	...
	if (index==0) {
		...
		cobj=nes_evalf(sid->N, "{_tuples=1,_rows={[0]={id=0,pid=%d,did=%d,_data={assignedby=%d,assignedto=%d,cn='New Call'}}}}", sid->dat->uid, sid->dat->uid, sid->dat->did, sid->dat->did);
		nes_linkval(sid->N, tobj, cobj);
		return EC_NOERROR;
	}
	if ((tobj=ldir_getentry(sid->N, "call", NULL, index, sid->dat->did))==NULL) return EC_NORECORD;
...
/*
	...
	if (index==0) {
		call->obj_uid=sid->dat->uid;
		call->obj_gid=sid->dat->gid;
		call->obj_did=sid->dat->did;
		call->obj_gperm=1;
		call->obj_operm=1;
		call->assignedby=sid->dat->uid;
		call->assignedto=sid->dat->uid;
	...
	if (authlevel&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_calls where callid = %d AND obj_did = %d", index, sid->dat->did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_calls where callid = %d and (obj_uid = %d or assignedby = %d or assignedto = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d) AND obj_did = %d", index, sid->dat->uid, sid->dat->uid, sid->dat->uid, sid->dat->gid, perm, perm, sid->dat->did)<0) return -1;
	}
...
int dbwrite_call(CONN *sid, int index, obj_t **qobj)
{
	int auth=auth_priv(sid, "calls");

	if (auth<2) return EC_NOPERM;
	if (!(auth&A_INSERT)&&(index==0)) return EC_NOPERM;
	return ldir_saveentry(sid, index, "call", qobj);
/*
	char curdate[32];
	char query[12288];
	int authlevel;
