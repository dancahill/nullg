...
{
/*
	char *option[]={ "All", "Personal", "Calls", "Contacts", "Events", "Invoices", "Notes", "Projects", "Tasks", "Users" };
	char *ptemp;
	int i;
	int j;
	SQLRES sqr;
...
/*
	...
	if (auth_priv(sid, "admin")&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT noteid, notetitle, notetext FROM gw_notes WHERE tablename = '%s' AND tableindex = %d AND obj_did = %d ORDER BY noteid ASC", table, index, sid->dat->did)<0) return;
	} else {
...
/*
	...
	if ((note.obj_uid==sid->dat->uid)||(auth_priv(sid, "admin")&A_ADMIN)) {
...
/*
	...
	} else if ((note.obj_uid==sid->dat->uid)||((note.obj_gid==sid->dat->gid)&&(note.obj_gperm>1))||(note.obj_operm>1)) {
		prints(sid, " [<A HREF=%s/notes/edit?noteid=%d>edit</A>]", sid->dat->in_ScriptName, noteid);
	}
	...
	if (strcmp(note.tablename, "calls")==0) {
		...
		sql_freeresult(&sqr);
		prints(sid, "&nbsp;</TD></TR>\r\n");
	} else if (strcmp(note.tablename, "invoices")==0) {
		...
		if (sql_queryf(&sqr, "SELECT invoiceid, contactid FROM gw_finance_invoices WHERE invoiceid = %d", note.tableindex)<0) return;
		if (sql_numtuples(&sqr)>0) {
			prints(sid, "<A HREF=%s/invoices/view?invoiceid=%d>", sid->dat->in_ScriptName, note.tableindex);
...
/*
	...
	switch (table) {
		...
		case 5: { snprintf(tablename, sizeof(tablename)-1, "invoices"); break; }
		case 6: { snprintf(tablename, sizeof(tablename)-1, "notes"); break; }
	...
	if (auth_priv(sid, "admin")&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT noteid, notetitle, obj_mtime, tablename, tableindex FROM gw_notes WHERE obj_uid = %d AND tablename like '%s' AND obj_did = %d ORDER BY noteid DESC", userid, tablename, sid->dat->did)<0) return;
	} else {
		if (sql_queryf(&sqr, "SELECT noteid, notetitle, obj_mtime, tablename, tableindex FROM gw_notes WHERE obj_uid = %d AND tablename like '%s' AND (obj_uid = %d or (obj_gid = %d and obj_gperm>0) or obj_operm>0) AND obj_did = %d ORDER BY noteid DESC", userid, tablename, sid->dat->uid, sid->dat->gid, sid->dat->did)<0) return;
	}
	if (sql_numtuples(&sqr)>0) {
		prints(sid, "<BR />\r\n");
		...
		for (i=offset;(i<sql_numtuples(&sqr))&&(i<offset+sid->dat->maxlist);i++) {
			...
			} else if (strcasecmp(sql_getvalue(&sqr, i, 3), "invoices")==0) {
				prints(sid, "<TD NOWRAP STYLE='border-style:solid'><A HREF=%s/invoices/view?invoiceid=%d>Order</A></TD>", sid->dat->in_ScriptName, atoi(sql_getvalue(&sqr, i, 4)));
...
/*
	...
	if ((auth_priv(sid, "admin")&A_ADMIN)||(note.obj_uid==sid->dat->uid)) {
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		db_log_activity(sid, "notes", note.noteid, "delete", "%s - %s deleted note %d", sid->dat->in_RemoteAddr, sid->dat->username, note.noteid);
		if (strcmp(note.tablename, "calls")==0) {
			...
			prints(sid, "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL=%s/invoices/view?invoiceid=%d\">\r\n", sid->dat->in_ScriptName, note.tableindex);
		} else if (strcmp(note.tablename, "projects")==0) {
		...
		db_log_activity(sid, "notes", note.noteid, "insert", "%s - %s added note %d", sid->dat->in_RemoteAddr, sid->dat->username, note.noteid);
		...
		db_log_activity(sid, "notes", note.noteid, "modify", "%s - %s modified note %d", sid->dat->in_RemoteAddr, sid->dat->username, note.noteid);
		prints(sid, "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL=%s/notes/view?noteid=%d\">\r\n", sid->dat->in_ScriptName, note.noteid);
	}
*/
	return;
...
DllExport int mod_main(CONN *sid)
{
	...
	if (strncmp(RequestURI, "/notes/edit", 11)==0) {
		notesedit(sid);
	} else if (strncmp(RequestURI, "/notes/view", 11)==0) {
		notesview(sid);
	} else if (strncmp(RequestURI, "/notes/list", 11)==0) {
		noteslist(sid);
	} else if (strncmp(RequestURI, "/notes/save", 11)==0) {
		notessave(sid);
	}
	htpage_footer(sid);
	return 0;
...
DllExport int mod_init(_PROC *_proc)
{
	...
	tobj=nes_settable(proc->N, nes_settable(proc->N, &proc->N->g, "GWMODULES"), "notes");
	nes_exec(proc->N, "GWMODULES.notes={submenu=3,name='NOTEBOOK',pic='default.png',uri='/notes/list',perm='',fn_name='mod_main',fn_uri='/notes/'};");
