...
//	SQLRES sqr2;
	...
	prints(sid, "<FORM METHOD=GET ACTION=%s/calendar/mlist>", ScriptName);
	...
	prints(sid, "<CENTER>\r\n<FONT COLOR=#000000><B><A HREF=%s/calendar/mlist?&status=%d&month=%d&year=%d", ScriptName, status, tempmonth, tempyear);
	...
	prints(sid, "<A HREF=%s/calendar/mlist?&status=%d&month=%d&year=%d", ScriptName, status, tempmonth, tempyear);
	if (userid>0) prints(sid, "&userid=%d", userid);
	if (groupid>0) prints(sid, "&groupid=%d", groupid);
	prints(sid, ">&gt;&gt;</A>");
...
//	if (sql_queryf(&sqr2, "SELECT userid, groupid, username FROM gw_users WHERE groupid = %d ORDER BY userid ASC", groupid)<0) return;
	for (i=printdate;i<dim[today.month-1]+1;i+=7) {
		...
		for (j=0;j<7;j++) {
			...
			prints(sid, " VALIGN=TOP style='cursor:hand; border-style:solid' onClick=\"window.location.href='%s/calendar/list?day=%d&status=%d", ScriptName, unixdate, status);
			if (userid>0) prints(sid, "&userid=%d", userid);
			if (groupid>0) prints(sid, "&groupid=%d", groupid);
			prints(sid, "'\">");
			t=unixdate*86400;
			...
			strftime(posttime1, sizeof(posttime1), "%Y-%m-%d %H:%M:%S", gmtime(&t));
			t-=time_tzoffset(sid, t);
			prints(sid, "<NOBR><FONT SIZE=2><A HREF=%s/calendar/list?day=%d&status=%d", ScriptName, unixdate, status);
			if (userid>0) prints(sid, "&userid=%d", userid);
			if (groupid>0) prints(sid, "&groupid=%d", groupid);
...
//				t2=time_sql2unix(sql_getvalue(&sqr1, index, 1))+time_tzoffset(sid, time_sql2unix(sql_getvalue(&sqr1, index, 1)));
				...
				prints(sid, "<A HREF=%s/calendar/edit?eventid=%d TITLE=\"", ScriptName, atoi(sql_getvalue(&sqr1, index, 0)));
				if (groupid>0) {
