...
int dbread_task(CONN *sid, short int perm, int index, obj_t **qobj)
{
/*
	int authlevel;
	SQLRES sqr;
	...
	if (index==0) {
		task->obj_uid=sid->dat->uid;
		task->obj_gid=sid->dat->gid;
		task->obj_did=sid->dat->did;
		task->obj_gperm=1;
		task->obj_operm=1;
		task->assignedby=sid->dat->uid;
		task->assignedto=sid->dat->uid;
	...
	if (authlevel&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_tasks where taskid = %d and obj_did = %d", index, sid->dat->did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_tasks where taskid = %d and (obj_uid = %d or assignedby = %d or assignedto = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d) and obj_did = %d", index, sid->dat->uid, sid->dat->uid, sid->dat->uid, sid->dat->gid, perm, perm, sid->dat->did)<0) return -1;
	}
...
int dbwrite_task(CONN *sid, int index, obj_t **qobj)
{
/*
	char curdate[32];
	char query[12288];
