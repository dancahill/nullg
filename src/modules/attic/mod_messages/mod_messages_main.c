...
/*
	...
	if (sql_queryf(&sqr, "SELECT groupid, groupname FROM gw_groups WHERE obj_did = %d order by groupname ASC", sid->dat->did)<0) return;
...
/*
	...
	if (sql_queryf(&sqr, "SELECT messageid FROM gw_messages WHERE obj_uid = %d AND obj_did = %d AND messageid = %d", sid->dat->uid, sid->dat->did, messageid)<0) return;
	...
	if (status=='0') {
		sql_updatef("DELETE FROM gw_messages WHERE obj_uid = %d AND obj_did = %d and messageid = %d", sid->dat->uid, sid->dat->did, messageid);
	} else {
		sql_updatef("UPDATE gw_messages SET obj_mtime = '%s', status = %c WHERE obj_uid = %d AND messageid = %d", timebuffer, status, sid->dat->uid, messageid);
	}
*/	return;
}
...
/*
	...
	if (sql_queryf(&sqr, "SELECT userid, username, givenname, surname FROM gw_users WHERE userid != %d AND groupid = %d AND domainid = %d ORDER BY username ASC", sid->dat->uid, groupid, sid->dat->did)<0) return;
	prints(sid, "<SCRIPT LANGUAGE=JavaScript>\r\n<!--\r\n");
...
/*
	...
	if (strcmp(sid->dat->in_RequestMethod,"POST")==0) {
		...
		if (strlen(message)>0) {
			...
			if (sql_updatef("INSERT INTO gw_messages (obj_ctime, obj_mtime, obj_uid, obj_did, sender, rcpt, status, message) values ('%s', '%s', %d, %d, %d, %d, 3, '%s')", curdate, curdate, userid, sid->dat->did, sid->dat->uid, userid, str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, message))<0) return;
			if (sql_updatef("INSERT INTO gw_messages (obj_ctime, obj_mtime, obj_uid, obj_did, sender, rcpt, status, message) values ('%s', '%s', %d, %d, %d, %d, 3, '%s')", curdate, curdate, sid->dat->uid, sid->dat->did, sid->dat->uid, userid, str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, message))<0) return;
		}
	}
...
/*
	...
	for (retry=0;retry<10;retry++) {
		if (sql_queryf(&sqr, "SELECT messageid, sender, rcpt, message FROM gw_messages WHERE obj_uid = %d AND obj_did = %d AND status > 2 AND (rcpt = %d OR sender = %d) ORDER BY obj_ctime, messageid ASC", sid->dat->uid, sid->dat->did, userid, userid)<0) return;
		if (sql_numtuples(&sqr)>0) {
			for (i=0;i<sql_numtuples(&sqr);i++) {
				if (atoi(sql_getvalue(&sqr, i, 1))==sid->dat->uid) {
					if (tcp_fprintf(&sid->socket, "WriteTop(\"<B><FONT COLOR=black>%s: </FONT></B>%s<BR />\");\r\n", sid->dat->username, str2html(sid, sql_getvalue(&sqr, i, 3)))<0) break;
				} else {
...
/*
	...
	memset(username, 0, sizeof(username));
	if ((ptemp=getgetenv(sid, "userid"))!=NULL) userid=atoi(ptemp);
	if (sql_queryf(&sqr, "SELECT userid, username FROM gw_users WHERE userid = %d AND domainid = %d", userid, sid->dat->did)<0) return;
	if (sql_numtuples(&sqr)!=1) {
		sql_freeresult(&sqr);
		return;
	}
	...
	if (sql_queryf(&sqr, "SELECT messageid, obj_ctime, sender, rcpt, status, message FROM gw_messages WHERE obj_uid = %d AND obj_did = %d AND status > 0 AND (rcpt = %d OR sender = %d) ORDER BY messageid ASC", sid->dat->uid, sid->dat->did, userid, userid)<0) return;
	if (sql_numtuples(&sqr)<1) {
	...
	for (i=0;i<sql_numtuples(&sqr);i++) {
		...
		prints(sid, "<TD VALIGN=TOP WIDTH=100%%><B><FONT COLOR=%s>", atoi(sql_getvalue(&sqr, i, 2))==sid->dat->uid?"black":"blue");
		if (atoi(sql_getvalue(&sqr, i, 2))==sid->dat->uid) {
			prints(sid, "%s", str2html(sid, sid->dat->username));
		} else {
...
DllExport int mod_main(CONN *sid)
{
	...
	if (strncmp(RequestURI, "/messages/userlist", 18)==0) {
		messages_userlist(sid);
	} else if (strncmp(RequestURI, "/messages/frame", 15)==0) {
		messages_frameset(sid);
	} else if (strncmp(RequestURI, "/messages/send", 14)==0) {
		messages_send(sid);
	} else if (strncmp(RequestURI, "/messages/reload", 16)==0) {
		messages_reload(sid);
	} else if (strncmp(RequestURI, "/messages/history", 17)==0) {
		messages_history(sid);
	} else if (strncmp(RequestURI, "/messages/white", 15)==0) {
		messages_white(sid);
	} else if (strncmp(RequestURI, "/messages/delete", 16)==0) {
...
DllExport int mod_init(_PROC *_proc)
{
	...
	tobj=nes_settable(proc->N, nes_settable(proc->N, &proc->N->g, "GWMODULES"), "messages");
	nes_exec(proc->N, "GWMODULES.messages={submenu=1,name='MESSENGER',pic='messenger.png',uri='javascript:ListUsers();',perm='messages',fn_name='mod_main',fn_uri='/messages/'};");
