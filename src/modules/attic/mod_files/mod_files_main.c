...
/*
	...
	prints(sid, "</SELECT></TD></TR>\r\n");
	if ((file.obj_uid==sid->dat->uid)||(auth_priv(sid, "files")&A_ADMIN)) editperms=1;
...
/*
	...
	if ((auth_priv(sid, "files")&A_ADMIN)||(file.obj_uid==sid->dat->uid)) {
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		snprintf(filename, sizeof(filename)-1, "%s/%04d/files%s%s", config->dir_var_domains, sid->dat->did, file.filepath+6, file.filename);
		fixslashes(filename);
		...
		if (sql_updatef("DELETE FROM gw_files WHERE fileid = %d AND obj_did = %d", file.fileid, sid->dat->did)<0) return;
		prints(sid, "<CENTER>file %d deleted successfully</CENTER><BR />\r\n", file.fileid);
		db_log_activity(sid, "files", file.fileid, "delete", "%s - %s deleted file %d %s", sid->dat->in_RemoteAddr, sid->dat->username, file.fileid, file.filename);
	} else {
		if (!(auth_priv(sid, "files")&A_MODIFY)) {
			prints(sid, "<BR /><CENTER>%s</CENTER><BR />\r\n", lang.err_noaccess);
			return;
		...
		strncatf(query, sizeof(query)-strlen(query)-1, " WHERE fileid = %d AND obj_did = %d", file.fileid, sid->dat->did);
		if (sql_update(query)<0) return;
		prints(sid, "<CENTER>file %d modified successfully</CENTER><BR />\r\n", file.fileid);
		db_log_activity(sid, "files", file.fileid, "modify", "%s - %s modified file %d %s", sid->dat->in_RemoteAddr, sid->dat->username, file.fileid, file.filename);
	}
...
DllExport int mod_main(CONN *sid)
{
	...
	if ((strncmp(RequestURI, "/fileul", 7)!=0)&&(strncmp(RequestURI, "/filerecv", 9)!=0)) {
		htpage_topmenu(sid, MENU_FILES);
		prints(sid, "<BR />\r\n");
	}
	if (strncmp(RequestURI, "/fileul", 7)==0) {
		fileul(sid);
	} else if (strncmp(RequestURI, "/filerecv", 9)==0) {
		filerecv(sid);
	} else if (strncmp(RequestURI, "/filescan", 9)==0) {
		filescan(sid);
	} else if (strncmp(RequestURI, "/fileinfoedit", 13)==0) {
		fileinfoedit(sid);
	} else if (strncmp(RequestURI, "/fileinfosave", 13)==0) {
		fileinfosave(sid);
	} else if (strncmp(RequestURI, "/filemkdir", 10)==0) {
		filemkdir(sid);
	} else if (strncmp(RequestURI, "/filedirsave", 12)==0) {
		filedirsave(sid);
	} else if (strncmp(RequestURI, "/fileedit", 9)==0) {
		fileedit(sid);
	} else if (strncmp(RequestURI, "/filesave", 9)==0) {
		filesave(sid);
	}
	htpage_footer(sid);
	return 0;
...
DllExport int mod_init(_PROC *_proc)
{
	...
	tobj=nes_settable(proc->N, nes_settable(proc->N, &proc->N->g, "GWMODULES"), "files");
	nes_exec(proc->N, "GWMODULES.files={submenu=3,name='FILES',pic='default.png',uri='/files/~/',perm='files',fn_name='mod_main',fn_uri='/file'};");
