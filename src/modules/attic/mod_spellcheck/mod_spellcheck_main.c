...
int spellcheck_shellexec(CONN *sid)
{
	...
	if (!CreateProcess(NULL, Command, NULL, NULL, TRUE, CREATE_NEW_CONSOLE|CREATE_NO_WINDOW, NULL, Path, &si, &pi)) {
		...
		log_error(proc->N, "mod_spellcheck", __FILE__, __LINE__, 1, "program failed. [%s]", Command);
	...
	if ((pipe(pset1)==-1) || (pipe(pset2)==-1)) {
		close(pset1[0]);
		close(pset1[1]);
		log_error(proc->N, "mod_spellcheck", __FILE__, __LINE__, 1, "pipe() error");
		send_error(sid, 500, "Internal Server Error", "Unable to create pipe.");
		return 0;
	}
	...
	pid=fork();
	if (pid<0) {
		log_error(proc->N, "mod_spellcheck", __FILE__, __LINE__, 1, "fork() error");
		return 0;
	} else if (pid==0) {
		close(local.in);
		close(local.out);
		...
		log_error(proc->N, "mod_spellcheck", __FILE__, __LINE__, 1, "execve() error [%s][%s]", args[0], args[1]);
		exit(0);
	} else {
		close(remote.in);
		close(remote.out);
...
int spellcheck_check(CONN *sid)
{
	...
	prints(sid, "<script language=\"javascript\" src=\"/scripts/wordWindow.js\"></script>\r\n");
...
int spellcheck_controls(CONN *sid)
{
	prints(sid, "<html>\r\n");
	prints(sid, "<head>\r\n");
//	prints(sid, "<LINK REL=\"stylesheet\" TYPE=\"text/css\" HREF=\"/nullgs/themes/%s/style.css\">\r\n", sid->dat->user_theme);
	prints(sid, "<script src=\"/scripts/controlWindow.js\"></script>\r\n");
	prints(sid, "<script>\r\n");
...
int spellcheck_test(CONN *sid)
{
	...
	prints(sid, "<script src=\"/nullgs/js/spellcheck/spellChecker.js\"></script>\r\n");
	prints(sid, "<script>\r\n");
...
DllExport int mod_main(CONN *sid)
{
	...
	if (strncmp(RequestURI, "/spellcheck/blank", 17)==0) {
		return 0;
	} else if (strncmp(RequestURI, "/spellcheck/test", 16)==0) {
		spellcheck_test(sid);
	} else if (strncmp(RequestURI, "/spellcheck/popup", 17)==0) {
		spellcheck_popup(sid);
	} else if (strncmp(RequestURI, "/spellcheck/controls", 20)==0) {
		spellcheck_controls(sid);
	} else if (strncmp(RequestURI, "/spellcheck/check", 17)==0) {
...
DllExport int mod_init(_PROC *_proc)
{
	...
	tobj=nes_settable(proc->N, nes_settable(proc->N, &proc->N->g, "GWMODULES"), "spellcheck");
	nes_exec(proc->N, "GWMODULES.spellcheck={fn_name='mod_main',fn_uri='/spellcheck/'};");
