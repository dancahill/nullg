...
void xmlrpc_contact_newstruct(CONN *sid)
{
	...
	xmlrpc_addmember(sid, "obj_uid",        "int",      "%d", sid->dat->uid);
	xmlrpc_addmember(sid, "obj_gid",        "int",      "%d", sid->dat->gid);
	xmlrpc_addmember(sid, "obj_did",        "int",      "0");
	xmlrpc_addmember(sid, "obj_gperm",      "int",      "1");
	xmlrpc_addmember(sid, "obj_operm",      "int",      "1");
	xmlrpc_addmember(sid, "folderid",       "int",      "0");
	...
	xmlrpc_addmember(sid, "timezone",       "int",      "%d", sid->dat->timezone);
	...
	xmlrpc_addmember(sid, "website",        "string",   "");
...
void xmlrpc_contact_addstruct(CONN *sid, obj_t **qobj, int tuple)
{
/*	if ((tuple<0)||(tuple>=sql_numtuples(sqr))) return;
	...
	xmlrpc_addmember(sid, "folderid",       "int",      "%d", atoi(sql_getvalue(sqr, tuple, 8)));
	xmlrpc_addmember(sid, "username",       "string",   "%s", sql_getvalue(sqr, tuple, 9));
	xmlrpc_addmember(sid, "password",       "string",   "%s", sql_getvalue(sqr, tuple, 10));
	xmlrpc_addmember(sid, "enabled",        "int",      "%d", atoi(sql_getvalue(sqr, tuple, 11)));
	xmlrpc_addmember(sid, "geozone",        "int",      "%d", atoi(sql_getvalue(sqr, tuple, 12)));
	xmlrpc_addmember(sid, "timezone",       "int",      "%d", atoi(sql_getvalue(sqr, tuple, 13)));
	xmlrpc_addmember(sid, "surname",        "string",   "%s", sql_getvalue(sqr, tuple, 14));
	xmlrpc_addmember(sid, "givenname",      "string",   "%s", sql_getvalue(sqr, tuple, 15));
	xmlrpc_addmember(sid, "salutation",     "string",   "%s", sql_getvalue(sqr, tuple, 16));
	xmlrpc_addmember(sid, "contacttype",    "string",   "%s", sql_getvalue(sqr, tuple, 17));
	xmlrpc_addmember(sid, "referredby",     "string",   "%s", sql_getvalue(sqr, tuple, 18));
	xmlrpc_addmember(sid, "altcontact",     "string",   "%s", sql_getvalue(sqr, tuple, 19));
	xmlrpc_addmember(sid, "prefbilling",    "string",   "%s", sql_getvalue(sqr, tuple, 20));
	xmlrpc_addmember(sid, "website",        "string",   "%s", sql_getvalue(sqr, tuple, 21));
	xmlrpc_addmember(sid, "email",          "string",   "%s", sql_getvalue(sqr, tuple, 22));
	xmlrpc_addmember(sid, "homenumber",     "string",   "%s", sql_getvalue(sqr, tuple, 23));
	xmlrpc_addmember(sid, "worknumber",     "string",   "%s", sql_getvalue(sqr, tuple, 24));
	xmlrpc_addmember(sid, "faxnumber",      "string",   "%s", sql_getvalue(sqr, tuple, 25));
	xmlrpc_addmember(sid, "mobilenumber",   "string",   "%s", sql_getvalue(sqr, tuple, 26));
	xmlrpc_addmember(sid, "jobtitle",       "string",   "%s", sql_getvalue(sqr, tuple, 27));
	xmlrpc_addmember(sid, "organization",   "string",   "%s", sql_getvalue(sqr, tuple, 28));
	xmlrpc_addmember(sid, "homeaddress",    "string",   "%s", sql_getvalue(sqr, tuple, 29));
	xmlrpc_addmember(sid, "homelocality",   "string",   "%s", sql_getvalue(sqr, tuple, 30));
	xmlrpc_addmember(sid, "homeregion",     "string",   "%s", sql_getvalue(sqr, tuple, 31));
	xmlrpc_addmember(sid, "homecountry",    "string",   "%s", sql_getvalue(sqr, tuple, 32));
	xmlrpc_addmember(sid, "homepostalcode", "string",   "%s", sql_getvalue(sqr, tuple, 33));
	xmlrpc_addmember(sid, "workaddress",    "string",   "%s", sql_getvalue(sqr, tuple, 34));
	xmlrpc_addmember(sid, "worklocality",   "string",   "%s", sql_getvalue(sqr, tuple, 35));
	xmlrpc_addmember(sid, "workregion",     "string",   "%s", sql_getvalue(sqr, tuple, 36));
	xmlrpc_addmember(sid, "workcountry",    "string",   "%s", sql_getvalue(sqr, tuple, 37));
	xmlrpc_addmember(sid, "workpostalcode", "string",   "%s", sql_getvalue(sqr, tuple, 38));
	prints(sid, "</struct>\r\n</value>\r\n");*/
	return;
}
...
/*	SQLRES sqr;
	...
	if (contactid==0) {
		...
		rc=sql_queryf(&sqr, "SELECT * from gw_contacts WHERE contactid = %d  AND (obj_uid = %d or (obj_gid = %d and obj_gperm>=1) or obj_operm>=1) ORDER BY contactid ASC", contactid, sid->dat->uid, sid->dat->gid);
	}
	if (rc<0) {
		xmlrpc_fault(sid, -1, lang.err_noaccess);
		return;
...
/*	HTMOD_CONTACTS_READ mod_contacts_read;
	...
	if (contact.obj_uid<1) contact.obj_uid=sid->dat->uid;
	if (contact.obj_gid<1) contact.obj_gid=sid->dat->gid;
	if ((auth_priv(sid, "contacts")&A_ADMIN)||(contact.obj_uid==sid->dat->uid)) {
	...
	if ((ptemp=getxmlstruct(sid, "FOLDERID",       "INT"))!=NULL) contact.folderid=atoi(ptemp);
	...
	if ((ptemp=getxmlstruct(sid, "WEBSITE",        "STRING"))!=NULL) snprintf(contact.website, sizeof(contact.website)-1, "%s", ptemp);
	...
	if (contact.contactid==0) {
		...
		db_log_activity(sid, "contacts", contact.contactid, "insert", "%s - %s added contact %d (xml-rpc)", sid->dat->in_RemoteAddr, sid->dat->username, contact.contactid);
	} else {
		if (!(auth_priv(sid, "contacts")&A_MODIFY)) {
			xmlrpc_fault(sid, -1, lang.err_noaccess);
			return;
		...
		db_log_activity(sid, "contacts", contact.contactid, "modify", "%s - %s modified contact %d (xml-rpc)", sid->dat->in_RemoteAddr, sid->dat->username, contact.contactid);
	}
