...
/*
	...
	if (auth_priv(sid, "domainadmin")&A_ADMIN) {
		...
		if (sql_queryf(&sqr1, "SELECT zoneid, obj_did, zonename FROM gw_zones WHERE obj_did = %d ORDER BY obj_did, zonename ASC", sid->dat->did)<0) return;
	}
	if (auth_priv(sid, "domainadmin")&A_ADMIN) {
		rc=sql_queryf(&sqr2, "SELECT domainid, domainname FROM gw_domains ORDER BY domainname ASC");
	} else {
		rc=sql_queryf(&sqr2, "SELECT domainid, domainname FROM gw_domains WHERE domainid = %d", sid->dat->did);
	}
	if (rc<0) {
		sql_freeresult(&sqr1);
		return;
...
/*
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		db_log_activity(sid, "zones", zone.zoneid, "delete", "%s - %s deleted zone %d", sid->dat->in_RemoteAddr, sid->dat->username, zone.zoneid);
		...
		db_log_activity(sid, "zones", zone.zoneid, "insert", "%s - %s added zone %d", sid->dat->in_RemoteAddr, sid->dat->username, zone.zoneid);
	} else {
		if (!(auth_priv(sid, "admin")&A_ADMIN)) {
			prints(sid, "<BR /><CENTER>%s</CENTER><BR />\r\n", lang.err_noaccess);
			return;
		...
		db_log_activity(sid, "zones", zone.zoneid, "modify", "%s - %s modified zone %d", sid->dat->in_RemoteAddr, sid->dat->username, zone.zoneid);
	}
