...
{
	int authlevel=auth_priv(sid, "admin");
	obj_t *tobj;
	...
	if (index==0) {
		...
		nes_linkval(sid->N, tobj, NULL);
		tobj->val->type=NT_TABLE;
		nes_setnum(sid->N, tobj, "_tuples", 1);
		tobj=nes_settable(sid->N, tobj, "_rows");
		tobj=nes_settable(sid->N, tobj, "0");
		nes_setnum(sid->N, tobj, "id", 0);
		nes_setnum(sid->N, tobj, "pid", 0);
		nes_setnum(sid->N, tobj, "did", 0);
		tobj=nes_settable(sid->N, tobj, "_data");
		nes_setstr(sid->N, tobj, "cn", "New Domain", strlen("New Domain"));
		return 0;
	}
	if ((*qobj=ldir_getentry(sid->N, "organization", NULL, index, index))==NULL) return -1;
...
int dbread_group(CONN *sid, short int perm, int index, obj_t **qobj)
{
	...
	if (!(authlevel&A_INSERT)&&(index==0)) return -1;
	if (index==0) {
		group->obj_uid=sid->dat->uid;
		group->obj_gid=sid->dat->gid;
		group->obj_did=sid->dat->did;
		group->obj_gperm=1;
		group->obj_operm=1;
	...
	if (auth_priv(sid, "domainadmin")&A_ADMIN) {
		...
		if (sql_queryf(&sqr, "SELECT * FROM gw_groups where groupid = %d AND obj_did = %d", index, sid->dat->did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_groups where groupid = %d AND obj_did = %d and (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d)", index, sid->dat->did, sid->dat->uid, sid->dat->gid, perm, perm)<0) return -1;
	}
...
int dbread_groupmember(CONN *sid, short int perm, int index, obj_t **qobj)
{
	...
	if (!(authlevel&A_INSERT)&&(index==0)) return -1;
	if (index==0) {
		groupmember->obj_uid=sid->dat->uid;
		groupmember->obj_gid=sid->dat->gid;
		groupmember->obj_did=sid->dat->did;
		groupmember->obj_gperm=1;
		groupmember->obj_operm=1;
		if (auth_priv(sid, "domainadmin")&A_ADMIN) {
			if (strcmp(sid->dat->in_RequestMethod,"POST")==0) {
				if ((ptemp=getpostenv(sid, "GROUPID"))!=NULL) {
			...
			if (strcmp(sid->dat->in_RequestMethod,"POST")==0) {
				if ((ptemp=getpostenv(sid, "USERID"))!=NULL) {
					groupmember->userid=atoi(ptemp);
				}
			} else if ((ptemp=getgetenv(sid, "USERID"))!=NULL) {
				groupmember->userid=atoi(ptemp);
			}
			if (groupmember->groupid>0) {
				if (sql_queryf(&sqr, "SELECT obj_did FROM gw_groups where groupid = %d", groupmember->groupid)<0) return -1;
				if (sql_numtuples(&sqr)!=1) {
					sql_freeresult(&sqr);
					return -2;
				}
				groupmember->obj_did = atoi(sql_getvalue(&sqr, 0, 0));
			} else  if (groupmember->userid>0) {
				if (sql_queryf(&sqr, "SELECT obj_did FROM gw_users where userid = %d", groupmember->userid)<0) return -1;
				if (sql_numtuples(&sqr)!=1) {
					sql_freeresult(&sqr);
					return -2;
				}
	...
	if (auth_priv(sid, "domainadmin")&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_groups_members where groupmemberid = %d", index)<0) return -1;
	} else if (authlevel&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_groups_members where groupmemberid = %d AND obj_did = %d", index, sid->dat->did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_groups_members where groupmemberid = %d AND obj_did = %d and (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d)", index, sid->dat->did, sid->dat->uid, sid->dat->gid, perm, perm)<0) return -1;
	}
	if (sql_numtuples(&sqr)!=1) {
		sql_freeresult(&sqr);
		return -2;
...
int dbread_user(CONN *sid, short int perm, int index, obj_t **qobj)
{
	...
	if (index==0) {
		user->obj_uid=0;
		user->obj_gid=0;
		user->obj_did=sid->dat->did;
		user->obj_gperm=0;
		user->obj_operm=0;
		user->groupid=2;
		user->domainid=sid->dat->did;
		user->enabled=1;
		...
		user->authemail=A_READ+A_MODIFY+A_INSERT+A_DELETE;
		user->authfiles=A_READ+A_MODIFY+A_INSERT+A_DELETE;
		user->authfinance=A_READ+A_MODIFY+A_INSERT+A_DELETE;
		...
		user->authprojects=A_READ+A_MODIFY+A_INSERT+A_DELETE;
		user->authquery=A_READ;
		user->prefdaystart=0;
		user->prefdaylength=24;
		user->prefmailcurrent=0;
		user->prefmaildefault=1;
		user->prefmaxlist=15;
	...
	if (auth_priv(sid, "domainadmin")&A_ADMIN) {
		...
		if (sql_queryf(&sqr, "SELECT * FROM gw_users where userid = %d AND domainid = %d", index, sid->dat->did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_users where userid = %d AND domainid = %d AND (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d)", index, sid->dat->did, sid->dat->uid, sid->dat->gid, perm, perm)<0) return -1;
	}
	if (sql_numtuples(&sqr)!=1) {
		sql_freeresult(&sqr);
		return -2;
	...
	user->authemail		= atoi(sql_getvalue(&sqr, 0, 19));
	user->authfiles		= atoi(sql_getvalue(&sqr, 0, 20));
	user->authfinance	= atoi(sql_getvalue(&sqr, 0, 21));
	user->authforums	= atoi(sql_getvalue(&sqr, 0, 22));
	user->authmessages	= atoi(sql_getvalue(&sqr, 0, 23));
	user->authprofile	= atoi(sql_getvalue(&sqr, 0, 24));
	user->authprojects	= atoi(sql_getvalue(&sqr, 0, 25));
	user->authquery		= atoi(sql_getvalue(&sqr, 0, 26));
	...
	if (user->authemail&A_ADMIN) user->authemail=A_READ+A_MODIFY+A_INSERT+A_DELETE+A_ADMIN;
	if (user->authfiles&A_ADMIN) user->authfiles=A_READ+A_MODIFY+A_INSERT+A_DELETE+A_ADMIN;
	if (user->authfinance&A_ADMIN) user->authfinance=A_READ+A_MODIFY+A_INSERT+A_DELETE+A_ADMIN;
	...
	if (user->authprojects&A_ADMIN) user->authprojects=A_READ+A_MODIFY+A_INSERT+A_DELETE+A_ADMIN;
...
int dbread_zone(CONN *sid, short int perm, int index, obj_t **qobj)
{
	...
	if (!(authlevel&A_INSERT)&&(index==0)) return -1;
	if (index==0) {
		zone->obj_uid=sid->dat->uid;
		zone->obj_gid=sid->dat->gid;
		zone->obj_did=sid->dat->did;
		zone->obj_gperm=1;
		zone->obj_operm=1;
	...
	if (auth_priv(sid, "domainadmin")&A_ADMIN) {
		...
		if (sql_queryf(&sqr, "SELECT * FROM gw_zones where zoneid = %d AND obj_did = %d", index, sid->dat->did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_zones where zoneid = %d AND obj_did = %d AND (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d)", index, sid->dat->did, sid->dat->uid, sid->dat->gid, perm, perm)<0) return -1;
	}
	if (sql_numtuples(&sqr)!=1) {
		sql_freeresult(&sqr);
		return -2;
