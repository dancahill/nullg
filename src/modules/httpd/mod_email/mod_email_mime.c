...
int webmailheader(CONN *sid, obj_t **hobj, FILE **fp)
{
	...
	if (*hobj==NULL) {
		cobj=*hobj=calloc(1, sizeof(obj_t));
		nes_linkval(sid->N, cobj, NULL);
		cobj->val->type=NT_TABLE;
//		cobj->val->attr|=NST_AUTOSORT;
	}
	cobj=NULL;
	nes_setstr(sid->N, *hobj, "status", "n", 1);
	for (;;) {
		if (fp!=NULL) {
		...
		if (strncasecmp(inbuffer, "Reply-To:", 9)==0) {
			p1=inbuffer+9;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "reply-to", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "From:", 5)==0) {
			p1=inbuffer+5;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "from", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "To:", 3)==0) {
			p1=inbuffer+3;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "to", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "CC:", 3)==0) {
			p1=inbuffer+3;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "cc", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "Subject:", 8)==0) {
			p1=inbuffer+8;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "subject", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "Date:", 5)==0) {
			p1=inbuffer+5;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "date", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "Content-Type:", 13)==0) {
			p1=inbuffer+13;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "content-type", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "Content-Transfer-Encoding: ", 26)==0) {
			p1=inbuffer+26;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "content-transfer-encoding", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "In-Reply-To: ", 13)==0) {
			p1=inbuffer+13;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "in-reply-to", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "Message-ID: ", 12)==0) {
			p1=inbuffer+12;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			cobj=nes_setstr(sid->N, *hobj, "message-id", DecodeRFC2047(sid, p1), -1);
		} else if (strncasecmp(inbuffer, "Status:", 7)==0) {
			p1=inbuffer+7;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			if (strchr(p1, 'R')!=NULL) cobj=nes_setstr(sid->N, *hobj, "status", "r", -1);
		} else if ((inbuffer[0]==' ')||(inbuffer[0]=='\t')) {
			if (cobj==NULL) continue;
			p1=inbuffer;
			while ((*p1==' ')||(*p1=='\t')) p1++;
			nes_strcat(sid->N, cobj, p1, strlen(p1));
		} else {
			cobj=NULL;
		}
	}
	cobj=nes_getobj(sid->N, *hobj, "content-type");
	if (nes_isstr(cobj)&&(cobj->val->size>0)) {
		if ((p1=p_strcasestr(cobj->val->d.str, "boundary="))!=NULL) {
			p1+=9;
			if (*p1==' ') p1++;
			if (*p1=='\"') {
				p1++;
	...
	if (nes_isstr(cobj)&&(cobj->val->size>0)) {
		...
		if (*p1=='<') {
			...
			p2=p1;
			while ((*p2)&&(*p2!='>')) p2++;
			cobj=nes_setstr(sid->N, *hobj, "reply-to", p1, p2-p1);
		}
	} else {
		cobj=nes_getobj(sid->N, *hobj, "from");
		if (nes_isstr(cobj)&&(cobj->val->size>0)) {
			p1=cobj->val->d.str;
			while ((*p1)&&(*p1!='<')) p1++;
			if (*p1=='<') {
			...
			if (!nes_isstr(cobj)||(cobj->val->size<1)) {
				cobj=nes_setstr(sid->N, *hobj, "reply-to", nes_getstr(sid->N, *hobj, "from"), -1);
			}
		}
	}
	cobj=nes_getobj(sid->N, *hobj, "from");
	if (nes_isnull(cobj)) nes_setstr(sid->N, *hobj, "from", "(No Sender Name)", -1);
	cobj=nes_getobj(sid->N, *hobj, "subject");
	if (nes_isnull(cobj)) nes_setstr(sid->N, *hobj, "subject", "(No Subject)", -1);
	cobj=nes_getobj(sid->N, *hobj, "date");
	if (nes_isnull(cobj)) nes_setstr(sid->N, *hobj, "date", "(No Date)", -1);
	return 0;
}
...
/*
	...
	if (sql_queryf(&sqr, "SELECT mailheaderid, folder, hdr_contenttype, hdr_boundary, hdr_encoding FROM gw_email_headers WHERE obj_uid = %d and accountid = %d and status != 'd' AND mailheaderid = %d", sid->dat->uid, sid->dat->mailcurrent, msgnum)<0) return;
...
char *webmailfileul(CONN *sid, char *xfilename, char *xfilesize)
{
	...
	obj_t *cobj=nes_getobj(sid->N, pobj, "ATTACHMENT1");
...
/*
	...
	int ContentLength=(int)nes_getnum(sid->N, htobj, "CONTENT_LENGTH");
	char *filebody=NULL;
	char lfilename[1024];
	int filesize=0;
	char line[1024];
...
/*	while ((*pPostData!='\r')&&(*pPostData!='\n')&&(i<ContentLength)&&(strlen(boundary)<sizeof(boundary)-1)) {
...
/*	while ((*pPostData=='\n')||(*pPostData=='\r')) {
	...
	pPostData=sid->PostData;
	while ((strcmp(line, boundary2)!=0)&&(i<ContentLength)) {
		memset(line, 0, sizeof(line));
		j=0;
		while ((*pPostData!='\r')&&(*pPostData!='\n')&&(i<ContentLength)&&(strlen(line)<sizeof(line)-1)) {
			line[j]=*pPostData;
			pPostData++;
			i++;
			j++;
...
/*		while ((*pPostData=='\n')||(*pPostData=='\r')) {
		...
		filebody=pPostData;
		filesize=0;
		while ((strncmp(pPostData, boundary3, strlen(boundary3))!=0)&&(i<ContentLength)) {
			pPostData++;
			filesize++;
		}
	}
...
//			snprintf(sqltemp, bodysize+255, "UPDATE gw_email_headers SET msg_text = '%s' WHERE mailheaderid = %d AND obj_uid = %d AND obj_did = %d AND accountid = %d", str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, bodytemp), nummessage, sid->dat->uid, sid->dat->did, accountid);
//			sql_update(sqltemp);
//			free(sqltemp);
//		}
		if ((reply)||(strncasecmp(contenttype, "text/html", 9)!=0)) {
...
//		}
	...
	if ((!reply)&&(sid->dat->wm->numfiles>0)&&(depth<2)) {
		...
		for (i=0;i<sid->dat->wm->numfiles;i++) {
			prints(sid, "[<A HREF=%s/mail/file/%d/", ScriptName, nummessage);
			printhex(sid, "%s", DecodeRFC2047(sid, sid->dat->wm->files[i]));
			ptemp=get_mime_type(sid, sid->dat->wm->files[i]);
			if (strncasecmp(ptemp, "image/", 6)==0) {
