...
/*
	...
	prints(sid, "}\r\n");
	prints(sid, "//--></SCRIPT>\r\n");
	if (sql_queryf(&sqr1, "SELECT userid, surname, givenname, email from gw_users WHERE domainid = %d ORDER BY surname, givenname ASC", sid->dat->did)<0) return;
	if (auth_priv(sid, "contacts")&A_ADMIN) {
		if (sql_queryf(&sqr2, "SELECT contactid, surname, givenname, email from gw_contacts WHERE obj_did = %d ORDER BY surname, givenname ASC", sid->dat->did)<0) return;
	} else {
		if (sql_queryf(&sqr2, "SELECT contactid, surname, givenname, email from gw_contacts WHERE (obj_uid = %d or (obj_gid = %d and obj_gperm>=1) or obj_operm>=1) AND obj_did = %d ORDER BY surname, givenname ASC", sid->dat->uid, sid->dat->gid, sid->dat->did)<0) return;
	}
