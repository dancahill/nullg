...
//	SQLRES sqr2;
	...
	prints(sid, "<BR /><CENTER>\r\n<B><FONT COLOR=#808080 SIZE=3>GroupServer E-Mail Notice</FONT></B>\r\n");
...
/*
	if (sql_queryf(&sqr1, "SELECT mailaccountid, accountname, poppassword FROM gw_email_accounts where obj_uid = %d ORDER BY accountname ASC", sid->dat->uid)<0) return;
	for (i=0;i<sql_numtuples(&sqr1);i++) {
		sid->dat->mailcurrent=atoi(sql_getvalue(&sqr1, i, 0));
		if (sql_queryf(&sqr2, "SELECT count(mailheaderid) FROM gw_email_headers WHERE obj_uid = %d AND accountid = %d AND folder = 1", sid->dat->uid, sid->dat->mailcurrent)<0) continue;
		nummessages=atoi(sql_getvalue(&sqr2, 0, 0));
		sql_freeresult(&sqr2);
		if (sql_queryf(&sqr2, "SELECT count(mailheaderid) FROM gw_email_headers WHERE obj_uid = %d AND accountid = %d AND folder = 1 AND status = 'n'", sid->dat->uid, sid->dat->mailcurrent)<0) continue;
...
/*
	...
	sql_updatef("UPDATE gw_email_accounts SET obj_mtime = '%s',  poppassword = '' WHERE obj_uid = %d AND mailaccountid = %d", timebuffer, sid->dat->uid, sid->dat->mailcurrent);
	wmloginform(sid);
*/
	return;
}
...
//	if (sql_queryf(&sqr, "SELECT mailheaderid, folder, status FROM gw_email_headers WHERE obj_uid = %d and accountid = %d AND mailheaderid = %d and status != 'd'", sid->dat->uid, sid->dat->mailcurrent, msgnum)<0) return;
	...
	accountid=atoi(ldir_getval(&qobj, 0, "accountid"));
	folderid=atoi(ldir_getval(&qobj, 0, "folderid"));
	headerid=atoi(ldir_getval(&qobj, 0, "mailheaderid"));
	memset(msgfilename, 0, sizeof(msgfilename));
//	snprintf(msgfilename, sizeof(msgfilename)-1, "%s/%04d/mail/%04d/%04d/%06d.msg", config->dir_var_domains, sid->dat->did, sid->dat->mailcurrent, folderid, msgnum);
	snprintf(msgfilename, sizeof(msgfilename)-1, "%s/domains/%04d/mail/%04d/%04d/%06d.msg", nes_getstr(proc->N, confobj, "var_path"), sid->dat->did, accountid, folderid, headerid);
...
void webmaillist(CONN *sid)
{
	...
	// not elegant, but it gets a default selected...
//	accountid=atoi(ldir_getval(&tobj, 0, "id"));
	...
	if ((ptemp=getgetenv(sid, "FOLDERID"))!=NULL) {
		...
		for (i=0;i<ldir_numentries(&tobj);i++) {
			if (atoi(ldir_getval(&tobj, i, "folderid"))==1) {
				folderid=atoi(ldir_getval(&tobj, i, "id"));
				break;
			}
		}
		ldir_freeresult(&tobj);
...
//			sql_updatef("UPDATE gw_users SET prefmailcurrent = '%d' WHERE username = '%s'", atoi(ptemp), sid->dat->username);
			if ((qobj1=ldir_getentry(sid->N, "person", NULL, sid->dat->uid, sid->dat->did))!=NULL) {
				...
				ldir_saveentry(sid, sid->dat->uid, "person", &qobj1);
...
//			folderid=1;
	...
	htselect_mailjump(sid, accountid, folderid);
	prints(sid, "</TD></FORM>\r\n");
...
/*
	...
	prints(sid, "<FORM METHOD=GET NAME=mailjump ACTION=%s/mail/list onChange=\"mailjump()\">\r\n", ScriptName);
	prints(sid, "<TD ALIGN=RIGHT>\r\n");
	htselect_mailjump(sid, accountid);
	prints(sid, "</TD></FORM></TR></TABLE>\r\n");
*/
	flushbuffer(sid);
	if (!folderid) return;
	if ((tobj=ldir_getlist(sid->N, "emailheader", accountid, sid->dat->did))==NULL) {
...
//	if (offset>nummessages-sid->dat->maxlist) offset=nummessages-sid->dat->maxlist;
	...
	if (nummessages>sid->dat->maxlist) {
		if (offset>0) {
			i=offset-sid->dat->maxlist;
			if (i<0) i=0;
			prints(sid, "<A HREF=%s/mail/list?folderid=%d&offset=%d&order=%d%s>&lt;&lt;</A>", ScriptName, folderid, i, order, searchstring);
		} else {
			prints(sid, "&lt;&lt;");
		}
		prints(sid, " Listing %d-%d of %d ", offset+1, offset+sid->dat->maxlist, nummessages);
		if (offset+sid->dat->maxlist<nummessages) {
			i=offset+sid->dat->maxlist;
			if (i>nummessages-sid->dat->maxlist) i=nummessages-sid->dat->maxlist;
			prints(sid, "<A HREF=%s/mail/list?folderid=%d&offset=%d&order=%d%s>&gt;&gt;</A>", ScriptName, folderid, i, order, searchstring);
		} else {
			prints(sid, "&gt;&gt;");
		}
	...
	prints(sid, "<TH ALIGN=LEFT NOWRAP WIDTH=120 style='cursor:hand; border-style:solid' onClick=\"window.location.href='%s/mail/list?folderid=%d&order=%d%s'\">", ScriptName, folderid, neworder, searchstring);
	prints(sid, "&nbsp;<A HREF=\"list?folderid=%d&order=%d%s\">%s</A>&nbsp;</TH>\r\n", folderid, neworder, searchstring, lang_gets(sid, "mod_email", "mail_from"));
	neworder=(order==5?4:5);
	prints(sid, "<TH ALIGN=LEFT NOWRAP style='cursor:hand; border-style:solid' onClick=\"window.location.href='%s/mail/list?folderid=%d&order=%d%s'\" WIDTH=100%%>", ScriptName, folderid, neworder, searchstring);
	prints(sid, "&nbsp;<A HREF=\"list?folderid=%d&order=%d%s\">%s</A>&nbsp;</TH>\r\n", folderid, neworder, searchstring, lang_gets(sid, "mod_email", "mail_subject"));
	neworder=(order==0?1:0);
	prints(sid, "<TH ALIGN=LEFT NOWRAP style='cursor:hand; border-style:solid' onClick=\"window.location.href='%s/mail/list?folderid=%d&order=%d%s'\">", ScriptName, folderid, neworder, searchstring);
	prints(sid, "&nbsp;<A HREF=\"list?folderid=%d&order=%d%s\">%s</A>&nbsp;</TH>\r\n", folderid, neworder, searchstring, lang_gets(sid, "mod_email", "mail_date"));
	neworder=(order==6?7:6);
	prints(sid, "<TH ALIGN=LEFT NOWRAP style='cursor:hand; border-style:solid' onClick=\"window.location.href='%s/mail/list?folderid=%d&order=%d%s'\">", ScriptName, folderid, neworder, searchstring);
	prints(sid, "&nbsp;<A HREF=\"list?folderid=%d&order=%d%s\">%s</A>&nbsp;</TH>\r\n", folderid, neworder, searchstring, lang_gets(sid, "mod_email", "mail_size"));
	prints(sid, "<TH ALIGN=LEFT STYLE='border-style:solid'>&nbsp;</TH>");
	prints(sid, "</TR>\r\n");
	for (i=offset;(i<nummessages)&&(i<offset+sid->dat->maxlist);i++) {
		if (atoi(ldir_getval(&tobj, i, "folderid"))!=folderid) continue;
		if (strcmp(ldir_getval(&tobj, i, "status"), "n")==0) {
			// sql_updatef("UPDATE gw_email_headers SET status = 'o' WHERE accountid = %d AND obj_uid = %d AND status = 'n'", sid->dat->mailcurrent, sid->dat->uid);
			// you get the idea...  update 'n' headers to 'o'
		}
//		dbread_getheader(sid, &sqr, i, &header);
		prints(sid, "<TR BGCOLOR=\"%s\">", strcmp(ldir_getval(&tobj, i, "status"), "r")!=0?"#D0D0FF":"#F0F0F0");
		if (searchstring[0]=='\0') {
			prints(sid, "<TD NOWRAP STYLE='padding:0px; border-style:solid'><INPUT TYPE=checkbox NAME=%d VALUE=\"%s\"></TD>\r\n", ldir_getval(&tobj, i, "id"), ldir_getval(&tobj, i, "uidl"));
		}
		prints(sid, "<TD NOWRAP TITLE=\"%s\" STYLE='border-style:solid'>", str2html(sid, ldir_getval(&tobj, i, "hdr_from")));
		prints(sid, "<DIV STYLE='width:120px;overflow:hidden'>%s&nbsp;</DIV></TD>\r\n", str2html(sid, ldir_getval(&tobj, i, "hdr_from")));//str2html(sid, header.FromName));
		prints(sid, "<TD NOWRAP style='cursor:hand; border-style:solid' onClick=\"");
		if (menustyle>0) {
			prints(sid, "window.parent.wmread.location.href='%s/mail/read?msg=%d&order=%d%s'\" ", ScriptName, atoi(ldir_getval(&tobj, i, "id")), order, searchstring);
		} else {
			prints(sid, "window.location.href='%s/mail/read?msg=%d&order=%d%s'\" ", ScriptName, atoi(ldir_getval(&tobj, i, "id")), order, searchstring);
		}
		prints(sid, "TITLE=\"%s\"><DIV STYLE='width:315px;overflow:hidden'>", str2html(sid, ldir_getval(&tobj, i, "hdr_subject")));
		prints(sid, "<A HREF=%s/mail/read?msg=%d&order=%d%s", ScriptName, atoi(ldir_getval(&tobj, i, "id")), order, searchstring);
		prints(sid, "%s TITLE=\"%s\">", menustyle>0?" TARGET=wmread":"", str2html(sid, ldir_getval(&tobj, i, "hdr_subject")));
		prints(sid, "%s</A>&nbsp;</DIV></TD>\r\n", str2html(sid, ldir_getval(&tobj, i, "hdr_subject")));
		unixdate=time_sql2unix(ldir_getval(&tobj, i, "hdr_date"));
		...
		size=atoi(ldir_getval(&tobj, i, "size"));
		if (size>=1048576) {
			prints(sid, "%1.1f M", (float)size/1048576.0);
		} else {
			prints(sid, "%1.1f K", (float)size/1024.0);
		}
		prints(sid, "</TD>\r\n<TD STYLE='border-style:solid'>");
		if (p_strcasestr(ldir_getval(&tobj, i, "hdr_contenttype"), "multipart/mixed")!=NULL) {
			prints(sid, "<IMG BORDER=0 SRC=/nullgs/images/paperclip.png HEIGHT=16 WIDTH=11 ALT='File Attachments'>");
		} else {
			prints(sid, "&nbsp;&nbsp;&nbsp;");
		}
		prints(sid, "</TD></TR>\r\n");
	...
	if (nummessages>sid->dat->maxlist) {
		if (offset>0) {
			i=offset-sid->dat->maxlist;
			if (i<0) i=0;
			prints(sid, "[<A HREF=%s/mail/list?folderid=%d&offset=%d&order=%d%s>%s</A>]", ScriptName, folderid, i, order, searchstring, lang_gets(sid, "mod_email", "mail_prevpage"));
		} else {
			prints(sid, "[%s]", lang_gets(sid, "mod_email", "mail_prevpage"));
		}
		if (offset+sid->dat->maxlist<nummessages) {
			i=offset+sid->dat->maxlist;
			if (i>nummessages-sid->dat->maxlist) i=nummessages-sid->dat->maxlist;
			prints(sid, "[<A HREF=%s/mail/list?folderid=%d&offset=%d&order=%d%s>%s</A>]", ScriptName, folderid, i, order, searchstring, lang_gets(sid, "mod_email", "mail_nextpage"));
		} else {
			prints(sid, "[%s]", lang_gets(sid, "mod_email", "mail_nextpage"));
		}
	}
	prints(sid, "</CENTER>\r\n");
	ldir_freeresult(&tobj);
...
//	memset((char *)&header, 0, sizeof(header));
	...
	folderid=atoi(ldir_getval(&tobj, 0, "pid"));
//	ldir_freeresult(&tobj);
	nummessages=1;

//	if (sql_queryf(&sqr, "SELECT mailheaderid, folder FROM gw_email_headers WHERE obj_uid = %d and accountid = %d and status != 'd' and mailheaderid = %d", sid->dat->uid, sid->dat->mailcurrent, localid)<0) return;
//	if (sql_numtuples(&sqr)==1) {
...
//	}
	...
	prints(sid, "<CENTER>\r\n");
	if (remoteid>1) {
//		prints(sid, "[<A HREF=%s/mail/read?msg=%d&order=%d%s>%s</A>]\r\n", ScriptName, atoi(sql_getvalue(&sqr, remoteid-2, 0)), order, searchstring, lang_gets(sid, "mod_email", "mail_prev"));
		prints(sid, "[<A HREF=%s/mail/read?msg=%d&order=%d%s>%s</A>]\r\n", ScriptName, atoi(ldir_getval(&tobj, remoteid-2, "id")), order, searchstring, lang_gets(sid, "mod_email", "mail_prev"));
	} else {
		prints(sid, "[%s]\r\n", lang_gets(sid, "mod_email", "mail_prev"));
	}
	if (menustyle>0) {
		prints(sid, "[<A HREF=javascript:ReplyTo();>%s</A>]\r\n",  lang_gets(sid, "mod_email", "mail_reply"));
	...
	if ((remoteid<nummessages)&&(remoteid>-1)) {
		prints(sid, "[<A HREF=%s/mail/read?msg=%d&order=%d%s>%s</A>]\r\n", ScriptName, atoi(ldir_getval(&tobj, remoteid, "id")), order, searchstring, lang_gets(sid, "mod_email", "mail_next"));
	} else {
		prints(sid, "[%s]\r\n", lang_gets(sid, "mod_email", "mail_next"));
	}
...
//	prints(sid, "<A HREF=\"javascript:MsgTo('&quot;%s&quot; <%s>');\">Send Mail</A>", encodeurl(sid, header.FromName), header.ReplyTo);
	...
	printht(sid, "%s", ldir_getval(&tobj, 0, "hdr_to"));
	prints(sid, "&nbsp;</TD></TR>\r\n");
	if (strlen(ldir_getval(&tobj, 0, "hdr_cc"))) {
		prints(sid, "<TR><TH ALIGN=LEFT VALIGN=TOP>&nbsp;%s&nbsp;</TH><TD CLASS=\"FIELDVAL\" WIDTH=100%%>&nbsp;", lang_gets(sid, "mod_email", "mail_cc"));
		printht(sid, "%s", ldir_getval(&tobj, 0, "hdr_cc"));
		prints(sid, "&nbsp;</TD></TR>\r\n");
	}
	if (strlen(ldir_getval(&tobj, 0, "hdr_bcc"))) {
		prints(sid, "<TR><TH ALIGN=LEFT VALIGN=TOP>&nbsp;%s&nbsp;</TH><TD CLASS=\"FIELDVAL\" WIDTH=100%%>&nbsp;", lang_gets(sid, "mod_email", "mail_bcc"));
		printht(sid, "%s", ldir_getval(&tobj, 0, "hdr_bcc"));
		prints(sid, "&nbsp;</TD></TR>\r\n");
	}
	prints(sid, "<TR><TH ALIGN=LEFT VALIGN=TOP>&nbsp;%s&nbsp;</TH><TD CLASS=\"FIELDVAL\" WIDTH=100%%>&nbsp;", lang_gets(sid, "mod_email", "mail_subject"));
	printht(sid, "%s", ldir_getval(&tobj, 0, "hdr_subject"));
	prints(sid, "&nbsp;</TD></TR>\r\n");
	prints(sid, "<TR><TH ALIGN=LEFT VALIGN=TOP>&nbsp;%s&nbsp;</TH><TD CLASS=\"FIELDVAL\" WIDTH=100%%>&nbsp;", lang_gets(sid, "mod_email", "mail_date"));
	memset(curdate, 0, sizeof(curdate));
	unixdate=time_sql2unix(ldir_getval(&tobj, 0, "hdr_date"));
	unixdate+=time_tzoffset(sid, unixdate);
	if (unixdate<0) unixdate=0;
	strftime(curdate, 30, "%B %d, %Y %I:%M %p", gmtime(&unixdate));
	printht(sid, "%s", curdate);
	prints(sid, "&nbsp;</TD></TR>\r\n");
	if (strlen(ldir_getval(&tobj, 0, "hdr_scanresult"))) {
		prints(sid, "<TR><TH ALIGN=LEFT VALIGN=TOP NOWRAP>&nbsp;Scan Result&nbsp;</TH><TD CLASS=\"FIELDVAL\" WIDTH=100%%>&nbsp;%s&nbsp;</TD></TR>\r\n", ldir_getval(&tobj, 0, "hdr_scanresult"));
	}
	prints(sid, "</TABLE></TD></TR>\r\n");
	prints(sid, "<TR CLASS=\"FIELDVAL\"><TD STYLE='border-style:solid'>[<A HREF=%s/mail/raw?msg=%d TARGET=_blank>%s</A>]</TD></TR>\r\n", ScriptName, localid, lang_gets(sid, "mod_email", "mail_viewsource"));
	prints(sid, "<TR CLASS=\"FIELDVAL\"><TD STYLE='border-style:solid'>\r\n");
//	sql_updatef("UPDATE gw_email_headers SET status = 'r' WHERE uidl = '%s' AND obj_uid = %d AND accountid = %d", str2sql(getbuffer(sid), sizeof(sid->dat->smallbuf[0])-1, ldir_getval(&tobj, 0, "uidl")), sid->dat->uid, sid->dat->mailcurrent);
	memset(msgfilename, 0, sizeof(msgfilename));
	snprintf(msgfilename, sizeof(msgfilename)-1, "%s/domains/%04d/mail/%04d/%04d/%06d.msg", nes_getstr(proc->N, confobj, "var_path"), sid->dat->did, atoi(ldir_getval(&tobj, 0, "accountid")), atoi(ldir_getval(&tobj, 0, "folderid")), atoi(ldir_getval(&tobj, 0, "mailheaderid")));
	fixslashes(msgfilename);
	if (stat(msgfilename, &sb)!=0) {
		memset(oldfilename, 0, sizeof(oldfilename));
		snprintf(oldfilename, sizeof(oldfilename)-1, "%s/domains/%04d/mail/%04d/%06d.msg", nes_getstr(proc->N, confobj, "var_path"), sid->dat->did, atoi(ldir_getval(&tobj, 0, "accountid")), atoi(ldir_getval(&tobj, 0, "mailheaderid")));
...
//		webmailmime(sid, &fp, header.contenttype, header.encoding, header.boundary, localid, 0, 0);
		webmailmime(sid, &fp, ldir_getval(&tobj, 0, "hdr_contenttype"), ldir_getval(&tobj, 0, "hdr_encoding"), ldir_getval(&tobj, 0, "hdr_boundary"), atoi(ldir_getval(&tobj, 0, "mailheaderid")), 0, 0);
	} else {
...
//			return;
//		} else {
//			folderid=atoi(sql_getvaluebyname(&sqr, 0, "folderid"));
//		}
...
//			strncpy(msgcc, header.CC, sizeof(msgcc)-strlen(msgto)-1);
	...
	prints(sid, "	var msgbody = document.wmcompose.msgbody;\r\n");
	prints(sid, "	var speller = new spellChecker( msgbody );\r\n");
...
//	prints(sid, "<INPUT TYPE=hidden NAME=inreplyto VALUE=\"%s\">\r\n", header.MessageID);
	...
	prints(sid, "	ALT='Bold'                   SRC='/images/wmedit/bold.png'      onClick=wmformat('Bold')                 BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Italic'                 SRC='/images/wmedit/italic.png'    onClick=wmformat('Italic')               BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Underline'              SRC='/images/wmedit/underline.png' onClick=wmformat('Underline')            BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Font Colour'            SRC='/images/wmedit/fontcol.png'   onClick=SetForeColour()                  BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)></TD><TD BGCOLOR=#000000 STYLE='padding:1px'></TD><TD STYLE='padding:0px'><IMG\r\n");
	prints(sid, "	ALT='Formatting Numbers'     SRC='/images/wmedit/numlist.png'   onClick=wmformat('InsertOrderedList')    BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Formatting Bullets'     SRC='/images/wmedit/bullist.png'   onClick=wmformat('InsertUnorderedList')  BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Decrease Indentation'   SRC='/images/wmedit/outdent.png'   onClick=wmformat('Outdent')              BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Increase Indentation'   SRC='/images/wmedit/indent.png'    onClick=wmformat('Indent')               BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)></TD><TD BGCOLOR=#000000 STYLE='padding:1px'></TD><TD STYLE='padding:0px'><IMG\r\n");
	prints(sid, "	ALT='Align Left'             SRC='/images/wmedit/left.png'      onClick=wmformat('JustifyLeft')          BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Center'                 SRC='/images/wmedit/center.png'    onClick=wmformat('JustifyCenter')        BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Align Right'            SRC='/images/wmedit/right.png'     onClick=wmformat('JustifyRight')         BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Justify'                SRC='/images/wmedit/justify.png'   onClick=wmformat('Justify')              BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)></TD><TD BGCOLOR=#000000 STYLE='padding:1px'></TD><TD STYLE='padding:0px' width=100%><IMG\r\n");
	prints(sid, "	ALT='Insert Horizontal Line' SRC='/images/wmedit/rule.png'      onClick=wmformat('InsertHorizontalRule') BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Create a Hyperlink'     SRC='/images/wmedit/link.png'      onClick=doLink()                         BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)><IMG\r\n");
	prints(sid, "	ALT='Insert Picture'         SRC='/images/wmedit/image.png'     onClick=doImage()                        BORDER=0 WIDTH=22 HEIGHT=22 CLASS=butClass onMouseOver=selOn(this) onMouseOut=selOff(this)></TD>\r\n");
	prints(sid, "</TR></TABLE>\r\n");
	prints(sid, "<CENTER><IFRAME id=\"wmeditor\" HEIGHT=236 WIDTH=646 SRC=\"/mail/blank\"></IFRAME></CENTER>");
	prints(sid, "</DIV>");
...
//				webmailmime(sid, &fp, header.contenttype, header.encoding, header.boundary, msgnum, 1, 0);
	...
	prints(sid, "<INPUT TYPE=BUTTON CLASS=frmButton VALUE='Check Spelling' onClick=openSpellChecker();>\r\n");
	prints(sid, "</FORM>\r\n</CENTER>\r\n");
	prints(sid, "<SCRIPT LANGUAGE=JavaScript>\r\n<!--\r\n");
...
//		}
	...
	if (sql_numtuples(proc->N, &qobj)<1) {
		prints(sid, "<SCRIPT LANGUAGE=JavaScript>\r\n<!--\r\nlocation.replace(\"%s/mail/accounts/editnew\");\r\n// -->\r\n</SCRIPT>\r\n", ScriptName);
		prints(sid, "<NOSCRIPT>\r\n<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0; URL=%s/mail/accounts/editnew\">\r\n</NOSCRIPT>\r\n", ScriptName);
		prints(sid, "</HTML>\r\n");
	} else {
...
//		flushbuffer(sid);
	...
	if (sid->dat->wm==NULL) {
		if ((sid->dat->wm=calloc(1, sizeof(EMAIL)))==NULL) return 0;
	}
	if (strncmp(suburi, "main", 4)==0) {
		if (menustyle>0) {
			...
			RequestURI=nes_tostr(sid->N, nes_setstr(sid->N, htobj, "REQUEST_URI", "/mail/list", strlen("/mail/list")));
			suburi=RequestURI+6;
		}
	}
...
DllExport int mod_init(_PROC *_proc)
{
	...
	tobj=nes_settable(proc->N, nes_settable(proc->N, &proc->N->g, "GWMODULES"), "email");
	nes_exec(proc->N, "GWMODULES.email={submenu=1,name='E-MAIL',pic='email.png',uri='/mail/main',perm='email',fn_name='mod_main',fn_uri='/mail/'};");
	...
	/* if (mod_export_function("mod_email", "mod_email_sync", wmsync)!=0) return -1; */
	return 0;
}
