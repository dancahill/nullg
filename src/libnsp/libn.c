...
#define __FN__ __FILE__ ":nl_base_method()"
	...
	if (ftype == NT_STRING) {
		if (nc_strcmp(fname, "replace") == 0) {
			return nl_strrep(N);
		}
		else if (nc_strcmp(fname, "split") == 0) {
			return nl_strsplit(N);
		}
		else if (nc_strcmp(fname, "str") == 0 || nc_strcmp(fname, "istr") == 0) {
			return nl_strstr(N);
		}
		else if (nc_strcmp(fname, "sub") == 0 || nc_strcmp(fname, "substring") == 0) {
			return nl_strsub(N);
		}
		else if (nc_strcmp(fname, "tolower") == 0 || nc_strcmp(fname, "toupper") == 0) {
			return nl_strtolower(N);
		}
	}
	...
	else if (nc_strcmp(fname, "gettype") == 0) {
		return nl_typeof(N);
	}
	// this should be an exception, but that leaks memory!
	//	n_warn(N, __FN__, "object has no method named '%s'.", fname);
