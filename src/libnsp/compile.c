...
#define __FN__ __FILE__ ":n_decompose_sub()"
	...
	while (*N->readptr) {
		...
		if (op == OP_UNDEFINED) {
			...
			n_warn(N, __FN__, "bad op? index=%d op=%d:%d name='%s'", state->index, op, N->readptr[0], lastname);
			for (cobj=state->tobj1->val->d.table.f;cobj;cobj=cobj->next) {
			cobj->val->attr=0;
			}
			n_dumpvars(N, &N->g, 0);
			n_error(N, NE_SYNTAX, __FN__, "bad op");
			*/
			return;
		}
		else if (op == OP_LABEL) {
			...
			/*
				if (op==OP_POBRACE) {
				nobj=nsp_settable(N, tobj, n_ntoa(N, x, *index, 10, 0));
				*index+=1;
				while (N->lastop!=OP_PCBRACE) {
				n_decompose_sub(N, srctext, nobj, index);
				}
				if (N->lastop==OP_PCBRACE) {
				nsp_setstr(N, tobj, n_ntoa(N, x, *index, 10, 0), lastname, -1);
				*index+=1;
				}
				}
...
uchar   *n_decompose(nsp_state *N, uchar *srcfile, uchar *srctext, uchar **dsttext, int *dstsize)
{
