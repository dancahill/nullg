	...
	x:
		...
		case OP_POPAREN:
			if (nobj->val->type == NT_NFUNC || nobj->val->type == NT_CFUNC) {
				...
				obj_t *xobj = nsp_getobj(N, cobj, "_constructor");
				if (!nsp_isnull(xobj)) nobj = n_execfunction(N, xobj, cobj, isnewobject);
				isnewobject = 0;
				return;
			}
			else {
		...
		case NT_TABLE: {
			if (isnewobject) {
				...
				nsp_zlink(N, cobj, nobj);
				obj_t *xobj = nsp_getobj(N, cobj, "_constructor");
				if (!nsp_isnull(xobj)) n_execfunction(N, xobj, cobj, isnewobject);
			}
			else {
