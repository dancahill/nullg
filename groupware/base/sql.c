...
#endif
/* shared vars */
static short int sqlIsConnected=0;
#ifdef WIN32
...
int SQLiteDLLInit()
{
	...
	return 0;
fail:
	logerror(NULL, __FILE__, __LINE__, "ERROR: Failed to load libsqlite");
...
int sqliteConnect(CONNECTION *sid)
{
	...
	if (sqlIsConnected) return 0;
	snprintf(dbname, sizeof(dbname)-1, "%s/groupware.db", config.server_etc_dir);
	fixslashes(dbname);
 ...
 *	Notes	: None
 ***************************************************************************/
void sqlDisconnect(CONNECTION *sid)
{
...
void sqlUnsafeDisconnect(CONNECTION *sid)
{
	logaccess(sid, 4, "SQL Disconnection");
	if (strcmp(config.sql_type, "ODBC")==0) {
		odbcDisconnect();
...
int sqlUpdate(CONNECTION *sid, char *sqlquery)
{
	int rc=-1;
...
int sqlUpdatef(CONNECTION *sid, char *format, ...)
{
	...
	va_end(ap);
	return sqlUpdate(sid, sqlquery);
}

int sqlQuery(CONNECTION *sid, char *query)
{
	int i;
	int rc=-1;
...
int sqlQueryf(CONNECTION *sid, char *format, ...)
{
	unsigned char sqlquery[8192];
	va_list ap;
