...
#endif
	logaccess(NULL, 0, "Stopping %s (%s)", SERVER_NAME, __DATE__);
...
void server_restart()
{
	...
	if ((pthread_t)pthread_self()!=DaemonThread) return;
#endif
	logaccess(NULL, 0, "Restarting %s (%s)", SERVER_NAME, __DATE__);
	pthread_kill(ListenThread, 14);
	shutdown(ListenSocket, 2);
	closesocket(ListenSocket);
	sleep(1);
...
void init()
{
	...
	printf(".");
	conn=calloc(config.server_maxconn, sizeof(CONNECTION));
	logaccess(NULL, 0, "Starting %s (%s)", SERVER_NAME, __DATE__);
	sqlreply=calloc(config.sql_maxconn, sizeof(SQLRES));
	if (sanity_dbcheck()==-1) {
		logerror(NULL, __FILE__, __LINE__, "SQL subsystem failed sanity check");
#ifdef WIN32
...
void dorequest(CONNECTION *sid)
{
	...
	snprintf(file, sizeof(file)-1, "%s%s", config.server_htdocs_dir, sid->dat->in_RequestURI);
	...
	if (auth_getcookie(sid)!=0) {
		if (auth_setcookie(sid)==0) {
			logaccess(sid, 0, "%s - Login: username=%s", sid->dat->in_RemoteAddr, sid->dat->user_username);
			...
			logaccess(sid, 0, "%s - Login failed: username=%s", sid->dat->in_RemoteAddr, sid->dat->user_username);
			htpage_login(sid);
			DEBUG_OUT(sid, "dorequest()");
			return;
		} else {
	...
	if (strncmp(sid->dat->in_RequestURI, "/logout", 7)==0) auth_logout(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/main/main",  10)==0) htpage_motd(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/main/temp",  10)==0) htpage_reload(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/main/menu",  10)==0) htpage_sidemenu(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/admin/",      7)==0) adminmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/calendar/",  10)==0) calendarmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/cgi-bin/",    9)==0) cgi_main(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/calls/",      7)==0) callsmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/contacts/",  10)==0) contactmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/file",        5)==0) filemain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/forums/",     8)==0) forummain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/bookmarks/", 11)==0) bookmarksmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/mail/",       6)==0) webmailmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/messages/",  10)==0) messagemain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/notes/",      7)==0) notesmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/orders/",     8)==0) ordermain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/products/",  10)==0) productmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/profile/",    9)==0) profilemain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/search/",     8)==0) searchmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/tasks/",      7)==0) taskmain(sid);
	else if (strncmp(sid->dat->in_RequestURI, "/stats",       6)==0) htpage_stats(sid);
		...
		logerror(sid, __FILE__, __LINE__, "%s - Incorrect function call '%s' by %s", sid->dat->in_RemoteAddr, sid->dat->in_RequestURI, sid->dat->user_username);
	}
	DEBUG_OUT(sid, "dorequest()");
	return;
}
