...
static int htselect_mailfolder_r(CONN *sid, SQLRES *sqr, int depth, int parentid, int selected)
{
	int foldercount=0;
	int fid, pid;
	int i, j;
...
void htselect_mailfolder(CONN *sid, int selected, short int allow_zero, short int show_root)
{
	...
	if (sql_queryf(&sqr, "SELECT mailfolderid, parentfolderid, foldername FROM gw_mailfolders WHERE obj_uid = %d and accountid = %d ORDER BY parentfolderid ASC, foldername ASC", sid->dat->user_uid, sid->dat->user_mailcurrent)<0) return;
...
void htselect_mailaccount(CONN *sid, int selected)
{
	...
	if (sql_queryf(&sqr, "SELECT mailaccountid, accountname FROM gw_mailaccounts WHERE obj_uid = %d AND obj_did = %d ORDER BY mailaccountid ASC", sid->dat->user_uid, sid->dat->user_did)<0) return;
	for (i=0;i<sql_numtuples(&sqr);i++) {
...
void htselect_mailjump(CONN *sid, int selected)
{
	...
	if (sql_queryf(&sqr, "SELECT mailaccountid, accountname FROM gw_mailaccounts WHERE obj_uid = %d AND obj_did = %d ORDER BY accountname ASC", sid->dat->user_uid, sid->dat->user_did)<0) return;
	if ((sql_numtuples(&sqr)>0)&&(sid->dat->user_mailcurrent==0)) {
		sid->dat->user_mailcurrent=atoi(sql_getvalue(&sqr, 0, 0));
		sql_updatef("UPDATE gw_users SET prefmailcurrent = %d WHERE userid = %d AND domainid = %d", sid->dat->user_mailcurrent, sid->dat->user_uid, sid->dat->user_did);
	}
...
//	prints(sid, "	%slocation='%s/mail/%s?'+document.mailjump.accountid.options[document.mailjump.accountid.selectedIndex].value\r\n", sid->dat->user_menustyle>0?"top.gwmain.":"", sid->dat->in_ScriptName, sid->dat->user_menustyle>0?"main":"list");
	...
	for (i=0;i<sql_numtuples(&sqr1);i++) {
		...
		depth=1;
		indent=0;
		if (sql_queryf(&sqr2, "SELECT mailfolderid, parentfolderid, foldername FROM gw_mailfolders WHERE obj_uid = %d and accountid = %d ORDER BY parentfolderid ASC, foldername ASC", sid->dat->user_uid, account)<0) continue;
		if (sql_numtuples(&sqr2)<1) {
			sql_freeresult(&sqr2);
			wmfolder_makedefaults(sid, account);
			if (sql_queryf(&sqr2, "SELECT mailfolderid, parentfolderid, foldername FROM gw_mailfolders WHERE obj_uid = %d and accountid = %d ORDER BY parentfolderid ASC, foldername ASC", sid->dat->user_uid, sid->dat->user_mailcurrent)<0) continue;
		}
