...
int dbread_forumgroup(CONN *sid, short int perm, int index, REC_FORUMGROUP *forumgroup)
{
	...
	if (authlevel&A_ADMIN) {
		if (sql_queryf(&sqr, "SELECT * FROM gw_forumgroups where forumgroupid = %d AND obj_did = %d", index, sid->dat->user_did)<0) return -1;
	} else {
		if (sql_queryf(&sqr, "SELECT * FROM gw_forumgroups where forumgroupid = %d and (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d) AND obj_did = %d", index, sid->dat->user_uid, sid->dat->user_gid, perm, perm, sid->dat->user_did)<0) return -1;
	}
...
int dbwrite_forumgroup(CONN *sid, int index, REC_FORUMGROUP *forumgroup)
{
	...
	if (index==0) {
		if (sql_query(&sqr, "SELECT max(forumgroupid) FROM gw_forumgroups")<0) return -1;
		forumgroup->forumgroupid=atoi(sql_getvalue(&sqr, 0, 0))+1;
		sql_freeresult(&sqr);
		if (forumgroup->forumgroupid<1) forumgroup->forumgroupid=1;
		strcpy(query, "INSERT INTO gw_forumgroups (forumgroupid, obj_ctime, obj_mtime, obj_uid, obj_gid, obj_did, obj_gperm, obj_operm, title, description) values (");
