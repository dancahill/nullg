...
int dbread_forumgroup(CONN *sid, short int perm, int index, REC_FORUMGROUP *forumgroup)
{
	...
	if (authlevel&A_ADMIN) {
		if ((sqr=sql_queryf("SELECT * FROM gw_forumgroups where forumgroupid = %d AND obj_did = %d", index, sid->dat->user_did))<0) return -1;
	} else {
		if ((sqr=sql_queryf("SELECT * FROM gw_forumgroups where forumgroupid = %d and (obj_uid = %d or (obj_gid = %d and obj_gperm>=%d) or obj_operm>=%d) AND obj_did = %d", index, sid->dat->user_uid, sid->dat->user_gid, perm, perm, sid->dat->user_did))<0) return -1;
	}
