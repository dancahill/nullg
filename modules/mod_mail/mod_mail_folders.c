...
void wmfolder_list(CONN *sid)
{
	...
	prints(sid, "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=400>\n");
	prints(sid, "<TR BGCOLOR=%s STYLE='padding:2px'><TH ALIGN=left><FONT COLOR=%s>&nbsp;Mail Folder&nbsp;</FONT></TH></TR>\n", config->colour_th, config->colour_thtext);
	for (i=0; i<sql_numtuples(sqr); i++) {
		j=ptree[i].id;
		...
		prints(sid, "</TD><TD NOWRAP WIDTH=100%% style=\"cursor:hand\" onClick=\"window.location.href='%s/mail/folders/edit?folderid=%d'\">", sid->dat->in_ScriptName, atoi(sql_getvalue(sqr, j, 0)));
		prints(sid, "<A HREF=%s/mail/folders/edit?folderid=%d>", sid->dat->in_ScriptName, atoi(sql_getvalue(sqr, j, 0)));
		prints(sid, "<B>&nbsp;%s</B></A>&nbsp;", str2html(sid, sql_getvalue(sqr, j, 2)));
		if ((sqr2=sql_queryf(sid, "SELECT COUNT(mailheaderid) FROM gw_mailheaders WHERE obj_uid = %d AND accountid = %d AND folder = %d AND status != 'd'", sid->dat->user_uid, sid->dat->user_mailcurrent, atoi(sql_getvalue(sqr, j, 0))))<0) {
			free(ptree);
			free(btree);
			sql_freeresult(sqr);
			return;
		...
		if (sql_numtuples(sqr2)!=1) {
			...
			prints(sid, "(%d)", atoi(sql_getvalue(sqr2, 0, 0)));
		}
		sql_freeresult(sqr2);
		prints(sid, "</TD></TR></TABLE></TD></TR>\n");
	}
	prints(sid, "</TABLE>\n");
	prints(sid, "<A HREF=%s/mail/folders/editnew>New Folder</A>", sid->dat->in_ScriptName);
	prints(sid, "</CENTER>\n");
	free(ptree);
	free(btree);
	sql_freeresult(sqr);
...
void wmfolder_edit(CONN *sid)
{
	...
	if (strncmp(sid->dat->in_RequestURI, "/mail/folders/editnew", 21)==0) {
		folderid=0;
		accountid=sid->dat->user_mailcurrent;
		parentid=0;
		snprintf(foldername, sizeof(foldername)-1, "New Folder");
	} else {
		if ((ptemp=getgetenv(sid, "FOLDERID"))==NULL) return;
		folderid=atoi(ptemp);
		if ((sqr=sql_queryf(sid, "SELECT mailfolderid, accountid, parentfolderid, foldername FROM gw_mailfolders WHERE obj_uid = %d and accountid = %d and mailfolderid = %d ORDER BY parentfolderid ASC, foldername ASC", sid->dat->user_uid, sid->dat->user_mailcurrent, folderid))<0) return;
		if (sql_numtuples(sqr)==1) {
...
void wmfolder_save(CONN *sid)
{
	...
	if (((ptemp=getpostenv(sid, "SUBMIT"))!=NULL)&&(strcmp(ptemp, "Delete")==0)) {
		...
		if ((sqr=sql_queryf(sid, "SELECT mailheaderid FROM gw_mailheaders WHERE obj_uid = %d AND accountid = %d AND folder = %d", sid->dat->user_uid, sid->dat->user_mailcurrent, folderid))<0) return;
		...
		sql_freeresult(sqr);
		if ((sqr=sql_queryf(sid, "SELECT mailfolderid FROM gw_mailfolders WHERE obj_uid = %d AND accountid = %d AND parentfolderid = %d", sid->dat->user_uid, sid->dat->user_mailcurrent, folderid))<0) return;
		if (sql_numtuples(sqr)>0) {
			prints(sid, "<CENTER><B>This folder cannot be deleted unless it is empty.</B></CENTER>\n");
			sql_freeresult(sqr);
			return;
		...
		sql_freeresult(sqr);
		if (sql_updatef(sid, "DELETE FROM gw_mailfolders WHERE accountid = %d AND mailfolderid = %d", sid->dat->user_mailcurrent, folderid)<0) return;
		prints(sid, "<CENTER>Mail Folder %d deleted successfully</CENTER><BR>\n", folderid);
		prints(sid, "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL=%s/mail/folders\">\n", sid->dat->in_ScriptName);
	} else if (folderid==0) {
		if (strlen(foldername)<1) {
			prints(sid, "<CENTER>Folder name is too short</CENTER><BR>\n");
			return;
		}
		if ((sqr=sql_queryf(sid, "SELECT max(mailfolderid) FROM gw_mailfolders where accountid = %d", sid->dat->user_mailcurrent))<0) return;
		...
		strncatf(query, sizeof(query)-strlen(query)-1, "'%d', '%s', '%s', '%d', '0', '0', '0', '%d', ", folderid, curdate, curdate, sid->dat->user_uid, sid->dat->user_mailcurrent);
		...
		strncatf(query, sizeof(query)-strlen(query)-1, " WHERE mailfolderid = %d AND obj_uid = %d AND accountid = %d", folderid, sid->dat->user_uid, sid->dat->user_mailcurrent);
		if (sql_update(sid, query)<0) return;
