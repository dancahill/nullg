...
void ReadPOSTData(CONN *sid)
{
	...
	if (bytesleft<0) {
		log_error("ghttp", __FILE__, __LINE__, 1, "ERROR: negative Content-Length of %d provided by client.", sid->dat->in_ContentLength);
		sid->dat->in_ContentLength=0;
		closeconnect(sid, 1);
		return;
	} else if (bytesleft>http_proc.config.http_maxpostsize) {
		log_error("ghttp", __FILE__, __LINE__, 1, "ERROR: Content-Length of %d is too large.", sid->dat->in_ContentLength);
		sid->dat->in_ContentLength=0;
		closeconnect(sid, 1);
		return;
	}
	...
	if (sid->PostData==NULL) {
		log_error("ghttp", __FILE__, __LINE__, 1, "Memory allocation error while reading POST data.");
		closeconnect(sid, 1);
	}
	...
	if (http_proc.RunAsCGI) {
		...
		if (sid->socket.recvbufsize>bytesleft) {
			log_error("ghttp", __FILE__, __LINE__, 1, "ERROR: Content-Length is larger than advertised.");
			sid->dat->in_ContentLength=0;
			closeconnect(sid, 1);
			return;
		}
	...
	/* log_error("ghttp", __FILE__, __LINE__, 4, "[%s]", sid->PostData); */
	return;
}
...
void read_cgienv(CONN *sid)
{
	...
	if ((ptemp=getenv("CONTENT_LENGTH"))!=NULL) {
		...
		if (sid->dat->in_ContentLength<0) {
			log_error("ghttp", __FILE__, __LINE__, 2, "ERROR: negative Content-Length of %d provided by client.", sid->dat->in_ContentLength);
			sid->dat->in_ContentLength=0;
		}
	}
...
int read_header(CONN *sid)
{
	...
	if (*ptemp2) *ptemp2='\0';
/*
	log_error("ghttp", __FILE__, __LINE__, 2, "[%s]", line);
	log_error("ghttp", __FILE__, __LINE__, 2, "[%s][%s][%s]", sid->dat->in_RequestMethod, sid->dat->in_RequestURI, sid->dat->in_Protocol);
*/
...
*/
	...
	if (strcmp(sid->dat->in_RequestMethod, "POST")==0) {
		if (sid->dat->in_ContentLength<http_proc.config.http_maxpostsize) {
			...
			/* try to print an error : note the inbuffer being full may block us */
			send_error(sid, 413, "Bad Request", "Request entity too large.");
			log_error("ghttp", __FILE__, __LINE__, 1, "%s - Large POST (>%d bytes) disallowed", sid->dat->in_RemoteAddr, http_proc.config.http_maxpostsize);
			DEBUG_OUT(sid, "read_header()");
			closeconnect(sid, 1);
			return -1;
		}
...
void http_dorequest(CONN *sid)
{
	...
	log_error("ghttpd", __FILE__, __LINE__, 3, "%s - HTTP Request: %s %s", sid->dat->in_RemoteAddr, sid->dat->in_RequestMethod, sid->dat->in_RequestURI);
...
*/
	...
	if (strncmp(sid->dat->in_RequestURI, "/logout", 7)==0) {
		...
		log_error("ghttpd", __FILE__, __LINE__, 2, "%s - Bad request '%s' by %s", sid->dat->in_RemoteAddr, sid->dat->in_RequestURI, sid->dat->user_username);
	}
	DEBUG_OUT(sid, "dorequest()");
	return;
}
