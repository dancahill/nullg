...
#endif
{
	...
	log_error("core", __FILE__, __LINE__, 2, "Starting ghtloop() thread");
	...
	log_error("ghttpd", __FILE__, __LINE__, 4, "Opening connection thread [%u]", http_proc.conn[sid].socket.socket);
	...
	log_error("ghttpd", __FILE__, __LINE__, 4, "Closing connection thread [%u]", http_proc.conn[sid].socket.socket);
...
#endif
{
	...
	log_error("core", __FILE__, __LINE__, 1, "Starting ghttpd_accept_loop() thread (%s)", (conntype==CONN_SSL?"ssl":"std"));
#ifndef WIN32
	pthread_detach(pthread_self());
#endif
	if (pthread_attr_init(&thr_attr)) {
		log_error("ghttpd", __FILE__, __LINE__, 0, "pthread_attr_init()");
	...
	for (;;) {
		...
		if (conntype==CONN_STD) {
			...
			log_error("ghttpd", __FILE__, __LINE__, 0, "unknown connection type %d", conntype);
			exit(-1);
		}
		...
		if (http_proc.conn[sid].socket.socket==INVALID_SOCKET) {
			log_error("ghttpd", __FILE__, __LINE__, 2, "httpd_accept_loop() shutting down...");
			DEBUG_OUT(NULL, "httpd_accept_loop()");
			return 0;
#else
		if (http_proc.conn[sid].socket.socket<0) {
			...
			if (pthread_create(&http_proc.conn[sid].handle, &thr_attr, htloop, (void *)sid)==-1) {
				log_error("ghttpd", __FILE__, __LINE__, 0, "htloop() failed to start...");
				DEBUG_OUT(NULL, "httpd_accept_loop()");
				exit(-1);
			}
		}
...
DllExport int mod_init(_PROC *_proc, FUNCTION *_functions)
{
	...
	conf_read();
	lang_read();
	log_error("core", __FILE__, __LINE__, 1, "Starting %s ghttpd %s (%s)", SERVER_NAME, PACKAGE_VERSION, __DATE__);
...
DllExport int mod_cron()
{
	...
	for (i=0;i<http_proc.config.http_maxconn;i++) {
		...
		log_error("ghttpd", __FILE__, __LINE__, 4, "Reaping idle thread 0x%08X (idle %d seconds)", http_proc.conn[i].id, ctime-http_proc.conn[i].socket.atime);
...
{
	log_error("core", __FILE__, __LINE__, 1, "Stopping %s ghttpd %s (%s)", SERVER_NAME, PACKAGE_VERSION, __DATE__);
