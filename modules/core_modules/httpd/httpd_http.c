...
void ReadPOSTData(CONN *sid)
{
	...
	if (sid->PostData==NULL) {
		log_error("http", __FILE__, __LINE__, 1, "Memory allocation error while reading POST data.");
		closeconnect(sid, 1);
	}
	...
	if (http_proc.RunAsCGI) {
		...
		if (sid->socket.recvbufsize>bytesleft) {
			log_error("http", __FILE__, __LINE__, 1, "ERROR: Content-Length is larger than advertised.");
			sid->dat->in_ContentLength=0;
			closeconnect(sid, 1);
			return;
		}
...
void read_cgienv(CONN *sid)
{
	...
	if ((ptemp=getenv("CONTENT_LENGTH"))!=NULL) {
		...
		if (sid->dat->in_ContentLength<0) {
			log_error("http", __FILE__, __LINE__, 2, "ERROR: negative Content-Length of %d provided by client.", sid->dat->in_ContentLength);
			sid->dat->in_ContentLength=0;
		}
	}
...
void http_dorequest(CONN *sid)
{
	...
	log_error("httpd", __FILE__, __LINE__, 3, "%s - HTTP Request: %s %s", sid->dat->in_RemoteAddr, sid->dat->in_RequestMethod, sid->dat->in_RequestURI);
	...
	if (module_menucall(sid)) {
		...
		log_error("httpd", __FILE__, __LINE__, 2, "%s - Bad request '%s' by %s", sid->dat->in_RemoteAddr, sid->dat->in_RequestURI, sid->dat->user_username);
	}
	DEBUG_OUT(sid, "dorequest()");
	return;
}
