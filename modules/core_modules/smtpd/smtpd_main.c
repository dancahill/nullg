...
DllExport int mod_init(_PROC *_proc, FUNCTION *_functions)
{
	...
	if (mod_config.smtp_port) {
		if ((ListenSocketSTD=tcp_bind(mod_config.smtp_interface, mod_config.smtp_port))!=-1) loaded=1;
	}
	if ((proc->ssl_is_loaded)&&(mod_config.smtp_sslport)) {
		if ((ListenSocketSSL=tcp_bind(mod_config.smtp_interface, mod_config.smtp_sslport))!=-1) loaded=1;
	}
	if (loaded) {
...
DllExport int mod_exec()
{
...
#endif
	if ((mod_config.smtp_port)&&(ListenSocketSTD!=-1)) {
	...
	if ((mod_config.smtp_sslport)&&(ListenSocketSSL!=-1)&&(proc->ssl_is_loaded)) {
