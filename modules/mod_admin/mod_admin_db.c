...
//	if ((perm>1)&&(table==DB_PRODUCTS)&&(!(authlevel&A_ADMIN))) return -1;
	...
	user->authprojects	= atoi(sql_getvalue(sqr, 0, 27));
	user->authquery		= atoi(sql_getvalue(sqr, 0, 28));
	user->authwebmail	= atoi(sql_getvalue(sqr, 0, 29));
	user->prefdaystart	= atoi(sql_getvalue(sqr, 0, 30));
	user->prefdaylength	= atoi(sql_getvalue(sqr, 0, 31));
	user->prefmailcurrent	= atoi(sql_getvalue(sqr, 0, 32));
	user->prefmaildefault	= atoi(sql_getvalue(sqr, 0, 33));
	user->prefmaxlist	= atoi(sql_getvalue(sqr, 0, 34));
	user->prefmenustyle	= atoi(sql_getvalue(sqr, 0, 35));
	user->preftimezone	= atoi(sql_getvalue(sqr, 0, 36));
	user->prefgeozone	= atoi(sql_getvalue(sqr, 0, 37));
	strncpy(user->preflanguage,     sql_getvalue(sqr, 0, 38), sizeof(user->preflanguage)-1);
	strncpy(user->preftheme,        sql_getvalue(sqr, 0, 39), sizeof(user->preftheme)-1);
	strncpy(user->availability,     sql_getvalue(sqr, 0, 40), sizeof(user->availability)-1);
	strncpy(user->surname,          sql_getvalue(sqr, 0, 41), sizeof(user->surname)-1);
	strncpy(user->givenname,        sql_getvalue(sqr, 0, 42), sizeof(user->givenname)-1);
	strncpy(user->jobtitle,         sql_getvalue(sqr, 0, 43), sizeof(user->jobtitle)-1);
	strncpy(user->division,         sql_getvalue(sqr, 0, 44), sizeof(user->division)-1);
	strncpy(user->supervisor,       sql_getvalue(sqr, 0, 45), sizeof(user->supervisor)-1);
	strncpy(user->address,          sql_getvalue(sqr, 0, 46), sizeof(user->address)-1);
	strncpy(user->locality,         sql_getvalue(sqr, 0, 47), sizeof(user->locality)-1);
	strncpy(user->region,           sql_getvalue(sqr, 0, 48), sizeof(user->region)-1);
	strncpy(user->country,          sql_getvalue(sqr, 0, 49), sizeof(user->country)-1);
	strncpy(user->postalcode,       sql_getvalue(sqr, 0, 50), sizeof(user->postalcode)-1);
	strncpy(user->homenumber,       sql_getvalue(sqr, 0, 51), sizeof(user->homenumber)-1);
	strncpy(user->worknumber,       sql_getvalue(sqr, 0, 52), sizeof(user->worknumber)-1);
	strncpy(user->faxnumber,        sql_getvalue(sqr, 0, 53), sizeof(user->faxnumber)-1);
	strncpy(user->cellnumber,       sql_getvalue(sqr, 0, 54), sizeof(user->cellnumber)-1);
	strncpy(user->pagernumber,      sql_getvalue(sqr, 0, 55), sizeof(user->pagernumber)-1);
	strncpy(user->email,            sql_getvalue(sqr, 0, 56), sizeof(user->email)-1);
	strncpy(user->birthdate,        sql_getvalue(sqr, 0, 57), 10);
	strncpy(user->hiredate,         sql_getvalue(sqr, 0, 58), 10);
	strncpy(user->sin,              sql_getvalue(sqr, 0, 59), sizeof(user->sin)-1);
	strncpy(user->isactive,         sql_getvalue(sqr, 0, 60), sizeof(user->isactive)-1);
