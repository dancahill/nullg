...
int dbread_profile(CONN *sid, short int perm, int index, REC_USER *profile)
{
	...
	profile->domainid		= atoi(sql_getvalue(sqr, 0, 14));
	profile->enabled		= atoi(sql_getvalue(sqr, 0, 15));
	profile->authdomainadmin	= atoi(sql_getvalue(sqr, 0, 16));
	profile->authadmin		= atoi(sql_getvalue(sqr, 0, 17));
	profile->authbookmarks		= atoi(sql_getvalue(sqr, 0, 18));
	profile->authcalendar		= atoi(sql_getvalue(sqr, 0, 19));
	profile->authcalls		= atoi(sql_getvalue(sqr, 0, 20));
	profile->authcontacts		= atoi(sql_getvalue(sqr, 0, 21));
	profile->authfiles		= atoi(sql_getvalue(sqr, 0, 22));
	profile->authforums		= atoi(sql_getvalue(sqr, 0, 23));
	profile->authmessages		= atoi(sql_getvalue(sqr, 0, 24));
	profile->authorders		= atoi(sql_getvalue(sqr, 0, 25));
	profile->authprofile		= atoi(sql_getvalue(sqr, 0, 26));
	profile->authprojects		= atoi(sql_getvalue(sqr, 0, 27));
	profile->authquery		= atoi(sql_getvalue(sqr, 0, 28));
	profile->authwebmail		= atoi(sql_getvalue(sqr, 0, 29));
	profile->prefdaystart		= atoi(sql_getvalue(sqr, 0, 30));
	profile->prefdaylength		= atoi(sql_getvalue(sqr, 0, 31));
	profile->prefmailcurrent	= atoi(sql_getvalue(sqr, 0, 32));
	profile->prefmaildefault	= atoi(sql_getvalue(sqr, 0, 33));
	profile->prefmaxlist		= atoi(sql_getvalue(sqr, 0, 34));
	profile->prefmenustyle		= atoi(sql_getvalue(sqr, 0, 35));
	profile->preftimezone		= atoi(sql_getvalue(sqr, 0, 36));
	profile->prefgeozone		= atoi(sql_getvalue(sqr, 0, 37));
	strncpy(profile->preflanguage,     sql_getvalue(sqr, 0, 38), sizeof(profile->preflanguage)-1);
	strncpy(profile->preftheme,        sql_getvalue(sqr, 0, 39), sizeof(profile->preftheme)-1);
	strncpy(profile->availability,     sql_getvalue(sqr, 0, 40), sizeof(profile->availability)-1);
	strncpy(profile->surname,          sql_getvalue(sqr, 0, 41), sizeof(profile->surname)-1);
	strncpy(profile->givenname,        sql_getvalue(sqr, 0, 42), sizeof(profile->givenname)-1);
	strncpy(profile->jobtitle,         sql_getvalue(sqr, 0, 43), sizeof(profile->jobtitle)-1);
	strncpy(profile->division,         sql_getvalue(sqr, 0, 44), sizeof(profile->division)-1);
	strncpy(profile->supervisor,       sql_getvalue(sqr, 0, 45), sizeof(profile->supervisor)-1);
	strncpy(profile->address,          sql_getvalue(sqr, 0, 46), sizeof(profile->address)-1);
	strncpy(profile->locality,         sql_getvalue(sqr, 0, 47), sizeof(profile->locality)-1);
	strncpy(profile->region,           sql_getvalue(sqr, 0, 48), sizeof(profile->region)-1);
	strncpy(profile->country,          sql_getvalue(sqr, 0, 49), sizeof(profile->country)-1);
	strncpy(profile->postalcode,       sql_getvalue(sqr, 0, 50), sizeof(profile->postalcode)-1);
	strncpy(profile->homenumber,       sql_getvalue(sqr, 0, 51), sizeof(profile->homenumber)-1);
	strncpy(profile->worknumber,       sql_getvalue(sqr, 0, 52), sizeof(profile->worknumber)-1);
	strncpy(profile->faxnumber,        sql_getvalue(sqr, 0, 53), sizeof(profile->faxnumber)-1);
	strncpy(profile->cellnumber,       sql_getvalue(sqr, 0, 54), sizeof(profile->cellnumber)-1);
	strncpy(profile->pagernumber,      sql_getvalue(sqr, 0, 55), sizeof(profile->pagernumber)-1);
	strncpy(profile->email,            sql_getvalue(sqr, 0, 56), sizeof(profile->email)-1);
	strncpy(profile->birthdate,        sql_getvalue(sqr, 0, 57), 10);
	strncpy(profile->hiredate,         sql_getvalue(sqr, 0, 58), 10);
	strncpy(profile->sin,              sql_getvalue(sqr, 0, 59), sizeof(profile->sin)-1);
	strncpy(profile->isactive,         sql_getvalue(sqr, 0, 60), sizeof(profile->isactive)-1);
