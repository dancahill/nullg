...
int dbread_profile(CONN *sid, short int perm, int index, REC_USER *profile)
{
	...
	profile->authquery=atoi(sql_getvalue(sqr, 0, 27));
	profile->authwebmail=atoi(sql_getvalue(sqr, 0, 28));
	profile->prefdaystart=atoi(sql_getvalue(sqr, 0, 29));
	profile->prefdaylength=atoi(sql_getvalue(sqr, 0, 30));
	profile->prefmailcurrent=atoi(sql_getvalue(sqr, 0, 31));
	profile->prefmaildefault=atoi(sql_getvalue(sqr, 0, 32));
	profile->prefmaxlist=atoi(sql_getvalue(sqr, 0, 33));
	profile->prefmenustyle=atoi(sql_getvalue(sqr, 0, 34));
	profile->preftimezone=atoi(sql_getvalue(sqr, 0, 35));
	profile->prefgeozone=atoi(sql_getvalue(sqr, 0, 36));
