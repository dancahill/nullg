...
int mod_import()
{
	...
	if ((db_read                     = _get_func("db_read"))==NULL) return -1;
	if ((db_write                    = _get_func("db_write"))==NULL) return -1;
...
int mod_export_main(char *mod_name, char *mod_menuname, char *mod_menuuri, char *fn_name, char *fn_uri, void *fn_ptr)
{
	...
	for (i=0;i<50;i++) {
		if (strlen(mod_menuitems[i].mod_name)>0) continue;
		if (strlen(mod_menuitems[i].mod_menuname)>0) continue;
		if (mod_menuitems[i].fn_ptr!=NULL) continue;
		snprintf(mod_menuitems[i].mod_name, sizeof(mod_menuitems[i].mod_name)-1, "%s", mod_name);
		snprintf(mod_menuitems[i].mod_menuname, sizeof(mod_menuitems[i].mod_menuname)-1, "%s", mod_menuname);
		snprintf(mod_menuitems[i].mod_menuuri, sizeof(mod_menuitems[i].mod_menuuri)-1, "%s", mod_menuuri);
		snprintf(mod_menuitems[i].fn_name, sizeof(mod_menuitems[i].fn_name)-1, "%s", fn_name);
		snprintf(mod_menuitems[i].fn_uri, sizeof(mod_menuitems[i].fn_uri)-1, "%s", fn_uri);
		mod_menuitems[i].fn_ptr=fn_ptr;
		return 0;
	}
	if (logerror!=NULL) logerror(NULL, __FILE__, __LINE__, "ERROR: Failed to find function %s", fn_name);
	return -1;
...
int mod_export_function(char *mod_name, char *fn_name, void *fn_ptr)
{
	...
	for (i=0;i<50;i++) {
		if (strlen(mod_functions[i].mod_name)>0) continue;
		if (strlen(mod_functions[i].fn_name)>0) continue;
		if (mod_functions[i].fn_ptr!=NULL) continue;
		snprintf(mod_functions[i].mod_name, sizeof(mod_functions[i].mod_name)-1, "%s", mod_name);
		snprintf(mod_functions[i].fn_name, sizeof(mod_functions[i].fn_name)-1, "%s", fn_name);
		mod_functions[i].fn_ptr=fn_ptr;
		return 0;
	}
	if (logerror!=NULL) logerror(NULL, __FILE__, __LINE__, "ERROR: Failed to find function %s", fn_name);
	return -1;
