...
int smtp_client(FILE *fp)
{
	...
	memset(inbuffer, 0, sizeof(inbuffer));
	if ((smtp_sock=smtp_connect(proc.config.smtp_relayhost))<0) goto quit;
	do {
		memset(inbuffer, 0, sizeof(inbuffer));
	...
	if (strncasecmp(inbuffer, "250", 3)!=0) goto quit;
	logaccess(NULL, 1, "mail sent from: '%s', to: '%s'", from, rcpt);
	smtp_disconnect(smtp_sock);
	return 0;
quit:
	logerror(NULL, __FILE__, __LINE__, "remote server response: %s", inbuffer);
	smtp_disconnect(smtp_sock);
	return -1;
}
...
#endif
{
	...
	snprintf(dirname, sizeof(dirname)-1, "%s/queue", proc.config.server_dir_var_mail);
	fixslashes(dirname);
	while (1) {
