...
void WINAPI ServiceMain(DWORD dwNumServiceArgs, LPTSTR *lpServiceArgs)
{
	...
	ghCtrlStatus=RegisterServiceCtrlHandler("nullgw-smtpd", (LPHANDLER_FUNCTION)XControlHandler);
	if (ghCtrlStatus==(SERVICE_STATUS_HANDLE)NULL) {
		printf("Error in nullgw-smtpd_Main-RegisterServiceCtrlHandler!\r\n");
		DebugBreak();
	}
	ghevDoForever=CreateEvent(NULL, FALSE, FALSE, "Nullnullgw-smtpdRunEvt");
	...
	rc=SetServiceStatus(ghCtrlStatus, &gStatus);
	if (!rc) {
		printf("Error in nullgw-smtpd_Main-SetServiceStatus!\r\n");
		DebugBreak();
	}
	...
	if (pthread_attr_init(&thr_attr)) exit(1);
	if (pthread_attr_setstacksize(&thr_attr, 65536L)) exit(1);
	if (pthread_create(&trc, &thr_attr, smtp_spool, NULL)==-1) {
		logerror(NULL, __FILE__, __LINE__, "smtp_spool() thread failed to start.");
		exit(0);
	}
...
int main(int argc, char *argv[])
{
	...
	if ((g_dwOSVersion!=OS_WINNT)&&(g_dwOSVersion!=OS_WIN2K)) {
		...
		if (pthread_create(&spoolthread, &thr_attr, smtp_spool, NULL)==-1) {
			logerror(NULL, __FILE__, __LINE__, "smtp_spool() loop failed to start.");
			exit(0);
		}
	...
	ghevDoForever=NULL;
	if ((ghevDoForever=OpenEvent(SYNCHRONIZE, FALSE, "Nullnullgw-smtpdRunEvt"))!=0) {
		printf("nullgw-smtpd is already running.\r\n");
		CloseHandle(ghevDoForever);
		exit(-1);
	}
	if (isServiceInstalled()==1) {
		if (!strcmp(ptemp, "install")) {
			printf("nullgw-smtpd is already installed.\r\n");
			exit(0);
		} else if (!strcmp(ptemp, "remove")) {
			if (uninstallService()) {
				printf("nullgw-smtpd could not be uninstalled.\r\n");
				exit(-1);
			}
			exit(0);
		}
		...
		if (!strcmp(ptemp, "install")) {
			if (installService()==0) {
				printf("nullgw-smtpd was installed successfully.\r\n");
				exit(0);
			} else {
				printf("nullgw-smtpd could not be installed.\r\n");
				exit(-1);
			}
			exit(0);
		} else if (!strcmp(ptemp, "remove")) {
			printf("nullgw-smtpd is not installed.\r\n");
			exit(-1);
		}
	}
	SrvTable[0].lpServiceName = "nullgw-smtpd";
