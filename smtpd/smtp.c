...
static void smtp_accept(CONN *sid, MAILCONN *mconn)
{
	...
	for (i=0;i<mconn->num_rcpts;i++) {
		...
		if (strcasecmp(host, "localhost")==0) {
			userid=0;
			if ((sqr=sql_queryf(sid, "SELECT userid FROM gw_users WHERE username = '%s'", tmpaddr))<0) return;
			if (sql_numtuples(sqr)==1) userid=atoi(sql_getvalue(sqr, 0, 0));
			sql_freeresult(sqr);
			if (userid<1) continue;
			memset(tmpname, 0, sizeof(tmpname));
			snprintf(tmpname, sizeof(tmpname)-1, "%s/local/%s", proc.config.server_dir_var_mail, tmpaddr);
...
#endif
					logerror(NULL, __FILE__, __LINE__, "Error creating directory '%s'", tmpname);
					continue;
				}
			}
retry1:
			...
			memset(tmpname, 0, sizeof(tmpname));
			snprintf(tmpname, sizeof(tmpname)-1, "%s/local/%s/%d%03d.msg", proc.config.server_dir_var_mail, tmpaddr, (int)ttime.tv_sec, (int)(ttime.tv_usec/1000));
			fixslashes(tmpname);
			if (stat(tmpname, &sb)==0) goto retry1;
			logaccess(NULL, 1, "local delivery from: '%s', to: '%s'", mconn->from, mconn->rcpt[i]);
		} else if (mconn->sender_is_local) {
			is_remote=1;
retry2:
			gettimeofday(&ttime, &tzone);
			memset(tmpname, 0, sizeof(tmpname));
			snprintf(tmpname, sizeof(tmpname)-1, "%s/queue/%d%03d.msg", proc.config.server_dir_var_mail, (int)ttime.tv_sec, (int)(ttime.tv_usec/1000));
			fixslashes(tmpname);
			if (stat(tmpname, &sb)==0) goto retry2;
		} else {
			logerror(sid, __FILE__, __LINE__, "relaying denied.");
			continue;
		}
		if ((fp=fopen(tmpname, "wb"))!=NULL) {
			...
			logerror(sid, __FILE__, __LINE__, "ERROR: Cannot write to file [%s].", tmpname);
		}
	}
	return;
}
...
static void smtp_data(CONN *sid, MAILCONN *mconn)
{
	...
	prints(sid, "354 Send the mail data, end with .\r\n");
	...
	do {
		...
		if (sgets(sid, line, sizeof(line)-1, sid->socket)<0) return;
		if ((strcmp(line, ".\r\n")==0)||(strcmp(line, ".\n")==0)) break;
	...
	if (mconn->msgbody) { free(mconn->msgbody); mconn->msgbody=NULL; }
	prints(sid, "250 Message accepted for delivery\r\n");
	return;
}
...
static void smtp_from(CONN *sid, MAILCONN *mconn, char *line)
{
	...
	if (strlen(mconn->from)) {
		prints(sid, "250 Sender '%s' OK\r\n", mconn->from);
	} else {
		prints(sid, "500 Sender Invalid\r\n");
	}
	return;
}
...
//	strncpy(sid->dat->in_RemoteAddr, inet_ntoa(sid->ClientAddr.sin_addr), sizeof(sid->dat->in_RemoteAddr)-1);
	...
	do {
		...
		if (strcasecmp(line, "quit")==0) {
			...
			logerror(NULL, __FILE__, __LINE__, "UNKNOWN COMMAND: '%s'", line);
			prints(sid, "500 Unknown Command\r\n");
		}
	} while (1);
cleanup:
	for (i=0;i<mconn.num_rcpts;i++) {
