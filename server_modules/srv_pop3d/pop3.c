...
static void pop3_local(CONN *sid)
{
	...
	snprintf(dirname, sizeof(dirname)-1, "%s/local/%s", config->server_dir_var_mail, sid->dat->user_username);
...
static void pop3_remote(CONN *sid)
{
	...
	for (i=0;i<sql_numtuples(sqr);i++) {
		...
		snprintf(mdir.msg[i]->filename, sizeof(mdir.msg[i]->filename)-1, "%s/%04d/%06d.msg", config->server_dir_var_mail, sid->dat->user_mailcurrent, mdir.msg[i]->localid);
