...
DllExport int mod_init(_PROC *_proc, FUNCTION *_functions)
{
	...
	log_error("smtp", __FILE__, __LINE__, 1, "Starting %s %s (%s)", SERVER_NAME, SERVER_VERSION, __DATE__);
	if (config->smtp_port) {
...
//	log_access("smtp", 1, "Stopping %s %s (%s)", SERVER_NAME, SERVER_VERSION, __DATE__);
	log_error("smtp", __FILE__, __LINE__, 1, "Stopping %s %s (%s)", SERVER_NAME, SERVER_VERSION, __DATE__);
