...
int read_header(CONN *sid)
{
	...
	strncpy(sid->dat->in_RemoteAddr, inet_ntoa(sid->socket.ClientAddr.sin_addr), sizeof(sid->dat->in_RemoteAddr)-1);
	x=time(NULL);
	do {
...
//		prints(sid, "<BR><CENTER>The requested resource can be found at <A HREF=%s>%s</A>.</CENTER>\n", pageuri, pageuri);
	...
	if (strncmp(sid->dat->in_RequestURI, "/logout", 7)==0) {
		...
		log_error("http", __FILE__, __LINE__, 2, "%s - Bad request '%s' by %s", sid->dat->in_RemoteAddr, sid->dat->in_RequestURI, sid->dat->user_username);
	}
	DEBUG_OUT(sid, "dorequest()");
	return;
}
