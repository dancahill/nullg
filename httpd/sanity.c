...
#endif
		logerror(NULL, __FILE__, __LINE__, 1, "Error accessing directory '%s'", dirname);
		return -1;
	}
	return 0;
}
...
static int sanity_dbcheck_table(char *tablename, char *indexname, int numfields)
{
	...
	if ((sqr=sql_queryf(NULL, "SELECT * FROM %s WHERE %s = 1", tablename, indexname))<0) {
		logerror(NULL, __FILE__, __LINE__, 0, "ERROR: Could not access %s table!", tablename);
		exit(-2);
	}
	if (sql_numfields(sqr)!=numfields) {
		logerror(NULL, __FILE__, __LINE__, 0, "ERROR: %s has %d fields, but it should have %d!", tablename, sql_numfields(sqr), numfields);
		sql_freeresult(sqr);
		return -1;
	}
	sql_freeresult(sqr);
...
//	int x, y;
	...
	if (sanity_dircheck("%s", proc.config.server_dir_var_files)!=0) exit(-2);
	if (sanity_dircheck("%s", proc.config.server_dir_var_log)!=0) exit(-2);
	if (sanity_dircheck("%s", proc.config.server_dir_var_mail)!=0) exit(-2);
	if (sanity_dircheck("%s/local", proc.config.server_dir_var_mail)!=0) exit(-2);
	if (sanity_dircheck("%s/queue", proc.config.server_dir_var_mail)!=0) exit(-2);
	...
	while ((sqr=sql_query(NULL, "SELECT count(username) FROM gw_users"))<0) {
		...
		if (i>6) {
			...
			logerror(NULL, __FILE__, __LINE__, 0, "%s", msgbuffer);
	...
	if ((sqr=sql_query(NULL, "SELECT dbversion, tax1name, tax2name, tax1percent, tax2percent FROM gw_dbinfo"))<0) {
		logerror(NULL, __FILE__, __LINE__, 0, "Could not read dbinfo from database");
		exit(-1);
	}
	if (sql_numtuples(sqr)!=1) {
		sql_freeresult(sqr);
		logerror(NULL, __FILE__, __LINE__, 0, "Missing dbinfo data");
		exit(-1);
	}
...
/*
	...
	for (i=0;i<100;i++) {
		if ((sqr=sql_query(NULL, "SELECT * from gw_users"))<0) exit(-1);
		sql_freeresult(sqr);
	}
	gettimeofday(&ttime, &tzone);
	x=((ttime.tv_sec-x)*1000000)-y+ttime.tv_usec;
	logerror(NULL, __FILE__, __LINE__, 2, "Query speed test finished [%s][queries=100][time=%1.3f seconds]", proc.config.sql_type, (float)x/(float)1000000);
*/
	return 0;
}
