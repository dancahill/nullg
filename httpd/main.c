...
void WINAPI ServiceMain(DWORD dwNumServiceArgs, LPTSTR *lpServiceArgs)
{
	...
	ghCtrlStatus=RegisterServiceCtrlHandler("nullgw-httpd", (LPHANDLER_FUNCTION)XControlHandler);
	if (ghCtrlStatus==(SERVICE_STATUS_HANDLE)NULL) {
		printf("Error in nullgw-httpd_Main-RegisterServiceCtrlHandler!\r\n");
		DebugBreak();
	}
	ghevDoForever=CreateEvent(NULL, FALSE, FALSE, "Nullnullgw-httpdRunEvt");
	...
	rc=SetServiceStatus(ghCtrlStatus, &gStatus);
	if (!rc) {
		printf("Error in nullgw-httpd_Main-SetServiceStatus!\r\n");
		DebugBreak();
	}
	...
	if (pthread_create(&trc, &thr_attr, conn_reaper, NULL)==-1) {
		logerror(NULL, __FILE__, __LINE__, 0, "conn_reaper() thread failed to start.");
		exit(-2);
	}
...
int main(int argc, char *argv[])
{
	...
	if (getenv("REQUEST_METHOD")!=NULL) {
		...
		if ((g_dwOSVersion!=OS_WINNT)&&(g_dwOSVersion!=OS_WIN2K)) {
			...
			conn_reaper(NULL);
			logerror(NULL, __FILE__, __LINE__, 0, "conn_reaper() loop failed to start.");
			exit(-2);
		}
		memset(&gStatus, 0x00, sizeof(gStatus));
		ghevDoForever=NULL;
		if ((ghevDoForever=OpenEvent(SYNCHRONIZE, FALSE, "Nullnullgw-httpdRunEvt"))!=0) {
			printf("nullgw-httpd is already running.\r\n");
			CloseHandle(ghevDoForever);
			exit(-2);
		}
		if (isServiceInstalled()==1) {
			if (!strcmp(ptemp, "install")) {
				printf("nullgw-httpd is already installed.\r\n");
				exit(-2);
			} else if (!strcmp(ptemp, "remove")) {
				if (uninstallService()) {
					printf("nullgw-httpd could not be uninstalled.\r\n");
					exit(-2);
				}
				exit(0);
			}
			...
			if (!strcmp(ptemp, "install")) {
				if (installService()==0) {
					printf("nullgw-httpd was installed successfully.\r\n");
					exit(0);
				} else {
					printf("nullgw-httpd could not be installed.\r\n");
					exit(-2);
				}
				exit(0);
			} else if (!strcmp(ptemp, "remove")) {
				printf("nullgw-httpd is not installed.\r\n");
				exit(-2);
			}
		}
		SrvTable[0].lpServiceName = "nullgw-httpd";
