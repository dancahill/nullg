...
found:
	...
	return 0;
fail:
	logerror(NULL, __FILE__, __LINE__, 0, "ERROR: Failed to load %s", libname);
...
int mysqlConnect(CONN *sid)
{
	...
	if (!(mysock=libmysql.real_connect(&mysql, proc.config.sql_hostname, proc.config.sql_username, proc.config.sql_password, proc.config.sql_dbname, proc.config.sql_port, NULL, 0))) {
		logerror(sid, __FILE__, __LINE__, 1, "MYSQL error: %s", libmysql.error(&mysql));
		return -1;
	}
	sql_is_connected=1;
	return 0;
...
int odbcConnect(CONN *sid)
{
	...
	if (SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &hEnv)==SQL_ERROR) {
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Connect - Unable to allocate an environment handle.");
		return -1;
	}
	rc=SQLSetEnvAttr(hEnv, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3, 0);
	if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
		SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Connect - SQLSetEnvAttr %s", buf);
		odbcDisconnect();
		return -1;
	}
	...
	if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
		SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Connect - SQLAllocHandle %s", buf);
		odbcDisconnect();
		return -1;
	}
	...
	if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
		SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Connect - SQLDriverConnect %s", buf);
		odbcDisconnect();
		return -1;
	}
	sql_is_connected=1;
...
int pgsqlConnect(CONN *sid)
{
	...
	if (libpgsql.status(pgconn)==CONNECTION_BAD) {
		logerror(sid, __FILE__, __LINE__, 1, "PGSQL Connect - %s", libpgsql.errormessage(pgconn));
		return -1;
	}
	sql_is_connected=1;
	return 0;
...
int sqliteConnect(CONN *sid)
{
	...
	if (db==0) {
		logerror(sid, __FILE__, __LINE__, 1, "SQLite Connect - %s", zErrMsg);
		return -1;
	}
	sql_is_connected=1;
...
int mysqlUpdate(CONN *sid, char *sqlquery)
{
	...
	if (libmysql.query(mysock, sqlquery)) {
		logerror(sid, __FILE__, __LINE__, 1, "MYSQL error: %s", libmysql.error(&mysql));
		logerror(sid, __FILE__, __LINE__, 2, "MYSQL: [%s]", sqlquery);
		return -1;
	}
	return 0;
#else
...
int odbcUpdate(CONN *sid, char *sqlquery)
{
	...
	if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
		SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Update - SQLAllocHandle %s", buf);
		return -1;
	}
	rc=SQLExecDirect(hStmt, sqlquery, SQL_NTS); 
	if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
		SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Update - SQLExecDirect %s", buf);
		logerror(sid, __FILE__, __LINE__, 2, "ODBC UPDATE: [%s]", sqlquery);
...
int pgsqlUpdate(CONN *sid, char *sqlquery)
{
	...
	if (libpgsql.resultstatus(pgres)!=PGRES_COMMAND_OK) {
		logerror(sid, __FILE__, __LINE__, 1, "PGSQL error: %s", libpgsql.errormessage(pgconn));
		return -1;
	}
	libpgsql.clear(pgres);
...
int mysqlQuery(CONN *sid, int sqr, char *sqlquery)
{
	...
	for (tuple=0;;tuple++) {
		...
		if (sqlreply[sqr].cursor[tuple]==NULL) {
			logerror(sid, __FILE__, __LINE__, 1, "malloc() error creating SQL cursor tuple.");
			exit(-1);
		}
...
int odbcQuery(CONN *sid, int sqr, char *sqlquery)
{
	...
	if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
		SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Query - SQLAllocHandle %s", buf);
		return -1;
	}
	rc=SQLExecDirect(hStmt, sqlquery, SQL_NTS); 
	if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
		SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Query - SQLExecDirect %s", buf);
		logerror(sid, __FILE__, __LINE__, 2, "ODBC SELECT: [%s]", sqlquery);
		return -1;
	}
	rc=SQLNumResultCols(hStmt, &pccol);
	if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
		SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
		logerror(sid, __FILE__, __LINE__, 1, "ODBC Query - SQLNumResultCols %s", buf);
		return -1;
	}
	...
	for (field=0;field<sqlreply[sqr].NumFields;field++) {
		...
		if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
			SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
			logerror(sid, __FILE__, __LINE__, 1, "ODBC Query - SQLDescribeCol %s", buf);
			return -1;
		}
		column+=strlen(column)+1;
	}
	...
	for (tuple=0;;tuple++) {
		...
		for (field=0;field<sqlreply[sqr].NumFields;field++) {
			...
			if ((rc!=SQL_SUCCESS)&&(rc!=SQL_SUCCESS_WITH_INFO)) {
				SQLError(hEnv, hDBC, hStmt, sqlstate, NULL, buf, sizeof(buf), NULL);
				logerror(sid, __FILE__, __LINE__, 1, "ODBC Getvalue - SQLGetData %s", buf);
				return -1;
			}
			column+=strlen(column)+1;
		}
...
int pgsqlQuery(CONN *sid, int sqr, char *sqlquery)
{
	...
	if (libpgsql.resultstatus(pgres) != PGRES_COMMAND_OK) {
		logerror(sid, __FILE__, __LINE__, 1, "PGSQL Query - %s", libpgsql.errormessage(pgconn));
		pgsqlDisconnect();
		return -1;
	}
	libpgsql.clear(pgres);
	...
	for (tuple=0;tuple<sqlreply[sqr].NumTuples;tuple++) {
		sqlreply[sqr].cursor[tuple]=calloc(MAX_TUPLE_SIZE, sizeof(char));
		if (sqlreply[sqr].cursor[tuple]==NULL) {
			logerror(sid, __FILE__, __LINE__, 1, "Memory allocation error while creating SQL cursor tuple.");
//			closeconnect(1);
			exit(-1);
		}
	}
...
void sql_disconnect(CONN *sid)
{
	...
	logerror(sid, __FILE__, __LINE__, 4, "SQL Disconnection");
...
//	logerror(NULL, __FILE__, __LINE__, 3, "SQL query [%d] freed", sqr);
	if (!proc.RunAsCGI) pthread_mutex_unlock(&Lock.SQL);
	return;
}
...
int sql_update(CONN *sid, char *sqlquery)
{
	...
	if (sid==NULL) {
		logerror(sid, __FILE__, __LINE__, 2, "SQL update by system: %s", sqlquery);
	} else {
		logerror(sid, __FILE__, __LINE__, 2, "SQL update by %s: %s", sid->dat->user_username, sqlquery);
	}
	if (strcmp(proc.config.sql_type, "ODBC")==0) {
