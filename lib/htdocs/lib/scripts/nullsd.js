...
/*
	...
	for (var i = 0; i < allElements.length; i++) {
		var x = allElements[i].getAttribute("aria-describedby");
...
//		var divname='contact_'+contactid;
	...
	emailaddrbook: {
		list: function (field, email) {
			contacts.emailaddrbook.outfield=field;
		...
		add: function (email) {
			var dest=document.getElementById('msg'+contacts.emailaddrbook.outfield);
			if (dest==null) {
				mail.compose(email);
				return;
			}
...
mail = {
	...
	view: function (emailid) {
		...
		for (i=0; i<table.rows.length; i++) {
			...
			if (div!=null) div.className = ''; // remove 'unread' class from tr
		}
		$.ajax({
...
*/
	...
	compose: function (emailid) {
		...
		$.ajax({
			...
			$('#'+divname).html(msg);
			if (emailaddr!="") {
				contacts.emailaddrbook.outfield="to";
				contacts.emailaddrbook.add(emailaddr);
			}
			dialog = $( "#"+divname ).dialog({
				autoOpen: false,
				height: $(window).height()*0.98,
...
_status = {
	check: function () {
		setTimeout(function() { _status.check(); }, 30000);
	...
	show: function (message) {
		...
		setTimeout(function() { _status.hide(); }, 10000);
	},
	hide: function () {
		var div=document.getElementById("status_popup");
		//div.innerHTML = "<span class=msg>goodbye</span>";
