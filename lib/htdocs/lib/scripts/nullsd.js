...
ns = {
	...
	contacts: {
		...
		location: {
			...
			viewmap: function (event, id) {
				ns.ModalForm(event, 'contactmap', id);
			},
			viewhistory: function (event, id) {
				ns.ModalForm(event, 'contacttimeline', id);
			},
			...
			map_history: function (rows) {
				...
				var mdx = document.getElementById("modal_timeline_x");
	...
	mail: {
		...
		*/
		...
		accountview(event, id) {
			ns.ModalForm(event, 'mailaccount', id);
		}
	},
	notes: {
		// why is this not using the generic version of view and submit?
		view: function (noteid) {
			var divname = 'note_form';
			ns.SetupDialogForm(divname);
			$.ajax({
				url: "/app/notes/note?id=" + noteid + "&method=ajax",
				context: document.body
			}).done(function (msg) {
				$('#' + divname).html(msg);
				dialog = $("#" + divname).dialog({
	...
	projects: {
		view: function (event, id) {
			ns.ModalForm(event, 'project', id);
		},
	},

	tasks: {
		view: function (event, id) {
			ns.ModalForm(event, 'task', id);
		},
	},
	// finish modules
	...
	status: {
		...
		show: function (message) {
			...
			setTimeout(function () { ns.status.hide(); }, 30000);
		},
		hide: function () {
			var div = document.getElementById("status_popup");
	...
	ModalForm: function (event, module, id) {
		try {
			var autoopen = false;
			var divname = 'modal_x';
			var divheight = 400;
			...
			switch (module) {
				case 'bookmark':
					//var url="/app/bookmarks/bookmark?id="+id+"&method=ajax";
					var title = "Bookmark: " + id;
					break;
				case 'bookmarkfolder':
					//var url="/app/bookmarks/bookmarkfolder?id="+id+"&method=ajax";
					var title = "Bookmark Folder: " + id;
					break;
				case 'contact':
					var url = "/app/contacts/contact?id=" + id + "&method=ajax";
					var title = "Contact: " + id;
					break;

				case 'contactmap':
					var url = "/app/contacts/location?id=" + id + "&method=ajax";
					var title = "Contact Map: " + id;
					break;
				case 'contacttimeline':
					var url = "/app/contacts/timeline?id=" + id + "&method=ajax";
					var title = "Contact Timeline: " + id;
					var divheight = $(window).height() * 0.98;
					var divwidth = $(window).width() * 0.98;
					var divname = 'modal_timeline_x';
					break;

				case 'event':
					if (url === undefined) var url = "/app/calendar/event?id=" + id + "&method=ajax";
					var title = "Event: " + id;
					break;
				case 'mailaccount':
					var url = "/app/mail/accounts/account?id=" + id + "&method=ajax";
					var title = "E-Mail Account: " + id;
					break;
				...
				case 'project':
					var url = "/app/projects/project?id=" + id + "&method=ajax";
					var title = "Project: " + id;
					break;
				case 'task':
					if (url === undefined) var url = "/app/tasks/task?id=" + id + "&method=ajax";
					var title = "Task: " + id;
					break;
				default:
					//throw "invalid module "+module;
					var title = "undefined module '" + module + "'";
					break;
			}
			var windowwidth = $(window).width() - 8;
			if (windowwidth < divwidth) divwidth = windowwidth;
	...
	ModalSubmit: function (event, module, action) {
		...
		try {
			...
			switch (module) {
				...
				case 'mailaccount':
					...
					var formURL = "/app/mail/accounts/account";
					break;
				case 'mailsearch':
			...
			$.ajax({
				...
				$('#' + divname).html(msg);
				// switch (formURL) {
				// 	case "/app/bookmarks/list":
				// 		//bad hack. maybe divname == 'main' would be a better check?
				// 		break;
				// 	default:
				// 		ns.ReGoTo();
				// }
