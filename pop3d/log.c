...
void logaccess(CONN *sid, int loglevel, const char *format, ...)
{
	...
	snprintf(file, sizeof(file)-1, "%s/pop3-access.log", proc.config.server_dir_var_log);
	fixslashes(file);
	fp=fopen(file, "a");
	if (fp!=NULL) {
		va_start(ap, format);
...
void logerror(CONN *sid, char *srcfile, int line, const char *format, ...)
{
	...
	snprintf(file, sizeof(file)-1, "%s/pop3-error.log", proc.config.server_dir_var_log);
	fixslashes(file);
	fp=fopen(file, "a");
	if (fp!=NULL) {
		va_start(ap, format);
...
void logdebug(CONN *sid, char *srcfile, int line, const char *format, ...)
{
	...
	snprintf(file, sizeof(file)-1, "%s/pop3-debug.log", proc.config.server_dir_var_log);
	fixslashes(file);
	fp=fopen(file, "a");
	if (fp!=NULL) {
		va_start(ap, format);
