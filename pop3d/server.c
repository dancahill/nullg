...
static int get_sid()
{
	...
	for (sid=0;sid<proc.config.pop3_maxconn;sid++) {
		if (conn[sid].id==pthread_self()) break;
	}
	if ((sid<0)||(sid>=proc.config.pop3_maxconn)) return -1;
	return sid;
}
...
#endif
{
	...
	if (sid) {
		logerror(sid, __FILE__, __LINE__, "%s - POP3 Request: ", inet_ntoa(sid->ClientAddr.sin_addr));
	}
//	closeconnect(sid, 2);
	pthread_exit(0);
}
	...
	for (;;) {
		...
		for (i=0;i<proc.config.pop3_maxconn;i++) {
			...
			if (conn[i].state==0) {
				...
				if (ctime-conn[i].atime<proc.config.pop3_maxidle) continue;
				logaccess(NULL, 4, "Reaping idle thread 0x%08X (idle %d seconds)", conn[i].id, ctime-conn[i].atime);
			}
...
void init()
{
	...
	if ((strcasecmp("ANY", proc.config.pop3_hostname)==0)||(strcasecmp("INADDR_ANY", proc.config.pop3_hostname)==0)) {
#ifndef WIN32
		logaccess(NULL, 2, "Binding to 'INADDR_ANY:%d'", proc.config.pop3_port);
#endif
		sin.sin_addr.s_addr=htonl(INADDR_ANY);
	} else {
#ifndef WIN32
		logaccess(NULL, 2, "Binding to '%s:%d'", proc.config.pop3_hostname, proc.config.pop3_port);
#endif
		hp=gethostbyname(proc.config.pop3_hostname);
		memmove((char *)&sin.sin_addr, hp->h_addr, hp->h_length);
	}
	sin.sin_port=htons((unsigned short)proc.config.pop3_port);
	option=1;
	...
	while (bind(proc.ListenSocket, (struct sockaddr *)&sin, sizeof(sin))<0) {
		...
		i++;
		if (i>6) {
			logerror(NULL, __FILE__, __LINE__, "bind() error [%s:%d]", proc.config.pop3_hostname, proc.config.pop3_port);
			perror("\r\nBind error");
			exit(0);
		}
	}
	...
	conn=calloc(proc.config.pop3_maxconn, sizeof(CONN));
	if (conn==NULL) {
		printf("\r\nconn calloc(%d, %d) failed\r\n", proc.config.pop3_maxconn, sizeof(CONN));
		exit(0);
	}
	for (i=0;i<proc.config.pop3_maxconn;i++) {
		conn[i].socket=-1;
...
void dorequest(CONN *sid)
{
	...
	mbox=0;
	prints(sid, "+OK Welcome to %s POP3d\r\n", SERVER_NAME);
	do {
	...
	sid->state=1;
	logaccess(sid, 1, "SUCCESSFUL AUTH: %s", username);
	if (mbox>0) {
		pop3_remote(sid);
	} else {
		pop3_local(sid);
