...
void WINAPI ServiceMain(DWORD dwNumServiceArgs, LPTSTR *lpServiceArgs)
{
	...
	ghCtrlStatus=RegisterServiceCtrlHandler("groupware", (LPHANDLER_FUNCTION)XControlHandler);
	if (ghCtrlStatus==(SERVICE_STATUS_HANDLE)NULL) {
		printf("Error in groupware_Main-RegisterServiceCtrlHandler!\r\n");
		DebugBreak();
	}
	ghevDoForever=CreateEvent(NULL, FALSE, FALSE, "NullGroupwareRunEvt");
	...
	rc=SetServiceStatus(ghCtrlStatus, &gStatus);
	if (!rc) {
		printf("Error in groupware_Main-SetServiceStatus!\r\n");
		DebugBreak();
	}
...
int main(int argc, char *argv[])
{
	...
	if (strcasestr(ptemp, ".exe")!=NULL) {
		ptemp=strcasestr(ptemp, ".exe");
	}
	...
	if (getenv("REQUEST_METHOD")!=NULL) {
		...
		ghevDoForever=NULL;
		if ((ghevDoForever=OpenEvent(SYNCHRONIZE, FALSE, "NullGroupwareRunEvt"))!=0) {
			printf("groupware is already running.\r\n");
			CloseHandle(ghevDoForever);
			exit(-1);
		}
		if (isServiceInstalled()==1) {
			if (!strcmp(ptemp, "install")) {
				printf("groupware is already installed.\r\n");
				exit(0);
			} else if (!strcmp(ptemp, "remove")) {
				if (uninstallService()) {
					printf("groupware could not be uninstalled.\r\n");
					exit(-1);
				}
				exit(0);
			}
			...
			if (!strcmp(ptemp, "install")) {
				if (installService()==0) {
					printf("groupware was installed successfully.\r\n");
					exit(0);
				} else {
					printf("groupware could not be installed.\r\n");
					exit(-1);
				}
				exit(0);
			} else if (!strcmp(ptemp, "remove")) {
				printf("groupware is not installed.\r\n");
				exit(-1);
			}
		}
		SrvTable[0].lpServiceName = "groupware";
